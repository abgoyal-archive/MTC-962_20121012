				...
				struct btd_device *device, gboolean success)
{
	...
	dtun_ssp_confirm_reply(&bdaddr.b, success);
#else
...
#ifdef BT_ALT_STACK
       error( "...oob_data_cb unimplemented" );
#else
...
#ifdef BT_ALT_STACK
       error( "...io_capa_oob_response unimplemented" );
#else
	dd = hci_open_dev(dev_id);
	if (dd < 0) {
		error("Unable to open hci%d", dev_id);
...
void hcid_dbus_notify_second_connection(bdaddr_t *local, bdaddr_t *peer, uint8_t result)  
{
	...
	adapter_emit_second_conn_info(adapter, peer, result);
}

void hcid_dbus_notify_slc_up(bdaddr_t *local, bdaddr_t *peer)  
{
	struct btd_adapter *adapter = manager_find_adapter(local);	
	if (! adapter) {
		error("Unable to find matching adapter");
	...
	adapter_emit_slc_up(adapter, peer);
}
				...
				int pin_length, uint8_t old_key_type)
{
	...
	info("key type 0x%02x old key type 0x%02x new key type 0x%02x",
					key_type, old_key_type, new_key_type);

	info("local auth 0x%02x and remote auth 0x%02x",
				...
				uint16_t handle, uint8_t reason)
{
	...
	adapter_remove_connection(adapter, device, handle, reason);
}
