...
#ifdef WAPI
int wpa_state_to_wapi_state(wpa_states wpa_state)
{
	switch (wpa_state) {
...
#ifdef WAPI
	if ( (ssid->wapi) || (ssid->proto & WPA_PROTO_WAPI) || (ssid->key_mgmt == WPA_KEY_MGMT_WAPI_PSK ) || (ssid->key_mgmt == WPA_KEY_MGMT_WAPI_CERT ) ) { /* associating to a WAPI network */

		wpa_printf(MSG_DEBUG, "WAPI ssid->wapi %d ssid->proto %d ssid->key_mgmt %d", ssid->wapi,
		...
		if (bss) {
			...
			if (bss->wapi_ie_len) {
				memcpy(wpa_s->ap_wapi_ie, wpa_scan_get_ie(bss, WLAN_EID_WAPI), bss->wapi_ie_len); 
		...
		//wpa_s->wapi_state = WAPI_ASSOCIATED;

		if (wapi_drv_set_wapi(wpa_s, wpa_s->wapi_conf->wapi_policy) < 0) {
			wpa_printf(MSG_ERROR, "Failed to enable WAPI in the driver.\n");
			return;
		}
		...
		if (!memcmp(wpa_s->bssid, "\x00\x00\x00\x00\x00\x00", ETH_ALEN)) /*2006-11-13*/
		{
			...
			if (wpa_drv_associate(wpa_s, &params))
				wpa_printf(MSG_DEBUG, "wapi_drv_associate() failed\n");
		}
...
int wpa_drv_scan(struct wpa_supplicant *wpa_s, struct wpa_ssid **ssid_ptr)
{
	...
	size_t ssid_len = 0;
	int ret = -1;
#if 0 //MPS-Conn: Disable Combo till driver support is implemented
...
int wpa_supplicant_driver_init(struct wpa_supplicant *wpa_s)
{
	...
	wpa_printf(MSG_DEBUG, "Own WAPI MAC address: " MACSTR, 
