...
//#define ENABLE_VOIP_LOOPBACK_DUMP
#ifdef ENABLE_VOIP_LOOPBACK_DUMP
struct file *fd;
...
#ifdef ENABLE_VOIP_LOOPBACK_DUMP
static void VoIP_Loopback_Dump(struct work_struct *work)
{
	if(fd)
	{
		fd->f_op->write(fd, &voip_loopbackbuffer[VOIP_buf_ul_index], LB_VOIP_FRAME_SIZE, &fd->f_pos);
	}
}
#endif
...
//        UInt32 lb_output;
    ...
    {
        // Start voip loopback - 100,mic,speaker,delay
           ...
           case 100:
            {
				if( !isLoopback )
				{
...
#ifdef ENABLE_VOIP_LOOPBACK_DUMP
	                fd = filp_open( "/data/voip.pcm", O_WRONLY |O_TRUNC |O_LARGEFILE |O_CREAT,0666);
	                if (IS_ERR(fd)) {
	                ...
	                intr_workqueue = create_workqueue("voip_loopback-dump");
	                if (!intr_workqueue)
	                {
						return TRUE;
	                }
	                INIT_WORK(&intr_work, VoIP_Loopback_Dump);
#endif	
                	...
                	// save the audio mode
	                AUDCTRL_SaveAudioModeFlag(voip.val3, AUDIO_APP_VOICE_CALL);
	                ...
	                // disable NS to pass loopback test case. 
	                if (voip.val3 == AUDIO_MODE_HEADSET)
						AUDCTRL_NS(FALSE); // set FALSE before AUDCTRL_EnableTelephony() calls AUDDRV_Telephony_Init().
					else if(voip.val3 == AUDIO_MODE_SPEAKERPHONE)
					{	//enable NS & EC for speaker output to prevent echo & noise
						AUDCTRL_NS(TRUE);
						AUDCTRL_EC(TRUE, 0);
					}
...
)
{
    ...
    memcpy(&voip_loopbackbuffer[VOIP_buf_ul_index], pSrc, LB_VOIP_FRAME_SIZE);
    ...
    VOIP_buf_ul_index += LB_VOIP_FRAME_SIZE;

    if ( VOIP_buf_ul_index >= LB_VOIP_BUFFER_SIZE )
    {
        VOIP_buf_ul_index = 0;
    }
    return TRUE;
...
{
    DEBUG(" VOIP_FillDL_CB :  VOIP_buf_dl_index 0x%x \r\n", VOIP_buf_dl_index);
	memcpy(pDst, &voip_loopbackbuffer[VOIP_buf_dl_index], LB_VOIP_FRAME_SIZE);

    VOIP_buf_dl_index += LB_VOIP_FRAME_SIZE;
    if ( VOIP_buf_dl_index >= LB_VOIP_BUFFER_SIZE )
    {
        VOIP_buf_dl_index = 0;
    }
    return TRUE;
