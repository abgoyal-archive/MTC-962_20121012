				...
				)
{
	...
	switch (tap)
	{
		case AUDIO_HW_TAP_VOICE:
            ...
            VPRIPCMDQ_SetBTNarrowBand( 0x11d );
...
void setExternalPreAmpGain(Int16 gain)
{
    ...
    {
        //max89xx_set_input_preamp_gain(MAX8986_INPUTB, gain);
        max89xx_set_ihf_preamp_gain(gain);
    }
    else if (audioMode == AUDIO_MODE_HEADSET)
    {
        //max89xx_set_input_preamp_gain(MAX8986_INPUTA, gain);
        max89xx_set_hs_preamp_gain(gain);
    }
}
...
void setExternalAmpGain(Int16 gain)
{
    ...
    if(audioMode == AUDIO_MODE_HANDSET || 
       audioMode == AUDIO_MODE_SPEAKERPHONE)
    {
        max89xx_audio_hs_ihf_set_gain(gain);
    }
    else if (audioMode == AUDIO_MODE_HEADSET)
    {
        max89xx_audio_hs_set_gain(AUDIO_HS_BOTH, gain);
    }
}
...
#elif PMU_MAX8986
            max89xx_audio_hs_poweron(FALSE);
#endif
...
#elif PMU_MAX8986
            max89xx_audio_hs_set_gain(hs_path, hs_gain);
            //max89xx_set_input_preamp_gain(MAX8986_INPUTA, preamp_gain);
            max89xx_set_hs_preamp_gain(preamp_gain);
#endif
		if ( HS_IsOn != TRUE )
		{
...
#elif PMU_MAX8986
            max89xx_audio_hs_ihf_set_gain(ihf_gain);
            //max89xx_set_input_preamp_gain(MAX8986_INPUTB, preamp_gain);
             max89xx_set_ihf_preamp_gain(preamp_gain);
#endif
		if ( IHF_IsOn != TRUE )
		{
