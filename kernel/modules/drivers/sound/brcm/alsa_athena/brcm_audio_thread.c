	...
	)
{
    ...
    switch (action_code)
    {
        ...
        new_mode = sgTableIDChannelOfDev[param_start->substream_number].speaker;
			...
			AUDIO_Policy_AddModeToQueue(BRCM_STATE_NORMAL, sgTableIDChannelOfDev[param_start->substream_number].speaker,app_profile);
        ...
        case ACTION_AUD_StopPlay:
        {
			...
			AudioMode_t  prev_mode = AUDIO_MODE_NUMBER_VOICE;
			AudioApp_t prev_app = AUDIO_APP_MUSIC;
		...
		if(AUDIO_Policy_RemoveModeFromQueue(BRCM_STATE_NORMAL, &prev_mode, &prev_app))
			AUDCTRL_SaveAudioModeFlag( prev_mode,prev_app);
        ...
        case ACTION_AUD_StartRecord:
        {
            ...
            AUDIO_DRIVER_Ctrl(param_start->drv_handle,AUDIO_DRIVER_START,NULL);
			 ...
			 AUDIO_Policy_AddModeToQueue(BRCM_STATE_RECORD, new_mode,app_profile);
        ...
        case ACTION_AUD_StopRecord:
        {
            BRCM_AUDIO_Param_Stop_t* param_stop = (BRCM_AUDIO_Param_Stop_t*) arg_param;
			AudioMode_t  prev_mode = AUDIO_MODE_NUMBER_VOICE;
			AudioApp_t prev_app = AUDIO_APP_RECORDING;
			...
			if(AUDIO_Policy_RemoveModeFromQueue(BRCM_STATE_RECORD, &prev_mode, &prev_app))
				AUDCTRL_SaveAudioModeFlag(prev_mode,prev_app);
	...
	case ACTION_AUD_OpenRecord:
        ...
        case ACTON_VOICECALL_START:
        {
			...
			//AUDIO_Policy_AddModeToQueue(BRCM_STATE_INCALL, audio_mode,app_profile);
        ...
        case ACTON_VOICECALL_STOP:
        {
			...
			//AudioMode_t  prev_mode = AUDIO_MODE_NUMBER_VOICE;
			//AudioApp_t prev_app = AUDIO_APP_VOICE_CALL;
            ...
            telephony_started = 0;
			//if(AUDIO_Policy_RemoveModeFromQueue(BRCM_STATE_INCALL, &prev_mode, &prev_app))
			//	AUDCTRL_SaveAudioModeFlag( prev_mode,prev_app);
             AUDIO_Policy_RestoreState();
        }
        break;
...
*/						
			...
			//AUDIO_Policy_AddModeToQueue(BRCM_STATE_FM, sgTableIDChannelOfDev[param_start->substream_number].speaker, app_profile);
        ...
        case ACTON_FM_STOP:
        {
			...
			//AudioMode_t  prev_mode = AUDIO_MODE_NUMBER_VOICE;
			//AudioApp_t prev_app = AUDIO_APP_FM;
			...
			//if(AUDIO_Policy_RemoveModeFromQueue(BRCM_STATE_FM, &prev_mode, &prev_app))
			//	AUDCTRL_SaveAudioModeFlag( prev_mode,prev_app);
            AUDIO_Policy_RestoreState();
        }
        break;
