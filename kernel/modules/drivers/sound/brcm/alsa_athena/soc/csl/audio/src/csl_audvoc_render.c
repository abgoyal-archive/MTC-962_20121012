...
{
	Log_DebugPrintf(LOGID_SOC_AUDIO, "csl_audvoc_render_pause:: streamID=%d.\n", streamID);

#if 0
// disable DMA
	if (streamID == CSL_AUDVOC_STREAM_AUDIO)
		AUDDRV_DisableHWOutput ( AUDDRV_AUDIO_OUTPUT, AUDDRV_REASON_DATA_DRIVER );
	else if (streamID == CSL_AUDVOC_STREAM_POLYRINGER)
		AUDDRV_DisableHWOutput ( AUDDRV_RINGTONE_OUTPUT, AUDDRV_REASON_DATA_DRIVER );
#else
