...
static void auddrv_selectSpkrDrvMode( AUDDRV_SPKR_Enum_t speaker );
static void auddrv_deselectSpkrDrv( AUDDRV_SPKR_Enum_t speaker );
...
#if ( defined(_ATHENA_) && CHIP_REVISION == 20 )  //B0
		//chal_audiovopath_DisableDitheringShuffling( vopath_handle, TRUE );
#endif
        // For B1 we need to enable suppression shuffling
        chal_audiovopath_EnableSuppressionShuffling(vopath_handle);
...
"	Turn on the digital slope gain
	**/
	...
	//#2
	auddrv_selectSpkrDrvMode( speaker );
...
***/
	...
	AUDDRV_SetVCflag( FALSE );  //audio_vdriver clears this flag too.
        auddrv_mixer_remove_input( AUDDRV_VOICE_OUTPUT, voiceSpkr1 );
...
//if not, then enable voice input later than enable voice output could have noise in ADC?
			...
			auddrv_mixer_add_input( AUDDRV_VOICE_OUTPUT, local_mixer_speaker_voice, AUDIO_CHANNEL_MONO );
				auddrv_selectSpkrDrvMode( local_mixer_speaker_voice );
		...
		case AUDDRV_AUDIO_OUTPUT:
			...
			auddrv_selectSpkrDrvMode( local_mixer_speaker_audio );
...
#if 0
            //enable this for direct interrupt and comment the previous line
            //chal_audioaopath_DisableAudInt( NULL, FALSE );
#endif
			chal_audioaopath_SetSampleRate (NULL, local_sample_rate_audio );
			...
			client_SetMusicMode( client_GetAudioMode(), client_GetAudioApp());
		...
		case AUDDRV_RINGTONE_OUTPUT:
			...
			auddrv_selectSpkrDrvMode( local_mixer_speaker_poly );
			...
			chal_audiopopath_SetSampleRate (NULL, local_sample_rate_poly );
	...
	switch(path)
	{
		...
		case AUDDRV_RINGTONE_OUTPUT:
			...
			polySpkr2 = AUDDRV_SPKR_NONE;
                        ...
                        auddrv_mixer_remove_input( path, AUDDRV_SPKR_IHF_STEREO );
                        auddrv_mixer_remove_input( path, AUDDRV_SPKR_HS );
auddrv_pwrOffUnusedSpkr( );
       ...
       )
{
	...
	switch(speaker)
	{
		...
		case AUDDRV_SPKR_HS:
			auddrv_selectSpkrDrvMode( speaker );
			auddrv_selectSpkrDrvMode( speaker_second );
...
//=============================================================================
//
// Function Name: AUDDRV_RegisterCB_getAudioMode
//
// Description:   Register callback for getting audio mode
//
//=============================================================================
...
{
	client_GetAudioApp = cb;
}


//=============================================================================
//
// Function Name: AUDDRV_RegisterCB_setAudioMode
//
// Description:   Register callback for setting audio mode
//
//=============================================================================
...
//=============================================================================
//
// Function Name: AUDDRV_RegisterCB_setMusicMode
//
// Description:   Register callback for setting music audio mode
//
//=============================================================================
...
//=============================================================================
//
// Function Name: auddrv_selectSpkrDrvMode
//
// Description:   select 100mW speaker driver, configure mode (diferential)
//
//=============================================================================
static void auddrv_selectSpkrDrvMode( AUDDRV_SPKR_Enum_t speaker )
{
	...
	temp32 = chal_audiospeaker_read_ANACR0 ( spkr_handle);

	Log_DebugPrintf(LOGID_AUDIO, "\n\r\t auddrv_selectSpkrDrvMode read anacr0 %x spkr %d \n\r", temp32, speaker);

	temp32 &= ~(SYSCFG_ANACR0_BGTCSP_MASK);  //clear the bits
	temp32 |= (0x3 << SYSCFG_ANACR0_BGTCSP_SHIFT);

	switch(speaker)
	{
		case AUDDRV_SPKR_IHF:
			temp32 &= ~(SYSCFG_ANACR0_MODE_SELL_MASK);  //clear the bits
			temp32 |= ( spkr_cfg[speaker].speaker_config << SYSCFG_ANACR0_MODE_SELL_SHIFT );

			temp32 |= (SYSCFG_ANACR0_DRVR_SELL_MASK);

            chal_audiospeaker_write_ANACR0 ( spkr_handle, temp32 );
			
			break;

		case AUDDRV_SPKR_EP:
			temp32 &= ~(SYSCFG_ANACR0_MODE_SELR_MASK);  //clear the bits
			temp32 |= ( spkr_cfg[speaker].speaker_config << SYSCFG_ANACR0_MODE_SELR_SHIFT );

			temp32 |= (SYSCFG_ANACR0_DRVR_SELR_MASK);
			chal_audiospeaker_write_ANACR0 ( spkr_handle, temp32 );
			break;
...
//=============================================================================
//
// Function Name: auddrv_deselectSpkrDrv
//
// Description:   de-select 100mW spekaer driver
//
//=============================================================================
static void auddrv_deselectSpkrDrv( AUDDRV_SPKR_Enum_t speaker )
{
	...
	{
		case AUDDRV_SPKR_IHF:
			temp32 &= ~(SYSCFG_ANACR0_DRVR_SELL_MASK);  //clear the bits
			chal_audiospeaker_write_ANACR0 ( spkr_handle, temp32 );
			break;

		case AUDDRV_SPKR_EP:
			temp32 &= ~(SYSCFG_ANACR0_DRVR_SELR_MASK);  //clear the bits
			chal_audiospeaker_write_ANACR0 ( spkr_handle, temp32 );
			break;
...
static void auddrv_pwrOffUnusedSpkr( void )
{
		...
		)
	{
        ...
        auddrv_deselectSpkrDrv( AUDDRV_SPKR_IHF_STEREO );
	}
	else
	{
			...
			)
		{
			...
			//chal_audiospeaker_PwrdnDacRef( spkr_handle, CHAL_AUDIO_SPKRA_L, TRUE);
            auddrv_deselectSpkrDrv( AUDDRV_SPKR_IHF );
		}
			...
			)
		{
			...
			//chal_audiospeaker_PwrdnDacRef( spkr_handle, CHAL_AUDIO_SPKRA_R, TRUE);
            	auddrv_deselectSpkrDrv( AUDDRV_SPKR_EP );
		}
	}
...
static void auddrv_powerOnSpkr(	void )
{
	...
	if ( auddrv_spkrIsUsed( AUDDRV_SPKR_IHF_STEREO ) )
	{
		...
		chal_audiospeaker_SetAnaPwrRamp(spkr_handle,  ANALOG_RAMP_UP );
        	Log_DebugPrintf(LOGID_AUDIO, "\n\r\t auddrv_powerOnSpkr IHF_STEREO \n\r");
		auddrv_selectSpkrDrvMode( AUDDRV_SPKR_IHF_STEREO );
		//OSTASK_Sleep(150);
	}
	else
	{
		if ( auddrv_spkrIsUsed( AUDDRV_SPKR_IHF ) )
		{
			...
			chal_audiospeaker_PwrdnDacDrv(		 spkr_handle, CHAL_AUDIO_SPKRA_L, FALSE);
			chal_audiospeaker_SetAnaPwrRamp(spkr_handle, ANALOG_RAMP_UP );
            	Log_DebugPrintf(LOGID_AUDIO, "\n\r\t auddrv_powerOnSpkr IHF \n\r");
			auddrv_selectSpkrDrvMode( AUDDRV_SPKR_IHF );
			//OSTASK_Sleep(150);
		}
		...
		if ( auddrv_spkrIsUsed( AUDDRV_SPKR_EP ) )
		{
			...
			chal_audiospeaker_PwrdnDacDrv(		 spkr_handle, CHAL_AUDIO_SPKRA_R, FALSE);
			chal_audiospeaker_SetAnaPwrRamp(spkr_handle, ANALOG_RAMP_UP );
            Log_DebugPrintf(LOGID_AUDIO, "\n\r\t auddrv_powerOnSpkr EP \n\r");
			auddrv_selectSpkrDrvMode( AUDDRV_SPKR_EP );
			//OSTASK_Sleep(150);
		}
	}
...
*/
    ...
    *(volatile UInt32 *)SYSCFG_BASE_ADDR &= ~(0x01000000);
	...
	if(on)
	{
        ...
        *(volatile UInt32 *)SYSCFG_BASE_ADDR |= 0x02000;   // IOCR0.GPEN8_MUX
		*(volatile UInt32 *)(SYSCFG_BASE_ADDR+0x0C) |= 0x0200000; // IOCR2.GPEN8_MUX_HI
				...
				)
{
	...
	switch(mic)
	{
	    ...
	    case AUDDRV_DUAL_MIC_DIGI1_ANALOG:
			//rule #1
			chal_audiomic_PowerOnADC( mic_handle, AUXMIC_BIAS_FPMODE );
...
static void auddrv_pwrOffUnusedMic( void )
{
	...
	//case AUDDRV_MIC_PCM_IF: 
	// Turn off PCM Interface if not in call
    if (AUDDRV_GetVCflag() == FALSE)
