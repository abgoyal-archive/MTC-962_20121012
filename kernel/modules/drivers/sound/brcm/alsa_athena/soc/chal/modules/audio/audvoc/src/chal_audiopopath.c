...
CHAL_HANDLE chal_audiopopath_Init(cUInt32 baseAddr)
{
    if (sPOPath.inited == 0) 
    {
		...
		sPOPath.inited = 1;
        BRCM_WRITE_REG_FIELD(sPOPath.base, DSP_AUDIO_PLRCH_R, PLCHMOD, 2); // L_out = L_in
		BRCM_WRITE_REG_FIELD(sPOPath.base, DSP_AUDIO_PLRCH_R, PRCHMOD, 2); // R_out = R_in
...
{
    UInt32 reg_addr;
	DSP_AUDIO_PLSLOPGAIN_R_TYPE reg_value = 0;

	if(mute_left==TRUE)
	{
		reg_addr = sPOPath.base + DSP_AUDIO_PLSLOPGAIN_R_OFFSET;
		reg_value = DSP_AUDIO_PLSLOPGAIN_R_PLSLOPGAINEN_MASK | (SLOPE_GAIN_MODE << DSP_AUDIO_PLSLOPGAIN_R_PLSLOPMOD_SHIFT);
	...
	if(mute_right==TRUE)
	{
	    reg_addr = sPOPath.base + DSP_AUDIO_PRSLOPGAIN_R_OFFSET;
		reg_value = DSP_AUDIO_PRSLOPGAIN_R_PRSLOPGAINEN_MASK | (SLOPE_GAIN_MODE << DSP_AUDIO_PRSLOPGAIN_R_PRSLOPMOD_SHIFT);
		*(volatile UInt16 *) reg_addr = reg_value;
	}
}
		...
		)
{
	...
	reg_addr = sPOPath.base + DSP_AUDIO_PLSLOPGAIN_R_OFFSET;
	reg_value = DSP_AUDIO_PLSLOPGAIN_R_PLSLOPGAINEN_MASK | ( SLOPE_GAIN_MODE << DSP_AUDIO_PLSLOPGAIN_R_PLSLOPMOD_SHIFT );
	reg_value = reg_value | ( gain << DSP_AUDIO_PLSLOPGAIN_R_PLTARGETGAIN_SHIFT );
	*(volatile UInt16 *) reg_addr = reg_value;
    ...
    reg_addr = sPOPath.base + DSP_AUDIO_PRSLOPGAIN_R_OFFSET;
	reg_value = DSP_AUDIO_PRSLOPGAIN_R_PRSLOPGAINEN_MASK | (SLOPE_GAIN_MODE << DSP_AUDIO_PRSLOPGAIN_R_PRSLOPMOD_SHIFT);
	reg_value = reg_value | ( gain << DSP_AUDIO_PRSLOPGAIN_R_PRTARGETGAIN_SHIFT );
	*(volatile UInt16 *) reg_addr = reg_value;
}
		...
		)
{
	...
	reg_addr = sPOPath.base + DSP_AUDIO_PLSLOPGAIN_R_OFFSET;
	reg_value = DSP_AUDIO_PLSLOPGAIN_R_PLSLOPGAINEN_MASK | ( SLOPE_GAIN_MODE << DSP_AUDIO_PLSLOPGAIN_R_PLSLOPMOD_SHIFT );
	reg_value = reg_value | ( gain_hex << DSP_AUDIO_PLSLOPGAIN_R_PLTARGETGAIN_SHIFT );
	*(volatile UInt16 *) reg_addr = reg_value;
		...
		)
{
	...
	reg_addr = sPOPath.base + DSP_AUDIO_PRSLOPGAIN_R_OFFSET;
	reg_value = DSP_AUDIO_PRSLOPGAIN_R_PRSLOPGAINEN_MASK | (SLOPE_GAIN_MODE << DSP_AUDIO_PRSLOPGAIN_R_PRSLOPMOD_SHIFT);
	reg_value = reg_value | ( gain_hex << DSP_AUDIO_PRSLOPGAIN_R_PRTARGETGAIN_SHIFT );
	*(volatile UInt16 *) reg_addr = reg_value;
}
