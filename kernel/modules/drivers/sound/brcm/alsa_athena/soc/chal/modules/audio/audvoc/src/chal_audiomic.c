				...
				)
{
	if (sMic.inited == 0) 
    {
		...
		sMic.inited = 1;
        //disable force power up
        BRCM_WRITE_REG_FIELD(sMic.syscfg_base, SYSCFG_ANACR3, ARX_FORCE_PWRUP_DISABLE, 1);
		...
		)
{
	...
	switch (bias_mode)
	{
		case AUXMIC_BIAS_OFF: //Power Down.
		//BRCM_WRITE_REG_FIELD(pDevice->base, SYSCFG_ANACR2, Pga_adc_standby_pwrup, 0);
			//BRCM_WRITE_REG_FIELD(pDevice->base, SYSCFG_ANACR2, I_MIC_VOICE_PWRDN, 1);
			//BRCM_WRITE_REG_FIELD(pDevice->base, SYSCFG_ANACR2, I_LDO_PWRDN, 1);
			reg_val |= (0x0003);
			break;

		case AUXMIC_BIAS_FPMODE: //Normal Operation (FP_MODE).
			//BRCM_WRITE_REG_FIELD(pDevice->base, SYSCFG_ANACR2, Pga_adc_standby_pwrup, 0);
		//BRCM_WRITE_REG_FIELD(pDevice->base, SYSCFG_ANACR2, I_MIC_VOICE_PWRDN, 0);
		//BRCM_WRITE_REG_FIELD(pDevice->base, SYSCFG_ANACR2, I_LDO_PWRDN, 0);
			reg_val |= (0x0004);
			break;

		case AUXMIC_BIAS_LPMODE: //LP_MODE for headset button detection.
		//BRCM_WRITE_REG_FIELD(pDevice->base, SYSCFG_ANACR2, Pga_adc_standby_pwrup, 0);
		//BRCM_WRITE_REG_FIELD(pDevice->base, SYSCFG_ANACR2, I_MIC_VOICE_PWRDN, 1);
			//BRCM_WRITE_REG_FIELD(pDevice->base, SYSCFG_ANACR2, I_LDO_PWRDN, 0);
			reg_val |= (0x0002);
