...
/* add for bug323677, official patch from brcm case563964 */
	#if defined(CONFIG_HAS_WAKELOCK)
...
//add for bug245797, only called once by hs_probe() in boot process
irqreturn_t hs_isr_temp_for_probe(int irq, void *dev_id)
{
    struct mic_t *p = (struct mic_t *)dev_id;
    unsigned long int val = readl(io_p2v(REG_ANACR12));
	int headset_state_gpio;
...
// for bug245797, add prerequisite, if MICINVAL/0x088800b0 bit[14:08] < MICINTH/0x08911040 bit[06:00], no headset plug in
	if(!val || (val < 0x2d00))
		return IRQ_NONE;
	...
	headset_state_gpio = gpio_get_value(HEADSET_DET_GPIO);
	
	if (p->headset_state == headset_state_gpio) {
		p->headset_state = !headset_state_gpio;
	}
        set_irq_type(mic.hsirq, (p->headset_state) ? IRQF_TRIGGER_FALLING : IRQF_TRIGGER_RISING);
	schedule_delayed_work(&(p->switch_data.work), 0);
    }
...
// modify hs_isr to hs_isr_temp_for_probe, for bug245797
	hs_isr_temp_for_probe(mic.hsirq,&mic);
