...
static int bmem_GetBiggestFreeBlock(void);
...
static struct bmem_logic bmem_fops = {
    ...
    .GetBiggestFreeBlock = bmem_GetBiggestFreeBlock,
};
...
static int bmem_GetBiggestFreeBlock(void)
{
    ...
    while(head != NULL) {
        if (!head->used)
        {
            biggest_chunk_avlbl = MAX(biggest_chunk_avlbl, head->size);
        }
        head = head->next;
    }
    return biggest_chunk_avlbl;
}
