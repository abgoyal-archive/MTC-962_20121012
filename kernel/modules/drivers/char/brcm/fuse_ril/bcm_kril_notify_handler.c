...
MSRadioActivityInd_t  gRadioAcTInfo[DUAL_SIM_SIZE];
...
#define MAX_SIMPLE_TLV_DATA_LENGTH  512
#define MAX_TLV_DATA_LENGTH         (MAX_SIMPLE_TLV_DATA_LENGTH + 4)
...
extern Boolean gIsFlightModeOnBoot[DUAL_SIM_SIZE];

extern Boolean gIsStkRefreshReset[DUAL_SIM_SIZE];
...
void DisplayStkTextString(SATKString_t pSTKstr)
{
    ...
    if (!pSTKstr.string)
    {
        KRIL_DEBUG(DBG_INFO,"pSTKstr.string is NULL!!!\n");
        return;
    }
    ...
    else if ((pSTKstr.len > 0) && 
        (codeType == UNICODE_80 || 
         codeType == UNICODE_81 ||
         codeType == UNICODE_82 ||
         codeType == UNICODE_UCS2
         ))
    {
        RawDataPrintfun(pSTKstr.string, pSTKstr.len, "string");
    }
}
...
UInt16 ParseItemData(UInt8 *simple_tlv_ptr, UInt8 numItems, UInt8 *pItemIdList, SATKString_t *pItemList)
{
    ...
    else if (NULL != pItemList)
    {
        ...
        {
            if (!pItemList[i].string)
            {
                KRIL_DEBUG(DBG_ERROR,"pItemList[%d].string is NULL. ERROR!!!.\n",i);
                return 0;
            }
            ...
            KRIL_DEBUG(DBG_INFO,"i:%d itemID:%d unicode_type:0x%02X len:%d total_byte:%d\n",
                i, pItemIdList[i], pItemList[i].unicode_type, pItemList[i].len, total_byte);
...
UInt16 ParseTlvData(char *tlv_data_string, UInt8 *simple_tlv_objects, UInt16 simple_tlv_length)
{
    ...
    if (simple_tlv_length > 255)
    {
        tlv_ptr[0] = 0x82;
        tlv_ptr[1] = (simple_tlv_length >> 8) & 0xFF;
        tlv_ptr[2] = simple_tlv_length & 0xFF;
        tlv_ptr += 3;
        tlv_length += 3;
    }
    else if (simple_tlv_length > 127)
    {
        tlv_ptr[0] = 0x81;
...
UInt16 ParseSATKSetupMenu(char* tlv_data_string, SetupMenu_t *pSetupMenu)
{
    ...
    DisplayStkTextString(pSetupMenu->title);
...
UInt16 ParseSATKSelectItem(char* tlv_data_string, SelectItem_t *pSelectItem)
{
    ...
    DisplayStkTextString(pSelectItem->title);
...
int ParseSATKLaunchBrowser(char* tlv_data_string, LaunchBrowserReq_t *pLaunchBrowserReq)
{
    ...
    // Fill Alpha identifier
    if (FALSE == pLaunchBrowserReq->default_alpha_id)
    {
...
void ProcessSATKSetupMenu(Kril_CAPI2Info_t *dataBuf)
{
    ...
    if (!pSetupMenu)
    {
        KRIL_DEBUG(DBG_ERROR,"pSetupMenu is NULL. Error!!\n");
        return;
    }
...
void ProcessSATKSelectItem(Kril_CAPI2Info_t *dataBuf)
{
    ...
    if (!pSelectItem)
    {
        KRIL_DEBUG(DBG_ERROR,"pSelectItem is NULL. Error!!\n");
        return;
    }
...
void ProcessSATKGetInput(Kril_CAPI2Info_t *dataBuf)
{
    ...
    if (!pGetInput)
    {
        KRIL_DEBUG(DBG_ERROR,"pGetInput is NULL. Error!!\n");
        return;
    }
...
void ProcessSATKGetInkey(Kril_CAPI2Info_t *dataBuf)
{
    ...
    if (!pGetInkey)
    {
        KRIL_DEBUG(DBG_ERROR,"pGetInkey is NULL. Error!!\n");
        return;
    }
...
void ProcessSATKDisplayText(Kril_CAPI2Info_t *dataBuf)
{
    ...
    if (!pDisplayText)
    {
        KRIL_DEBUG(DBG_ERROR,"pDisplayText is NULL. Error!!\n");
        return;
    }
...
void ProcessSATKSendMOSMS(Kril_CAPI2Info_t *dataBuf)
{
    ...
    if (!pSendMOSMS)
    {
        KRIL_DEBUG(DBG_ERROR,"pSendMOSMS is NULL. Error!!\n");
        return;
    }
...
void ProcessSATKSendSs(Kril_CAPI2Info_t *dataBuf)
{
    ...
    if (!pSendSs)
    {
        KRIL_DEBUG(DBG_ERROR,"pSendSs is NULL. Error!!\n");
        return;
    }
...
void ProcessSATKSendUssd(Kril_CAPI2Info_t *dataBuf)
{
    ...
    if (!pSendUssd)
    {
        KRIL_DEBUG(DBG_ERROR,"pSendUssd is NULL. Error!!\n");
        return;
    }
...
void ProcessSATKPlayTone(Kril_CAPI2Info_t *dataBuf)
{
    ...
    if (!pPlayTone)
    {
        KRIL_DEBUG(DBG_ERROR,"pPlayTone is NULL. Error!!\n");
        return;
    }
...
void ProcessSATKSendStkDtmf(Kril_CAPI2Info_t *dataBuf)
{
    ...
    if (!pSendStkDtmf)
    {
        KRIL_DEBUG(DBG_ERROR,"pSendStkDtmf is NULL. Error!!\n");
        return;
    }
...
void ProcessSATKSetupCall(Kril_CAPI2Info_t *dataBuf)
{
    ...
    if (!pSetupCall)
    {
        KRIL_DEBUG(DBG_ERROR,"pSetupCall is NULL. Error!!\n");
        return;
    }
...
void ProcessSATKIdleModeText(Kril_CAPI2Info_t *dataBuf)
{
    ...
    if (!pIdleModeText)
    {
        KRIL_DEBUG(DBG_ERROR,"pIdleModeText is NULL. Error!!\n");
        return;
    }
...
void ProcessSATKRefresh(Kril_CAPI2Info_t *dataBuf)
{
    ...
    if (!pRefresh)
    {
        KRIL_DEBUG(DBG_ERROR,"pRefresh is NULL. Error!!\n");
        return;
    }
    ...
    switch (pRefresh->refreshType)
    {
        ...
        case SMRT_RESET:
             ...
             data[1] = 0;
             gIsStkRefreshReset[dataBuf->SimId] = TRUE;
             KRIL_SendNotify(dataBuf->SimId, BRCM_RIL_UNSOL_STK_PROACTIVE_COMMAND, tlv_data_string, (tlv_length * 2 + 1));
...
void ProcessSATKLaunchBrowser(Kril_CAPI2Info_t *dataBuf)
{
    ...
    if (!pLaunchBrowserReq)
    {
        KRIL_DEBUG(DBG_ERROR,"pLaunchBrowserReq is NULL. Error!!\n");
        return;
    }
...
//******************************************************************************
//
// Function Name: ProcessSATKSendLanguageNotificationInd
//
// Description:   Process the SATK Send Language Notification Indication
//
// Notes:
//
//******************************************************************************
...
{
    Boolean bSendNotifyFlg = FALSE;
    MSRegInfo_t *pMSRegInfo = (MSRegInfo_t*)data->dataBuf;
    KRIL_DEBUG(DBG_INFO, "MSG_REG_GSM_IND::GetGSMRegStatus:%d regState:%d rat:%d gprs_supported:%d egprs_supported:%d\n", sCreg_state[data->SimId], pMSRegInfo->regState, pMSRegInfo->netInfo.rat, pMSRegInfo->netInfo.gprs_supported, pMSRegInfo->netInfo.egprs_supported);
    KRIL_DEBUG(DBG_INFO, "MSG_REG_GSM_IND::mcc:%d mnc:%d cause:%d\n", pMSRegInfo->mcc, pMSRegInfo->mnc, pMSRegInfo->cause);
    if (pMSRegInfo->regState != REG_STATE_NO_CHANGE)
    {
        ...
        else
        {
            ...
            else if(1 == KRIL_GetLocationUpdateStatus(data->SimId) &&
                    (gRegInfo[data->SimId].netInfo.gprs_supported != pMSRegInfo->netInfo.gprs_supported ||
                    gRegInfo[data->SimId].netInfo.egprs_supported != pMSRegInfo->netInfo.egprs_supported ||
                    gRegInfo[data->SimId].netInfo.dtm_supported != pMSRegInfo->netInfo.dtm_supported ||
                    gRegInfo[data->SimId].netInfo.hsdpa_supported != pMSRegInfo->netInfo.hsdpa_supported ||
                    gRegInfo[data->SimId].netInfo.hsupa_supported != pMSRegInfo->netInfo.hsupa_supported))
            {
                //we should notify URIL to change GPRS icon once cgreg supported info changed,even if cgreg state is not changed
                //Even if GPRS service is not supported, we also need to update <Act>, so notify here instead of ProcessGPRSStatus.
                bSendNotifyFlg = TRUE;
            }
        }

		//memcpy must happened before KRIL_SendNotify, otherwise URIL may get old value on KRIL_RegistationStateHandler.
...
{
    MSRegInfo_t *pMSRegInfo = (MSRegInfo_t*)data->dataBuf;
    KRIL_DEBUG(DBG_INFO, "MSG_REG_GPRS_IND::GetGPRSRegStatus:%d regState:%d rat:%d gprs_supported:%d egprs_supported:%d\n", sCgreg_state[data->SimId], pMSRegInfo->regState, pMSRegInfo->netInfo.rat, pMSRegInfo->netInfo.gprs_supported, pMSRegInfo->netInfo.egprs_supported);
    if (pMSRegInfo->regState != REG_STATE_NO_CHANGE)
    {
...
void ProcessUssdNotification(Kril_CAPI2Info_t* notify)
{
    ...
    else if (notify->msgType == MSG_MNSS_CLIENT_SS_SRV_REL)
    {
        ...
        switch(srvRel_rsp->type)
        {
           ...
           {
                rdata->type = 0; // "0"   USSD-Notify -- text in ((const char **)data)[1]
                if ((srvRel_rsp->operation == SS_OPERATION_CODE_UNSTRUCTURED_SS_NOTIFY) &&
                     (srvRel_rsp->type == SS_SRV_TYPE_USSD_SS_NOTIFY) &&
                     (srvRel_rsp->param.ussdInfo.length == 0))
                {
                    KRIL_DEBUG(DBG_ERROR, "Ignore MSG_MNSS_CLIENT_SS_SRV_REL!!\n");
                    break;
                }
                ProcessUssdData(srvRel_rsp->operation, &srvRel_rsp->param.ussdInfo, rdata);
                KRIL_SendNotify(notify->SimId, BRCM_RIL_UNSOL_ON_USSD, rdata, sizeof(KrilReceiveUSSDInfo_t));
...
void ProcessNotification(Kril_CAPI2Info_t *notify)
{
    ...
    switch((UInt32)notify->msgType)
    {
        ...
        {
            MSRadioActivityInd_t* pRadio = (MSRadioActivityInd_t *) notify->dataBuf;
            KRIL_DEBUG(DBG_INFO, "MSG_RADIO_ACTIVITY_IND::SimId:%d radio_direction:%d radio_status:%d\n", notify->SimId, pRadio->radio_direction, pRadio->radio_status, pRadio->cid, pRadio->max_dl_data_rate);
            memcpy(&gRadioAcTInfo[notify->SimId], pRadio, sizeof(MSRadioActivityInd_t));
            break;
        }
        ...
        {
            //Android doesn't support network initiated PDP activation, reject it here
            PDP_ActivateNWI_Ind_t *pind = (PDP_ActivateNWI_Ind_t *) notify->dataBuf;
            KRIL_DEBUG(DBG_ERROR, "MSG_PDP_ACTIVATION_NWI_IND::SIM %d,pdpAdd[%s],apn[%s]\n", notify->SimId, pind->pdpAddress, pind->apn);
            if(!KRIL_DevSpecific_Cmd(BCM_KRIL_CLIENT, notify->SimId, BRCM_RIL_REQUEST_PDP_ACTIVATION_NWI_IND_REJECT, pind, sizeof(PDP_ActivateNWI_Ind_t)))
            {
                KRIL_DEBUG(DBG_ERROR,"Command BRCM_RIL_REQUEST_PDP_ACTIVATION_NWI_IND_REJECT failed\n");
            }
            break;
        }
        ...
        case MSG_SIM_DETECTION_IND:
        {
            ...
            if (SIM_APPL_INVALID == pind->sim_appl_type)
            {
                ...
                radiostate = BCM_RADIO_STATE_SIM_LOCKED_OR_ABSENT;
            }
            else
            {
                radiostate = BCM_RADIO_STATE_SIM_READY;
            }
            ...
            if (!gIsFlightModeOnBoot[notify->SimId])
            {
                KRIL_SendNotify(notify->SimId, BRCM_RIL_UNSOL_RESPONSE_SIM_STATUS_CHANGED, &radiostate, sizeof(BRIL_RadioState));
            }
        ...
        case MSG_PBK_READY_IND:
            ...
            //}
            KRIL_DEBUG(DBG_INFO, "MSG_PBK_READY_IND::not process\n");
            break;
