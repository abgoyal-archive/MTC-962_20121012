...
// 
//----------------------------------------------------------------------------
UInt16 SIMLockGetLockCodeInform(SIMLockType_t lockType, UInt8* lockinfo, UInt16 len)
{
    SIMLOCK_NVDATA_t simlock_nvdata;
    SIMLock_CodeFile_t *simlockFile = GetSimlockData();
    UInt8 remain_attempt;
    ...
    if (NULL == lockinfo)
    {
        KRIL_DEBUG(DBG_ERROR,"lockinfo is NULL!!!\n");
        return 0;
    }
    ...
    if (GetSimlockNvdata(&simlock_nvdata) && (simlockFile != NULL))
    {
        switch (lockType)
        {
            case SIMLOCK_NETWORK_LOCK:
            {
                // Length = 1(network_lock_ind) + 1(remain_attempt) + 1(numNetworkLocks) + numNetworkLocks * MCC_MNC_CODE_SIZE
                datalen = 3 + simlockFile->numNetworkLocks * MCC_MNC_CODE_SIZE;
                if (datalen > len)
                {
                    KRIL_DEBUG(DBG_ERROR,"lockinfo buffer length:%d < datalen:%d Error!!!\n", len, datalen);
                    return 0;
                }
                
                // Fill lock indicator
                lockinfo[0] = (UInt8)simlock_nvdata.network_lock_ind;
                ...
                {
                    SIMLock_NetworkCode_t *network_code = (SIMLock_NetworkCode_t *)
                        (((UInt8 *) simlockFile) + sizeof(SIMLock_CodeFile_t));
                    
                    plockinfo = &lockinfo[3];
                    
                    for (i = 0; i < simlockFile->numNetworkLocks; i++, network_code++)
                    {
                        memcpy(plockinfo, network_code, MCC_MNC_CODE_SIZE);
            ...
            case SIMLOCK_NET_SUBSET_LOCK:
            {
                // Length = 1(network_subset_lock_ind) + 1(remain_attempt) + 1(numProviderLocks) + numNetSubsetLocks * MCC_MNC_CODE_SIZE
                datalen = 3 + simlockFile->numNetSubsetLocks * MCC_MNC_CODE_SIZE;
                if (datalen > len)
                {
                    KRIL_DEBUG(DBG_ERROR,"lockinfo buffer length:%d < datalen:%d Error!!!\n", len, datalen);
                    return 0;
                ...
                // Fill lock indicator
                lockinfo[0] = (UInt8)simlock_nvdata.network_subset_lock_ind;
                
                // Fill remain attempt
                unlock_attempt = simlockFile->resetUnlockCounter ? &Network_Unlock_Attempt : &simlock_nvdata.network_subset_unlock_attempt;
                if (NULL != unlock_attempt)
                {
                    remain_attempt = (*unlock_attempt < simlockFile->maxUnlockAttempt ? simlockFile->maxUnlockAttempt - (*unlock_attempt) : 0); 
                }
                ...
                if (simlockFile->numNetSubsetLocks > 0)
                {
                    SIMLock_NetSubSetCode_t *network_subset_code = (SIMLock_NetSubSetCode_t *) 
                        ( ((UInt8 *) simlockFile) + sizeof(SIMLock_CodeFile_t) + 
                        (sizeof(SIMLock_NetworkCode_t) * simlockFile->numNetworkLocks) + 
                        (sizeof(SIMLock_ProviderCode_t) * simlockFile->numProviderLocks) );
                    
                     plockinfo = &lockinfo[3];
                     
                    for (i = 0; i < simlockFile->numNetSubsetLocks; i++, network_subset_code++)
                    {
                        memcpy(plockinfo, network_subset_code, MCC_MNC_CODE_SIZE);
                        plockinfo += MCC_MNC_CODE_SIZE;
                    }
                }
            ...
            case SIMLOCK_PROVIDER_LOCK:
            {
                // Length = 1(service_provider_lock_ind) + 1(remain_attempt) + 1(numProviderLocks) + numProviderLocks * MCC_MNC_CODE_SIZE
                datalen = 3 + simlockFile->numProviderLocks * MCC_MNC_CODE_SIZE;
                if (datalen > len)
                {
                    KRIL_DEBUG(DBG_ERROR,"lockinfo buffer length:%d < datalen:%d Error!!!\n", len, datalen);
                    return 0;
                ...
                // Fill lock indicator
                lockinfo[0] = (UInt8)simlock_nvdata.service_provider_lock_ind;
                
                // Fill remain attempt
                unlock_attempt = simlockFile->resetUnlockCounter ? &Network_Unlock_Attempt : &simlock_nvdata.provider_unlock_attempt;
                if (NULL != unlock_attempt)
                {
                    remain_attempt = (*unlock_attempt < simlockFile->maxUnlockAttempt ? simlockFile->maxUnlockAttempt - (*unlock_attempt) : 0); 
                }
                ...
                if (simlockFile->numProviderLocks > 0)
                {
                    ...
                    for (i = 0; i < simlockFile->numProviderLocks; i++, provider_code++)
                    {
                        memcpy(plockinfo, provider_code, MCC_MNC_CODE_SIZE);
                        plockinfo += MCC_MNC_CODE_SIZE;
                    }
                }
            ...
            case SIMLOCK_CORP_LOCK:
            {
                // Length = 1(corporate_lock_ind) + 1(remain_attempt) + 1(numProviderLocks) + numCorpLocks * MCC_MNC_CODE_SIZE
                datalen = 3 + simlockFile->numCorpLocks * MCC_MNC_CODE_SIZE;
                if (datalen > len)
                {
                    KRIL_DEBUG(DBG_ERROR,"lockinfo buffer length:%d < datalen:%d Error!!!\n", len, datalen);
                    return 0;
                ...
                // Fill lock indicator
                lockinfo[0] = (UInt8)simlock_nvdata.corporate_lock_ind;
                
                // Fill remain attempt
                unlock_attempt = simlockFile->resetUnlockCounter ? &Network_Unlock_Attempt : &simlock_nvdata.corporate_unlock_attempt;
                if (NULL != unlock_attempt)
                {
                    remain_attempt = (*unlock_attempt < simlockFile->maxUnlockAttempt ? simlockFile->maxUnlockAttempt - (*unlock_attempt) : 0); 
                }
                ...
                if (simlockFile->numCorpLocks > 0)
                {
                    SIMLock_CorporateCode_t *corp_code = (SIMLock_CorporateCode_t *) 
                        ...
                        (sizeof(SIMLock_NetworkCode_t) * simlockFile->numNetworkLocks) + 
                        (sizeof(SIMLock_ProviderCode_t) * simlockFile->numProviderLocks) + 
                        (sizeof(SIMLock_NetSubSetCode_t) * simlockFile->numNetSubsetLocks) );
                            
                    plockinfo = &lockinfo[3];
                    
                    for (i = 0; i < simlockFile->numCorpLocks; i++, corp_code++)
                    {
                        memcpy(plockinfo, corp_code, MCC_MNC_CODE_SIZE);
                        plockinfo += MCC_MNC_CODE_SIZE;
                    }
                }
            ...
            case SIMLOCK_PHONE_LOCK:
            {
                // Length = 1(phone_lock_setting) + IMSI_DIGITS(IMSI)
                if (PH_SIM_LOCK_OFF != simlock_nvdata.phone_lock_setting)
                {
                ...
                // Fill lock indicator
                lockinfo[0] = (UInt8)simlock_nvdata.phone_lock_setting;
                
                // Fill the IMSI that phone is locked to                
                if (PH_SIM_LOCK_OFF != simlock_nvdata.phone_lock_setting)
                {
                    memcpy(&lockinfo[1], simlock_nvdata.phone_lock_imsi, IMSI_DIGITS);                
                }
            ...
            default:
                KRIL_DEBUG(DBG_ERROR,"Unsupport SIM lock type:%d Error!!!\n", lockType);
                break;                                            
        }
    }
