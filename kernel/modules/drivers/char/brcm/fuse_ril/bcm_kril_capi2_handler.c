...
void KRIL_Capi2HandleRespCbk(UInt32 tid, UInt8 clientID, MsgType_t msgType, SimNumber_t SimId, UInt32 DialogId, Result_t result, void *dataBuf, UInt32 dataLength, ResultDataBufHandle_t dataBufHandle)
{
    ...
    else if (MSG_MEASURE_REPORT_PARAM_IND == capi2_rsp->msgType && KRIL_GetQueryMeasureReportTID() != 0)  // Change the notify message to response message for the MSG_MEASURE_REPORT_PARAM_IND
    {
        capi2_rsp->tid = KRIL_GetQueryMeasureReportTID();        
        KRIL_SetQueryMeasureReportTID(0);
    }
