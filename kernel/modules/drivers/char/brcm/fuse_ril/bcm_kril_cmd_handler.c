...
static UInt32 sQueryMeasureReportTID = 0;
static Boolean sInQueryMeasureReportHandler = FALSE;
...
kril_capi2_handler_fn_t g_kril_capi2_handler_array[]=
{
     ...
     {BRCM_RIL_REQUEST_DIAL, KRIL_DialHandler, 0},
     ...
     {BRCM_RIL_REQUEST_OEM_HOOK_RAW, KRIL_OemHookRawHandler, sizeof(UInt32)},
     ...
     {BRCM_RIL_REQUEST_PDP_ACTIVATION_NWI_IND_REJECT, KRIL_RejectNWIPdpActivate, 0},
#ifdef VIDEO_TELEPHONY_ENABLE
     {BRCM_RIL_REQUEST_DIAL_VT, KRIL_DialHandler, 0},
...
Boolean IsNeedToWait(SimNumber_t SimId, unsigned long CmdID)
{
    ...
    if ( (BRCM_RIL_REQUEST_GET_NEIGHBORING_CELL_IDS == CmdID) && (KRIL_GetInNeighborCellHandler()) )
    {
    ...
    else if (BRCM_RIL_REQUEST_DIAL == CmdID && KRIL_GetInHoldCallHandler(SimId) == TRUE)
    {
    ...
    else if (BRCM_RIL_REQUEST_OEM_HOOK_RAW == CmdID && sInSetPrferredDataConnection == TRUE)
    {
        KRIL_DEBUG(DBG_INFO, "OEM Hook RAW handler MATCH::Cmd queue:0x%02lx\n", CmdID);        
        return TRUE;
    }
    else
    if ( (BRCM_RIL_REQUEST_OEM_HOOK_RAW == CmdID) && (KRIL_GetInQueryMeasureReportHandler()) )
    {
        KRIL_DEBUG(DBG_INFO, "MeasureReport MATCH::Cmd queue:0x%02lx\n", CmdID);
        return TRUE;
    }
            ...
            BRCM_RIL_REQUEST_CONFERENCE == CmdID) // Just only send one MPTY call request to CP to avoid call state error
    {
        ...
        list_for_each_safe(listptr, listpos, &gKrilCmdList.list)
        {
            ...
            if(BRCM_RIL_REQUEST_HANGUP_WAITING_OR_BACKGROUND == listentry->ril_cmd->CmdID ||
               ...
               BRCM_RIL_REQUEST_HANGUP == listentry->ril_cmd->CmdID)
            {
...
BRIL_LastCallFailCause KRIL_MNCauseToRilError(Cause_t inMNCause)
{
    ...
    switch (inMNCause)
    {
        ...
        default:
            failCause = inMNCause; // assign MN cause to framework directly
            break;
    }
...
//******************************************************************************
//
// Function Name: KRIL_SetInQueryMeasureReportHandler
//
// Description:   set the state of query engineer mode to be ture/false.
//
// Notes:
//******************************************************************************
void KRIL_SetInQueryMeasureReportHandler( Boolean inHandler )
{
    KRIL_DEBUG(DBG_ERROR,"inHandler:%s\n", (inHandler?"TRUE":"FALSE") );
    sInQueryMeasureReportHandler = inHandler;
}
...
// Function Name: KRIL_GetInSetPrferredNetworkTypeHandler
//
// Description:   Check whether in query engineer mode handler.
//
// Notes:
//******************************************************************************
Boolean KRIL_GetInQueryMeasureReportHandler(void)
{
    return sInQueryMeasureReportHandler;
}

//******************************************************************************
//
// Function Name: KRIL_SetQueryMeasureReportTID
//
// Description:   Store the TID for querying measure report.
//
// Notes:
//
//******************************************************************************
void KRIL_SetQueryMeasureReportTID(UInt32 tid)
{
    KRIL_DEBUG(DBG_ERROR,"KRIL_SetQueryMeasureReportTID:%ld\n", tid);
    sQueryMeasureReportTID = tid;
}

//******************************************************************************
//
// Function Name: KRIL_GetMeasureReportTID
//
// Description:   Get the TID for querying measure report.
//
// Notes:
//
//******************************************************************************
UInt32 KRIL_GetQueryMeasureReportTID(void)
{
    KRIL_DEBUG(DBG_ERROR,"KRIL_GetQueryMeasureReportTID:%ld\n", sQueryMeasureReportTID);
    return sQueryMeasureReportTID;
}
...
void KRIL_OemHookRawHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
        ...
        rawdata[3] == 'M')
    {
        ...
        switch (msgid)
        {
	     ...
	     //add by zhuyijun for the fast dormancy begin
	     case BRIL_HOOK_SET_FAST_DORMANCY:
		 KRIL_SetFastDormancy(ril_cmd, capi2_rsp);
		 break;
	     //add by zhuyijun for the fast dormancy end
	     //add by gewei for bug[178487] control connect when fdn active begin
           case BRIL_HOOK_SET_PDP_ACTIVATION_FDN_CONTROL:
               KRIL_SetPdpActivationFdnControlHandler(ril_cmd, capi2_rsp);
               break;

            case BRIL_HOOK_QUERY_NETWORK_LOCK:
                KRIL_QueryNetworkLockHandler(ril_cmd, capi2_rsp);
                break;
            
            case BRIL_HOOK_QUERY_NETWORK_SUBSET_LOCK:
                KRIL_QueryNetworkSubsetLockHandler(ril_cmd, capi2_rsp);
                break;

            case BRIL_HOOK_QUERY_SERVICE_PROVIDE_LOCK:
                KRIL_QueryServiceProvideLockHandler(ril_cmd, capi2_rsp);
                break;

            case BRIL_HOOK_QUERY_CORPORATE_LOCK:
                KRIL_QueryCorporateLockHandler(ril_cmd, capi2_rsp);
                break;

            case BRIL_HOOK_QUERY_PHONE_LOCK:
                KRIL_QueryPhoneLockHandler(ril_cmd, capi2_rsp);
                break;
            case BRIL_HOOK_QUERY_ENG_MODE:                
                KRIL_QueryMeasureReportHandler(ril_cmd, capi2_rsp);
                break;			   
            default:
