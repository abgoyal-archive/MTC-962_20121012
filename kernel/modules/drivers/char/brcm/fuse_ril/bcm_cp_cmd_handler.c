...
void CAPI2_FFS_Control(UInt32 tid, UInt8 clientID, UInt32 cmd, UInt32 address, UInt32 offset, UInt32 size)
{
    ...
    switch (cmd)
    {
        case FFS_CONTROL_COPY_NVS_FILE_TO_SM: // copy the NVS file to shared memory
        {
            ...
            if (IS_ERR(hFileTmp))
            {
                KRIL_DEBUG(DBG_ERROR,"Open cp_data.txt failed (maybe first boot)\n");
            ...
            if (hFileTmp->f_op && hFileTmp->f_op->read)
            {
				...
				retval = hFileTmp->f_op->read(hFileTmp, tmpBuf, size, &hFileTmp->f_pos);
                
                if (retval > 0)
                {
                    KRIL_DEBUG(DBG_ERROR,"COPY_NVS_FILE_TO_SM successfully\n");
					memcpy( pPersistDataAddr+offset, tmpBuf, size );
        ...
        {
            hFileTmp = filp_open("/data/cp_data.txt", O_WRONLY, 0);

            if (IS_ERR(hFileTmp))
            {
               // if the file does not exist, create it
               hFileTmp = filp_open("/data/cp_data.txt", O_CREAT|O_WRONLY, 0);
            }
            ...
            if (hFileTmp->f_op && hFileTmp->f_op->write)
            {
				...
				memcpy( tmpBuf, pPersistDataAddr+offset, size );
                // Write only partial data indicated by CP (for dual SIM case, MsDBNvData/cp_data.txt = 720 / 22528 = 3.2%)
                retval = hFileTmp->f_op->write(hFileTmp, tmpBuf, size, &hFileTmp->f_pos);
				kfree( tmpBuf );
				tmpBuf = NULL;
                ...
                if (retval != size || retval <= 0)
                {
                    KRIL_DEBUG(DBG_ERROR,"COPY_SM_TO_NVS_FILE failed!! retval:%d dataPtr.DataLength:%d ptr:0x%lx\n",
                               retval, size, (UInt32)(pPersistDataAddr+offset));
                    ffsctrlResult = FFS_CONTROL_IPCAP_PDS_OP_FAIL;
                }
                else
                {
                    KRIL_DEBUG(DBG_ERROR,"COPY_SM_TO_NVS_FILE successfully!!\n");
                    ffsctrlResult = FFS_CONTROL_PASS;
                }
            }
            else
