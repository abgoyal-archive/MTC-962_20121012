...
typedef enum
{
	...
	MS_LOCAL_SATK_ELEM_CALL_CONTROL_DEFAULT_CLIR,	///<Element type {::Boolean}, Whether Default CLIR is used if CC By SIM modifies the number

	//Phonebook Elements
	MS_LOCAL_PBK_ELEM_BDN_CACHE_CTR,                ///<Element type {::Boolean}, Whether BRCM PBK module supports BDN caching

	//Pch Elements
	MS_LOCAL_PCH_ELEM_ALLOW_NWI_PDP_ACTIVATION,		///<Element type {::UInt8}, TRUE if the Network Initiated (NWI) PDP activation is allowed. 
	MS_LOCAL_PCH_ELEM_SECONDARY_PDP_CONTEXT, 		///<Element type {::UInt8}, TRUE if the secondary PDP context support is enabled. 
	MS_LOCAL_PCH_ELEM_PDP_ACTIVATION_FDN_CONTROL, 	///<Element type {::UInt8}, TRUE if the PDP activation is controlled by FDN.
	...
	MS_NETWORK_ELEM_UAS_CONN_INFO,					///<Element type {::Uas_Conn_Info}, reflects the values sent in MNMM_URRC_INFO_IND
	...
	MS_CFG_ELEM_VCC_OPT_SIM_PIN_CHECK_ENABLED,		///<Element type {::UInt8}, TRUE if we can check SIM pin status for VM optimzation in VCC, 
													/// FALSE implies even if SimApi_Ispinok() returns false, we will not consider
													///this factor while doing vcc power saving optimization
