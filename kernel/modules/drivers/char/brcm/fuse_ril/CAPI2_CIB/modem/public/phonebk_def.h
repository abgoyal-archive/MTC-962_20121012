...
typedef enum
{
	PBK_3G_NAME = 0x01,		// Name data in EF-ADN
	PBK_3G_NUMBER = 0x02,	// Number data in EF-ADN
	PBK_3G_ANR = 0x03,		// First additional number
	PBK_3G_EMAIL = 0x04,	// Firest email address
	PBK_3G_SNE = 0x05,		// Second name data
	PBK_3G_GRP = 0x06,		// Grouping name
	PBK_3G_PBC = 0x07,		// Phonebook control file
	PBK_3G_ANR_A = 0x08,	// Second additional number
	PBK_3G_ANR_B = 0x09,	// Third additional number
	PBK_3G_ANR_C = 0x0A,	// Fourth additional number
	PBK_3G_EMAIL_A = 0x0B,	// Second email address
	PBK_3G_EMAIL_B = 0x0C,	// Second email address
	PBK_3G_EMAIL_C = 0x0D	// Second email address
} USIM_PBK_ID_t;

#define NUM_OF_USIM_PBK_ID ((UInt8) PBK_3G_EMAIL_C)

typedef struct
{
	USIM_PBK_ID_t pbkId;		// 3G PBK file ID
	UInt8 numOfTotalEntry;	// Total number of entries provisioned in SIM (0 means file not exist)
	UInt8 entryLength;		// Length of each entry provisioned in SIM
	UInt8 numOfUsedEntry;	// Number of entries which are not empty (not 0xFF)
} USIM_PBK_CONFIG_t;

typedef struct
{
	USIM_PBK_CONFIG_t configInfo[MAX_NUM_OF_ADN_SET][NUM_OF_USIM_PBK_ID];   // Number of valid "configInfo" elements is defined in "num_of_adn_set" element in 
																			// USIM_PBK_ADN_SET_t structure
} USIM_PBK_CONFIG_LIST_t;
...
{	
	...
	USIM_PBK_CONFIG_LIST_t config_info;	///<Detailed configuration info for each USIM PBK file
...
typedef struct
{
	...
	UInt16 next_valid_index;			///< Next non-empty record index (0-based) after phonebook reading block
