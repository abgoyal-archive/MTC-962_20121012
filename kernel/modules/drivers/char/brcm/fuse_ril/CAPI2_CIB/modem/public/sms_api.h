...
/**
	...
	Note2: In the case the EF-CBMI does not exist, this API will still be executed but the return
		   status will be SMS_CB_MIDS_DOES_NOT_EXIST instead RESULT_OK.
	...
	Possible return values are ::RESULT_OK, ::SMS_NO_SERVICE, ::SMS_SIM_BUSY, ::SMS_CB_MIDS_DOES_NOT_EXIST.
...
Result_t SmsApi_StartCellBroadcastWithChnlReq(ClientInfo_t* inClientInfoPtr, MS_MN_CB_MSG_IDS* cbmi, MS_MN_CB_LANGUAGES* lang);


//***************************************************************************************
/**
    Function to Start/Stop Cell Broadcast with CMAS Channel list
	@param		*inClientInfoPtr (in) ClientInfo_t pointer
	@param		*cbmi (in) Cell Broadcast Message Channel list to be used
	@param		*lang (in) Cell Broadcast Message Language list to be used (not used)
	@return		Result_t 
	@note
**
		- This API is only for CMAS feature.  The CMAS MsgID list passed in will be combined with 
			normal CB MsgID List before sent to Stack layer to start the CB service.
		- The data for channel list (or cbmi) should be passed to this API in the 
			T_MN_CB_MSG_IDS structure format.
		- Memory allocated in *cbmi and *lang need to be released by calling party.
		- The cbmi passed in will be for one-time use, will not be used to
			alter the cbmi stored in SIM, nor the structure stored in RAM
		- This feature is for US market only and only English is supported.
		- This API will only be used by MMI/glue client, not ATC client.
		- The response will be sent in MSG_SMS_CB_START_STOP_RSP message.

	Usage:
		- to enable CMAS:
			- enable CP side by define the CMAS_SUPPORT from the platform file: platform.COMMS_THUNDERBIRD_DUALSIM_HSPA_EDN51.
			- add all user channels first with SmsApi_AddCellBroadcastChnlReq()
			- call SmsApi_StartCellBroadcastForCMASReq() to start the CB service
		- to remove CMAS:
			- add all user channels first with SmsApi_AddCellBroadcastChnlReq()
			- SmsApi_StartReceivingCellBroadcastReq()
		- to start CB without CMAS:
			- same as "to remove CMAS"

**/
Result_t SmsApi_StartCellBroadcastForCMASReq(ClientInfo_t* inClientInfoPtr, MS_MN_CB_MSG_IDS* cbmi, MS_MN_CB_LANGUAGES* lang);
...
/**
	...
	@return		Result_t 
		Possible return values are ::SMS_SIM_BUSY, ::RESULT_OK, ::SMS_CB_MIDS_DOES_NOT_EXIST.
	@note
...
/**
	...
	@return		Result_t 
	@note
	Possible return values are ::SMS_NO_SERVICE, ::SMS_OPERATION_NOT_ALLOWED, SMS_CB_MIDS_DOES_NOT_EXIST
	::RESULT_OK. 
