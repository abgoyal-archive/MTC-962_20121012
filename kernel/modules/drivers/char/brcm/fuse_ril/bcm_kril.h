...
*
* This file is a exact copy of brcm/public/kernel/bcm_kril.h
...
typedef struct
{
    ...
    char   ReNewPasswd[MAX_PW_LENGTH+1];
...
typedef enum {
    ...
    BRIL_HOOK_SET_FAST_DORMANCY = 7,
    //add by zhuyijun for the fast dormancy end
    // bug[178487] add by gewei for control connect when fdn active,begin
    BRIL_HOOK_SET_PDP_ACTIVATION_FDN_CONTROL=8, // config ms database element MS_LOCAL_PCH_ELEM_PDP_ACTIVATION_FDN_CONTROL
    // bug[178487] add by gewei for control connect when fdn active,begin
...
typedef struct
{
    ...
    //Note: stack side/capi2 pass in ip string for Ipv4 only case in 
    //pdpAddress. But, binary for IPV4V6 case. To be consistent
    //KRIL will convert ip address from binary to ip string for IPV4 address in
    //IPV4V6 case
    char pdpAddressIPV4[PDP_ADDRESS_LEN_MAX]; 
    char pdpAddressIPV6[PDP_ADDRESS_LEN_MAX];
    char apn[PDP_APN_LEN_MAX];
    char pdpType[PDP_TYPE_LEN_MAX];       /* X.25, IP, IPV6, etc. */
    unsigned char priDNS[MAX_DNS_ADDR_LEN];
    unsigned char secDNS[MAX_DNS_ADDR_LEN];
    unsigned char priDNSIPV6[MAX_DNS_ADDR_LEN];
    unsigned char secDNSIPV6[MAX_DNS_ADDR_LEN];
    int cause;
} KrilPdpData_t;
...
#define BRCM_RIL_REQUEST_PDP_ACTIVATION_NWI_IND_REJECT 104
