...
// For STK
UInt8 terminal_profile_data[30] = {0xFF,0xDF,0xFF,0xFF,0x7F,0x92,0x00,0xDF,0xDF,0x00,0x00,
...
void KRIL_InitCmdHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    switch(pdata->handler_state)
    {
        ...
        case BCM_SATK_ELEM_ENABLE_TEXT_CONVERSIONS:
        {
            ...
            //data.data_u.bData = FALSE;//comment by Liuqiang for brcm's patch
            data.data_u.bData = TRUE;//add by Liuqiang for brcm's patch
            CAPI2_MsDbApi_SetElement(InitClientInfo(pdata->ril_cmd->SimId), &data);
...
//bug[178487] for control connect when fdn active from brcm merge by gewei begin 
...
void MeasureReportPage1(Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    strcpy(tempbuf, "BCCH Information:Which One Parmeter!\n");
...
void MeasureReportPage2(Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    strcat(QueryMeasureReportBuf+5,tempbuf);
    strcpy(tempbuf,"Frequency Hopping:Which One Parmeter!\n");
    strcat(QueryMeasureReportBuf+5,tempbuf);
}
...
void MeasureReportPage6(Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    strcpy(tempbuf,"Timers (RAU): Parameter Not Sure\n");
    strcat(QueryMeasureReportBuf+5,tempbuf);
...
void MeasureReportPage8(Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    strcpy(QueryMeasureReportBuf+5, "Cell Selection: Not Support\n");
...
void MeasureReportPage11(Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    strcat(QueryMeasureReportBuf+5,tempbuf);
    strcpy(tempbuf,"UTRA Carrier RSSI:Not Support\n");
    strcat(QueryMeasureReportBuf+5,tempbuf);
}
...
void MeasureReportPage14(Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    if(rsp->umts_param.no_gsm_ncells != 0) {
        ...
        strcat(QueryMeasureReportBuf+5,tempbuf);    
        strcpy(tempbuf,"Compressed Mode Active or Not :Not Support!\n");
        strcat(QueryMeasureReportBuf+5,tempbuf);
...
void MeasureReportPage16(Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    strcat(QueryMeasureReportBuf+5,tempbuf);
    strcpy(tempbuf,"AICH Status:Not Support!\n");
    strcat(QueryMeasureReportBuf+5,tempbuf);
...
void MeasureReportPage21(Kril_CAPI2Info_t *capi2_rsp)
{
    MS_RxTestParam_t *rsp = (MS_RxTestParam_t *)capi2_rsp->dataBuf;
    char tempbuf[100];
    sprintf(tempbuf,"Type of AMR Codec:%d\n",rsp->umts_param.amr_info.umts_amr_codec_mode);
    strcat(QueryMeasureReportBuf+5,tempbuf);
    strcpy(tempbuf,"Upgrade and Downgrade of the AMR Codec:Not Support!\n");    
    strcat(QueryMeasureReportBuf+5, tempbuf);
