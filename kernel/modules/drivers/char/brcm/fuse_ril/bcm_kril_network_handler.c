...
//add by zhuyijun for FD begin
extern void CAPI2_MS_ForcePsReleaseReq(UInt32 tid, UInt8 clientID);
//add by zhuyijun for FD end
        ...
        case BCM_MS_GetRegistrationInfo:
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    //Add by chenguoan for pr[254104] begin 
                    rdata->numeric[0]=((presult->mcc)>>8)&0x00ff;
                    rdata->numeric[1]=(presult->mcc)&0x00ff;
                    rdata->numeric[2]=(presult->mnc)&0x00ff;
                    //Add by chenguoan for pr[254104] end
...
//add by zhuyijun for fast dormancy beign
void KRIL_SetFastDormancy(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
	KRIL_DEBUG(DBG_INFO, "fast bcm_kril_network_handler->KRIL_SetFastDormancy\n");
       KRIL_CmdList_t *pdata = (KRIL_CmdList_t *)ril_cmd;
       ClientInfo_t* inClientInfoPtr = 
	InitClientInfo(pdata->ril_cmd->SimId);
     ...
     switch(pdata->handler_state)
     {
         case BCM_SendCAPI2Cmd:
         {
                   KRIL_DEBUG(DBG_INFO, "fast bcm_kril_network_handler->KRIL_SetFastDormancy 1\n");
                 CAPI2_MS_ForcePsReleaseReq(inClientInfoPtr->reserved, inClientInfoPtr->clientId);
                 pdata->handler_state = BCM_RESPCAPI2Cmd;
         }
         break;
         ...
         case BCM_RESPCAPI2Cmd:
         {
             ...
             else
             {
                 ...
                 resp[2] = (UInt8)'C';
                 resp[3] = (UInt8)'M';
                 resp[4] = (UInt8)BRIL_HOOK_SET_FAST_DORMANCY;
                 pdata->handler_state = BCM_FinishCAPI2Cmd;
             }
         }
         break;
...
//add by zhuyijun for fast dormancy end
