...
#define CMAS_ENABLE 1
#define CMAS_DISABLE 0

static UInt8 CMAS_FEATURE_FLAG = CMAS_DISABLE;
static UInt8 PASSED_STR[CHNL_IDS_SIZE];
...
void KRIL_SendSMSHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    if (capi2_rsp != NULL)
    {
        ...
        {
            KRIL_SetInSendSMSHandler(pdata->ril_cmd->SimId, FALSE);
            if (KRIL_GetSendSMSNumber(pdata->ril_cmd->SimId) > 0) // if the SMS have more bodies, need to trigger the cmd queue event to process next SMS body
            {
                KRIL_DecrementSendSMSNumber(pdata->ril_cmd->SimId);
                KRIL_CmdQueueWork();
            }
    ...
    switch(pdata->handler_state)
    {
        case BCM_SendCAPI2Cmd:
        {
            ...
            KRIL_SetInSendSMSHandler(pdata->ril_cmd->SimId, TRUE);
        ...
        case BCM_RESPCAPI2Cmd:
        {
            ...
            KRIL_SetInSendSMSHandler(pdata->ril_cmd->SimId, FALSE);
            if (KRIL_GetSendSMSNumber(pdata->ril_cmd->SimId) > 0) // if more SMS in queue, need to trigger the cmd queue event to process next SMS body
            {
                KRIL_DecrementSendSMSNumber(pdata->ril_cmd->SimId);
                KRIL_CmdQueueWork();
            }
...
void KRIL_SetBroadcastSmsHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    switch (pdata->handler_state)
    {
        case BCM_SendCAPI2Cmd:
        {
            ...
            UInt8  mode, Msg_Lang_mode;
            UInt8  *MsgId_Ptr=NULL, *LangId_Ptr=NULL;
            int    MsgId_Number, LangId_Number, i;            
            mode = (tdata->selected & 0x1);
            Msg_Lang_mode = ((tdata->selected >>1) & 0x1);

            KRIL_DEBUG(DBG_INFO, "mode:%d, Msg_Lang_mode:%d\n", mode, Msg_Lang_mode);
            if(Msg_Lang_mode == 1) {
                if(mode == 1) {
                    KRIL_DEBUG(DBG_INFO, "Add MsgID\n");
                }
                else
                if(mode == 0) {
                    KRIL_DEBUG(DBG_INFO, "Delete MsgID\n");
                }
            }
            else
            if(Msg_Lang_mode == 0) {
                if(mode == 1) {
                    KRIL_DEBUG(DBG_INFO, "Add LangID\n");
                }
                else
                if(mode == 0) {
                    KRIL_DEBUG(DBG_INFO, "Delete LangID\n");
                }
            }
            //Handle MsgIDs
            if(Msg_Lang_mode == 1) {

                KRIL_DEBUG(DBG_INFO, "handler CB MsgIDs!\n");
                //CMAS Feature
                if((tdata->fromServiceId == 4352) && (tdata->toServiceId == 6399)) {
                    if(mode == 1) {
                        KRIL_DEBUG(DBG_INFO, "Start CMAS Feature!\n");
                        CMAS_FEATURE_FLAG = CMAS_ENABLE;
                        cbmi.msg_id_range_list.A[0].start_pos = tdata->fromServiceId;
                        cbmi.msg_id_range_list.A[0].stop_pos = tdata->toServiceId;
                        cbmi.nbr_of_msg_id_ranges = 1;
		                CAPI2_SmsApi_StartCellBroadcastForCMASReq(InitClientInfo(pdata->ril_cmd->SimId), &cbmi, NULL);	                    
                    }
                    else
                    if(mode ==0) {
                        KRIL_DEBUG(DBG_INFO, "Stop CMAS Feature!\n");
                        CMAS_FEATURE_FLAG = CMAS_DISABLE;
                        cbmi.msg_id_range_list.A[0].start_pos = 0;
                        cbmi.msg_id_range_list.A[0].stop_pos = 0;
                        cbmi.nbr_of_msg_id_ranges = 0;
                        CAPI2_SmsApi_StartReceivingCellBroadcastReq(InitClientInfo(pdata->ril_cmd->SimId));                        
                    }
                    pdata->handler_state = BCM_RESPCAPI2Cmd;
                }//CMAS Feature
                else//Normal CB
                {    
                    MsgId_Number = (tdata->toServiceId - tdata->fromServiceId + 1);
                    if(mode == 1) {
                        KRIL_DEBUG(DBG_INFO, "Add CB MsgId \n");
                        MsgId_Ptr = kmalloc(sizeof(UInt8)*(MsgId_Number*2), GFP_KERNEL);

                        if(MsgId_Ptr != NULL) {
                            for(i=0;i<MsgId_Number;i++) {
                                MsgId_Ptr[i] = (0xff & tdata->fromServiceId);
                                MsgId_Ptr[i+1] = (0xff & tdata->fromServiceId>>8);
                                KRIL_DEBUG(DBG_INFO, "MsgId_Ptr[%d]=%x, MsgId_Ptr[%d]=%x\n", i,MsgId_Ptr[i],i+1,MsgId_Ptr[i+1]);
                            }
                            CAPI2_SmsApi_AddCellBroadcastChnlReq(InitClientInfo(pdata->ril_cmd->SimId), MsgId_Ptr, MsgId_Number*2);
                            if (CMAS_FEATURE_FLAG == CMAS_ENABLE){
                                pdata->handler_state = BCM_SMS_StartCBCMASService;
                            }
                            else
                            if(CMAS_FEATURE_FLAG == CMAS_DISABLE){
                                pdata->handler_state = BCM_SMS_StartCBService;
                            }
                            kfree(MsgId_Ptr);
                        }
                    }//if(mode == 1)
                    ...
                    if(mode == 0) {
                        KRIL_DEBUG(DBG_INFO, "Delete CB MsgId \n");
                        if(MsgId_Number > 1) {
                            KRIL_DEBUG(DBG_ERROR, "The number of MsgID is over than 1.\n");
                            pdata->handler_state = BCM_ErrorCAPI2Cmd;
                            return;
                        }
                        sprintf(PASSED_STR, "%u", tdata->fromServiceId);
                        KRIL_DEBUG(DBG_INFO, "Delete CB MsgID %s\n", PASSED_STR);
                        CAPI2_SmsApi_RemoveCellBroadcastChnlReq(InitClientInfo(pdata->ril_cmd->SimId), PASSED_STR);
                        if (CMAS_FEATURE_FLAG == CMAS_ENABLE){
                            pdata->handler_state = BCM_SMS_StartCBCMASService;
                        }
                        else
            ...
            else//Handles LangIDs
            if(Msg_Lang_mode == 0) {
                LangId_Number = (tdata->toCodeScheme - tdata->fromCodeScheme + 1);
                KRIL_DEBUG(DBG_INFO, "handler CB Lang IDs!\n");

                if(LangId_Number > 1) {
                    KRIL_DEBUG(DBG_ERROR, "The number of LangID is over than 1.\n");
                    pdata->handler_state = BCM_ErrorCAPI2Cmd;
                    return;
                }

                if(mode == 1) {
                    KRIL_DEBUG(DBG_INFO, "Add CB LangId %d\n", tdata->fromCodeScheme);
                    CAPI2_SmsApi_AddCellBroadcastLangReq(InitClientInfo(pdata->ril_cmd->SimId), tdata->fromCodeScheme);
                    if (CMAS_FEATURE_FLAG == CMAS_ENABLE){
                        pdata->handler_state = BCM_SMS_StartCBCMASService;
                    }
                    else
                ...
                if(mode == 0) {
                    sprintf(PASSED_STR, "%u", tdata->fromCodeScheme);
                    KRIL_DEBUG(DBG_INFO, "Delete CB LangId %s\n",PASSED_STR);
                    CAPI2_SmsApi_RemoveCellBroadcastLangReq(InitClientInfo(pdata->ril_cmd->SimId), PASSED_STR);
                    if (CMAS_FEATURE_FLAG == CMAS_ENABLE){
                        pdata->handler_state = BCM_SMS_StartCBCMASService;
                    }
                    else
        ...
        case BCM_SMS_StartCBService:
        {
            if(capi2_rsp->result != RESULT_OK )
            {
                KRIL_DEBUG(DBG_ERROR, "Add or Delete MsgID or LangID Fail, error result:%d \n",capi2_rsp->result);
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
                return;
            }
            KRIL_DEBUG(DBG_INFO, "Add or Delete MsgID or LangID OK \n");
            CAPI2_SmsApi_StartReceivingCellBroadcastReq(InitClientInfo(pdata->ril_cmd->SimId));
            pdata->handler_state = BCM_RESPCAPI2Cmd;
            break;
        }

        case BCM_SMS_StartCBCMASService:
        {
            if(capi2_rsp->result != RESULT_OK )
            {
                KRIL_DEBUG(DBG_ERROR, "Add or Delete MsgID or LangID Fail, error result:%d \n",capi2_rsp->result);
        ...
        case BCM_RESPCAPI2Cmd:
        {
            ...
            {
                if (CMAS_FEATURE_FLAG == CMAS_ENABLE){
                    KRIL_DEBUG(DBG_ERROR, "Start CB CMAS Service Fail, error result:%d \n",capi2_rsp->result);
                }
                else
                if(CMAS_FEATURE_FLAG == CMAS_DISABLE){
                    KRIL_DEBUG(DBG_ERROR, "Start CB Normal Service Fail, error result:%d \n",capi2_rsp->result);
                }
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
                return;
            }
            ...
            if (CMAS_FEATURE_FLAG == CMAS_ENABLE){
                KRIL_DEBUG(DBG_INFO, "Start CB CMAS Service OK \n");
            }
            else
            if(CMAS_FEATURE_FLAG == CMAS_DISABLE){
                KRIL_DEBUG(DBG_INFO, "Start CB Normal Service OK \n");
            }
...
void KRIL_SmsBroadcastActivationHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    switch (pdata->handler_state)
    {
        case BCM_SendCAPI2Cmd:
        {
            ...
            {                
                if(CMAS_FEATURE_FLAG == CMAS_ENABLE){
                    KRIL_DEBUG(DBG_INFO, "Start CB CMAS Service\n");                    
                    cbmi.msg_id_range_list.A[0].start_pos = 4352;
                    cbmi.msg_id_range_list.A[0].stop_pos = 6399;
                    cbmi.nbr_of_msg_id_ranges = 1;
                    CAPI2_SmsApi_StartCellBroadcastForCMASReq(InitClientInfo(pdata->ril_cmd->SimId), &cbmi, NULL);                      
                }
                else
                if(CMAS_FEATURE_FLAG == CMAS_DISABLE){
                    KRIL_DEBUG(DBG_INFO, "Start CB Normal Service\n");
                    CAPI2_SmsApi_StartReceivingCellBroadcastReq(InitClientInfo(pdata->ril_cmd->SimId));                
                }
            }
            else //turn-off
            {
                KRIL_DEBUG(DBG_INFO, "Stop CB CMAS/Normal Service\n");
        ...
        case BCM_RESPCAPI2Cmd:
        {
            if(capi2_rsp->result != RESULT_OK )
            {
                KRIL_DEBUG(DBG_ERROR, "Start or Stop CB Service Fail \n");
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
                return;
            }
            KRIL_DEBUG(DBG_INFO, "Start or Stop CB Service OK \n");
            pdata->handler_state = BCM_FinishCAPI2Cmd;
            break;
        }
