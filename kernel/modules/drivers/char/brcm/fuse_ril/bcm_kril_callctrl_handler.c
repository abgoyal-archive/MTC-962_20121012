...
#define DTMF_TONE_DURATION  3000
    ...
    switch(pdata->handler_state)
    {
        ...
        case BCM_CC_GetCallType:
        {
            ...
            else
            {
                ...
                CAPI2_CcApi_IsCurrentStateMpty(InitClientInfo(pdata->ril_cmd->SimId), rdata->KRILCallState[rdata->index].index);
        ...
        case BCM_CC_GetCNAPName:
        {
            ...
            if (rsp->nameLength != 0)
            {
                ...
                rdata->KRILCallState[rdata->index].codetype = rsp->nameCoding;
...
void KRIL_ExplicitCallTransferHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    switch(pdata->handler_state)
    {
        ...
        case BCM_CC_GetNextActiveCallIndex:
        {
            ...
            if (rsp->listSz != 0)
            {
                KRIL_DEBUG(DBG_INFO, "Active call index:%d\n", rsp->indexList[0]);
                if (1 == rsp->listSz)
                {
                    CAPI2_CcApi_TransferCall(InitClientInfo(pdata->ril_cmd->SimId), rsp->indexList[0]);
                }
                else
                {
                    if (INVALID_CALL != sLastJoinCallIndex)
            {
                        KRIL_DEBUG(DBG_INFO, "using call index cached from last join req:%d\n", sLastJoinCallIndex);
                        CAPI2_CcApi_TransferCall(InitClientInfo(pdata->ril_cmd->SimId), sLastJoinCallIndex);
            }
            else
            {
                        CAPI2_CcApi_TransferCall(InitClientInfo(pdata->ril_cmd->SimId), rsp->indexList[0]);
        ...
        case BCM_CC_GetNextHeldCallIndex:
        {
            ...
            if (rsp->listSz != 0)
            {
                KRIL_DEBUG(DBG_INFO, "Held call index:%d\n", rsp->indexList[0]);
                if (1 == rsp->listSz)
                {
                    CAPI2_CcApi_TransferCall(InitClientInfo(pdata->ril_cmd->SimId), rsp->indexList[0]);
                }
            ...
            {
                KRIL_DEBUG(DBG_ERROR, "No Active and Held call index:%d\n");
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
            }
            break;
        }
        ...
        case BCM_RESPCAPI2Cmd:
        {
            if(capi2_rsp->result == RESULT_OK || capi2_rsp->result == CC_TRANS_CALL_SUCCESS)
            {
                VoiceCallActionMsg_t *rsp = (VoiceCallActionMsg_t *)capi2_rsp->dataBuf;
                KRIL_DEBUG(DBG_INFO, "BCM_RESPCAPI2Cmd::rsp->callResult:%d\n", rsp->callResult);
