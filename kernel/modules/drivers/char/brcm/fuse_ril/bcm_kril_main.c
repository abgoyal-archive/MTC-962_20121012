...
static int KRIL_Release(struct inode *inode, struct file *filp)
{
    ...
    extern Boolean gIsSilencePoweron[DUAL_SIM_SIZE];
    gIsSilencePoweron[SIM_DUAL_FIRST] = TRUE;
    gIsSilencePoweron[SIM_DUAL_SECOND] = TRUE;
#endif
...
static unsigned int KRIL_Poll(struct file *filp, poll_table *wait)
{
    ...
    if (wake_lock_active(&kril_result_wake_lock) != 0 && TRUE == list_empty(&(gKrilResultQueue.list))) // if wake lock exist and result queue is empty, need to do wake_u
    {
...
void AGPS_CP_Init(void)
{
	...
	lcsClientInfo=InitClientInfo(SIM_DUAL_FIRST);
...
void G_RPC_RespCbk(RPC_Msg_t* pMsg, ResultDataBufHandle_t dataBufHandle, UInt32 userContextData)
{
    ...
    if (pMsg->msgId == MSG_MNSS_CLIENT_SS_SRV_IND)
    {
       gDialogID = reqRep->clientInfo.dialogId;
        KRIL_DEBUG(DBG_INFO, "G_RPC_RespCbk:: Set gDialogID = %lx\n", gDialogID);
    }
    ...
    if (RESULT_OK == reqRep->result && len != 0 && !dataBuf)
    {
        KRIL_DEBUG(DBG_ERROR, "msgId:0x%X len:%ld dataBuf is NULL. Error!!!\n", pMsg->msgId, len);
        RPC_SYSFreeResultDataBuffer(dataBufHandle);
        return;
    }
    // Merged from brcm by lvtongyuan for bug 211654, END
...
static void __exit bcm_fuse_kril_exit_module(void)
{
    ...
    wake_lock_destroy(&kril_result_wake_lock);
#endif
