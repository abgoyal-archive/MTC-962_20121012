...
int ParseRemainingPinAttempt(KRIL_CmdList_t *pdata, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    KRIL_DEBUG(DBG_INFO,"Remaining Attempt: pin1:%d pin2:%d puk1:%d puk2:%d\n",
...
int ParseIsPUK2Blocked(KRIL_CmdList_t *pdata, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    if (1 == *rsp)
    {
        ...
        KRIL_DEBUG(DBG_INFO,"PUK2 is blocked\n");
...
int ParseIsPIN2Blocked(KRIL_CmdList_t *pdata, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    if (1 == *rsp)
    {
        ...
        KRIL_DEBUG(DBG_INFO,"PIN2 is blocked\n");
...
int ParseIsPIN1Enable(KRIL_CmdList_t *pdata, Kril_CAPI2Info_t *capi2_rsp)
{
...

    {
        ...
        if (1 == *rsp)
        {
            KRIL_DEBUG(DBG_INFO,"PIN1 is enable\n");
            p_sim_status->pin1_enable = 1;
        }
        else
        {
            KRIL_DEBUG(DBG_INFO,"PIN1 is disable\n");
...
int ParseIsPIN2Enable(KRIL_CmdList_t *pdata, Kril_CAPI2Info_t *capi2_rsp)
{
...

    {
        ...
        if (1 == *rsp)
        {
            KRIL_DEBUG(DBG_INFO,"PIN2 is enable\n");
            p_sim_status->pin2_enable = 1;
        }
        else
        {
            KRIL_DEBUG(DBG_INFO,"PIN2 is disable\n");
...
void ParseFDNInfo(KRIL_CmdList_t *pcmd, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    KRIL_DEBUG(DBG_INFO,"FDN Info: SimId:%d sFDNfilesize:%d sFDNrecordlength:%d\n", pcmd->ril_cmd->SimId,
...
void SetSimPinStatusHandle(SimNumber_t SimId, SIM_PIN_Status_t SimPinState)
{
    KRIL_DEBUG(DBG_INFO,"SimId:%d SIM PIN status:%d\n", SimId, SimPinState);
...
int ChangeFacilityLockstateHandler(KRIL_CmdList_t *pdata, SIMLOCK_STATE_t* simlock_state)
{
    ...
    switch (GetSimPinStatusHandle(pdata->ril_cmd->SimId))
    {
        ...
        default:
            KRIL_DEBUG(DBG_ERROR,"SimId:%d SIMlock is blocked because attempt exceeded or Error SIM PIN status:%d \n", 
...
void KRIL_QueryNetworkLockHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    switch (pdata->handler_state)
    {
        case BCM_SendCAPI2Cmd:
        {
            ...
            length = SIMLockGetLockCodeInform(SIMLOCK_NETWORK_LOCK, lockinfo, sizeof(lockinfo));
            if (!length)
            {
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
                return;
            ...
            pdata->bcm_ril_rsp = kmalloc(sizeof(UInt8)*(length + 5), GFP_KERNEL);
            if (!pdata->bcm_ril_rsp)
            {
                KRIL_DEBUG(DBG_ERROR,"Allocate bcm_ril_rsp memory failed!!\n");
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
            ...
            pdata->rsp_len = sizeof(UInt8)*(length + 5) ;
            
            rsp = (UInt8*)pdata->bcm_ril_rsp;
            rsp[0] = (UInt8)'B';
            rsp[1] = (UInt8)'R';
            rsp[2] = (UInt8)'C';
            rsp[3] = (UInt8)'M';
            rsp[4] = (UInt8)BRIL_HOOK_QUERY_NETWORK_LOCK;
            memcpy(&rsp[5], lockinfo, length);
            RawDataPrintfun(rsp, (length + 5), "NETWORK_LOCK");
...
void KRIL_QueryNetworkSubsetLockHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    switch (pdata->handler_state)
    {
        case BCM_SendCAPI2Cmd:
        {
            ...
            length = SIMLockGetLockCodeInform(SIMLOCK_NET_SUBSET_LOCK, lockinfo, sizeof(lockinfo));
            if (!length)
            {
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
                return;
            ...
            rsp[2] = (UInt8)'C';
            rsp[3] = (UInt8)'M';
            rsp[4] = (UInt8)BRIL_HOOK_QUERY_NETWORK_SUBSET_LOCK;
            memcpy(&rsp[5], lockinfo, length);
            RawDataPrintfun(rsp, (length + 5), "NET_SUBSET_LOCK");
...
void KRIL_QueryServiceProvideLockHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    switch (pdata->handler_state)
    {
        case BCM_SendCAPI2Cmd:
        {
            ...
            length = SIMLockGetLockCodeInform(SIMLOCK_PROVIDER_LOCK, lockinfo, sizeof(lockinfo));
            if (!length)
            {
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
                return;
            ...
            rsp[2] = (UInt8)'C';
            rsp[3] = (UInt8)'M';
            rsp[4] = (UInt8)BRIL_HOOK_QUERY_SERVICE_PROVIDE_LOCK;
            memcpy(&rsp[5], lockinfo, length);
            RawDataPrintfun(rsp, (length + 5), "PROVIDER_LOCK");
...
void KRIL_QueryCorporateLockHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    switch (pdata->handler_state)
    {
        case BCM_SendCAPI2Cmd:
        {
            ...
            length = SIMLockGetLockCodeInform(SIMLOCK_CORP_LOCK, lockinfo, sizeof(lockinfo));
            if (!length)
            {
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
                return;
            ...
            rsp[2] = (UInt8)'C';
            rsp[3] = (UInt8)'M';
            rsp[4] = (UInt8)BRIL_HOOK_QUERY_CORPORATE_LOCK;
            memcpy(&rsp[5], lockinfo, length);
            RawDataPrintfun(rsp, (length + 5), "CORP_LOCK");
...
void KRIL_QueryPhoneLockHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    switch (pdata->handler_state)
    {
        case BCM_SendCAPI2Cmd:
        {
            ...
            length = SIMLockGetLockCodeInform(SIMLOCK_PHONE_LOCK, lockinfo, sizeof(lockinfo));
            if (!length)
            {
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
                return;
            ...
            rsp[2] = (UInt8)'C';
            rsp[3] = (UInt8)'M';
            rsp[4] = (UInt8)BRIL_HOOK_QUERY_PHONE_LOCK;
            memcpy(&rsp[5], lockinfo, length);
            RawDataPrintfun(rsp, (length + 5), "PHONE_LOCK");
