...
typedef SysAudioParm_t Aud_Parm_Array[AUDIO_APP_NUMBER][AUDIO_MODE_NUMBER];
typedef SysIndMultimediaAudioParm_t MM_Aud_Parm_Array[AUDIO_APP_NUMBER][AUDIO_MODE_NUMBER];
static SysAudioParm_t* audio_parm_table = NULL; //[AUDIO_APP_NUMBER][AUDIO_MODE_NUMBER];
static SysIndMultimediaAudioParm_t* mm_audio_parm_table = NULL; 
...
static int SYSPARM_GetNameFromPA(char* nameaddr, char *str)
{
    ...
    name_ptr = (char*)ioremap_nocache((UInt32)nameaddr, MAX_SYSPARM_NAME_SIZE);
    if(!name_ptr)
    {
        pr_err("[sysparm]: SYSPARM_GetNameFromPA: nameaddr ioremap failed\n");
        return 0;
    }
    ...
    {
        pr_err("[sysparm]: SYSPARM_GetNameFromPA: name_len is invalid:%d\n",name_len);
        iounmap(name_ptr);
        return 0;
    }
...
//
//******************************************************************************
static UInt32 SYSPARM_GePAtByIndex(char *name, unsigned int size, unsigned int flag)
{
    int i;
    ...
    for(i = 0; i < *total_index_ptr; i++)
    {
        ...
        if (!strcmp(sysparm_name, name))
        {
            ...
            {
                pr_err("[sysparm]: SYSPARM_GePAtByIndex: sysparm: \"%s\" size:%u mismatch input size:%u\n",
            ...
            {
                pr_err("[sysparm]: SYSPARM_GePAtByIndex: sysparm: \"%s\" flag:%u mismatch input flag:%u\n",
...
//******************************************************************************
#if 0
static int SYSPARM_SetByIndex(void* ptr, char *name, unsigned int size, unsigned int flag)
{
    UInt32 i, mask;
    ...
    for (i=0; i<*total_index_ptr; i++)
    {
        ...
        {
            if (size != sysparm_index[i].size) return 0;
            if (flag != sysparm_index[i].flag) return 0;
            local_irq_save(mask);
...
UInt16 SYSPARM_GetLogFormat(void)
{
    ...
    log_format_addr = SYSPARM_GePAtByIndex("log_format", sizeof(*log_format_ptr), 1);
    if(!log_format_addr)
    {
    ...
    log_format_ptr = (UInt16*)ioremap_nocache(log_format_addr, sizeof(UInt16));
    if(!log_format_ptr)
    {
...
SysAudioParm_t* APSYSPARM_GetAudioParmAccessPtr(void)
{
    ...
    // don't remap again if we've already done it
	if(audio_parm_table)
    ...
    // get physical address of the audio parms
//    audio_parm_addr = SYSPARM_GePAtByIndex("audio_parm", sizeof(SysAudioParm_t)*AUDIO_APP_NUMBER*AUDIO_MODE_NUMBER, 1);
    audio_parm_addr = SYSPARM_GePAtByIndex("audio_parm", sizeof(Aud_Parm_Array), 1);
    if(!audio_parm_addr)
    {
    ...
    audio_parm_table = (SysAudioParm_t*)ioremap_nocache(audio_parm_addr, sizeof(Aud_Parm_Array));
    if(!audio_parm_table)
    {
...
SysIndMultimediaAudioParm_t* APSYSPARM_GetMultimediaAudioParmAccessPtr(void)
{
    ...
    // don't remap again if we've done it already
    if ( mm_audio_parm_table )
    {
        return mm_audio_parm_table;
    }
    ...
    // get physical address of the multimedia audio parms
    mmaudio_parm_addr = SYSPARM_GePAtByIndex("mmaudio_parm", sizeof(SysIndMultimediaAudioParm_t)*AUDIO_APP_NUMBER*AUDIO_MODE_NUMBER, 1);
    ...
    // get virtual address of multimedia audio parms
    mm_audio_parm_table = (SysIndMultimediaAudioParm_t*)ioremap_nocache(mmaudio_parm_addr, sizeof(SysIndMultimediaAudioParm_t)*AUDIO_APP_NUMBER*AUDIO_MODE_NUMBER );
    if(!mm_audio_parm_table)
    {
...
SysAudioParm_t* SYSPARM_GetExtAudioParmAccessPtr(UInt8 AudioApp)
{
	...
	if(audio_parm_table)
	    return (audio_parm_table + (AUDIO_MODE_NUMBER*AudioApp));
...
UInt16 SYSPARM_GetDefault4p2VoltReading(void)
{
    ...
    batt_Default4p2VoltReading_addr = SYSPARM_GePAtByIndex("batt_Default4p2VoltReading", sizeof(*batt_Default4p2VoltReading_ptr), 1);
    ...
    batt_Default4p2VoltReading_ptr = (UInt16*)ioremap_nocache(batt_Default4p2VoltReading_addr, sizeof(*batt_Default4p2VoltReading_ptr));
    if(!batt_Default4p2VoltReading_ptr)
    {
        pr_err("[sysparm]: batt_Default4p2VoltReading_addr ioremap failed\n");
        return 0;
    }
...
UInt16 SYSPARM_GetActual4p2VoltReading(void)
{
    ...
    batt_Actual4p2VoltReading_addr = SYSPARM_GePAtByIndex("batt_Actual4p2VoltReading", sizeof(*batt_Actual4p2VoltReading_ptr), 1);
    ...
    batt_Actual4p2VoltReading_ptr = (UInt16*)ioremap_nocache(batt_Actual4p2VoltReading_addr, sizeof(*batt_Actual4p2VoltReading_ptr));
    if(!batt_Actual4p2VoltReading_ptr)
    {
        pr_err("[sysparm]: batt_Actual4p2VoltReading_addr ioremap failed\n");
        return 0;
    }
...
UInt16 SYSPARM_GetBattLowThresh(void)
{
    ...
    default_batt_low_thresh_addr = SYSPARM_GePAtByIndex("batt_low_thresh", sizeof(*default_batt_low_thresh_ptr), 1);
    if(!default_batt_low_thresh_addr)
    {
        pr_err("[sysparm]: Get default_batt_low_thresh_addr PA failed\n");
        return 0;
    }
    
    default_batt_low_thresh_ptr = (UInt16*)ioremap_nocache(default_batt_low_thresh_addr, sizeof(*default_batt_low_thresh_ptr));
    if(!default_batt_low_thresh_ptr)
    {
        pr_err("[sysparm]: default_batt_low_thresh_addr ioremap failed\n");
        return 0;
    }
...
UInt8* SYSPARM_GetGPIO_Default_Value(UInt8 gpio_index)
{
    ...
    gpioInit_addr = SYSPARM_GePAtByIndex("gpioInit", sizeof(gpioInit_table), 1);
    if(!gpioInit_addr)
    {
    ...
    gpioInit_ptr = (UInt8*)ioremap_nocache(gpioInit_addr, sizeof(gpioInit_table));
    if(!gpioInit_ptr)
    {
    ...
    memcpy(&gpioInit_table[gpio_index][0], &gpioInit_ptr[gpio_index*5], GPIO_INIT_FIELD_NUM*sizeof(UInt8));
...
Boolean SYSPARM_GetImeiStr(UInt8 sim_id, UInt8* inImeiStr)
{
	...
	{
		if ( (indexA != (SYS_IMEI_LEN - 1) && (pSysparmImei[indexA] & 0xF0) > 0x90) ||
			 (indexA != 0 && (pSysparmImei[indexA] & 0x0F) > 0x09) )
		{
            pr_err("[sysparm]: invalid imei digit, using default\n");
	...
	if ( !bIMEIError &&
		 ((pSysparmImei[0] & 0xF0) == 0 && pSysparmImei[1] == 0 && pSysparmImei[2] == 0 &&
		  pSysparmImei[3] == 0 && pSysparmImei[4] == 0 && pSysparmImei[5] == 0 &&
...
{
    if ( audio_parm_table )
        iounmap(audio_parm_table);
    if ( mm_audio_parm_table )
        iounmap(mm_audio_parm_table);
