...
#ifndef CAM_CORRUPTION_CONCURRENCY
#define CAM_CORRUPTION_CONCURRENCY 1
#endif
...
/**
*       CAM_WriteI2c_Byte write camera sensor registers (camRegID is one byte)
*/
HAL_CAM_Result_en_t CAM_WriteI2c_Byte( 
        UInt8 camRegID,                 ///< (in) 8-bit Sub-Address (register)   
        UInt16 DataCnt,                 ///< (in) count for data bytes to write   
        UInt8 *Data                     ///< (in) pointer to I2C data.  
        );

/**
*       CAM_ReadI2c read camera sensor registers (camRegID is one byte)
*/
HAL_CAM_Result_en_t CAM_ReadI2c_Byte(
        UInt8 camRegID,                 ///< (in) 8-bit Sub-Address (register)   
        UInt16 DataCnt,                 ///< (in) count for data bytes to read   
        UInt8 *Data                     ///< (in) pointer to I2C data.  
        );
...
struct sens_methods {
	...
	HAL_CAM_Result_en_t (*DRV_SetExposure)(int value);
	HAL_CAM_Result_en_t (*DRV_SetFirmware)(void);
};
