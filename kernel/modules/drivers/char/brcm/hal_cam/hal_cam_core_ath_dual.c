...
static int cam_power_up_init(CamSensorSelect_t sensor)
{
	int rc = -1;
...
static int cam_power_down(CamSensorSelect_t sensor)
{
	...
	if (c->sens_m->DRV_SetCamSleep(sensor) != HAL_CAM_SUCCESS) {
		printk(KERN_ERR "Cam power down DRV_SetCamSleep failed \n");
		//return -1;
	}
...
/*
		...
		c->state = CAM_INIT;
		c->sCaptureJpegSize = 0;
		pr_debug("Enabled camera STILLS\n");
	} else {
...
static int __init cam_init(void)
{
	...
	cam_power_up_init(cam_g->curr);
	/*fimware downloade */
	if(c->sens_m->DRV_SetFirmware!=NULL)
		c->sens_m->DRV_SetFirmware();
