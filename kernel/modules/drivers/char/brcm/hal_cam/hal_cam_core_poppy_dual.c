...
/* Changed by bin.yu@tcl-mobile.com , begin */
#define FOCUS_MODE_ENABLE 0
/* Changed by bin.yu@tcl-mobile.com , end */
...
void flash_en_ioctl_ioctl(unsigned long freq, unsigned long duty);

static void printk_cam_ioctl(unsigned int cmd,  unsigned long arg);
...
extern void Preview_config( unsigned int num );

extern struct sens_methods *CAMDRV_primary_get(void);
extern struct sens_methods *CAMDRV_secondary_get(void);
extern FlashLedState_t flash_switch;
int flash_flag_1capture_0preview=0;
static int capture_frame_conuter=0;
static CamStartMode_t poppy_camera_preview_start_mode=CamSM_Poweoff;
static CamStartMode_t poppy_camera_preview_start_mode_old=CamSM_Poweoff;
static char poppy_camera_preview_start_mode_flag=0;/* 1 stop CamStartMode_t; 0 use CamStartMode_t. */
static ktime_t t_fs[5];
static ktime_t t_fe[5];
static int fs_cnt = 0;
static int fe_cnt = 0;
static struct wake_lock flashled_wake_lock;
...
static void taskcallback(UInt32 intr_status, UInt32 rx_status, UInt32 image_addr, UInt32 image_size, UInt32 raw_intr_status, UInt32 raw_rx_status, void *userdata)
{
	...
	if((intr_status & CSL_CAM_INT_FRAME_START) && (c->mode == CAM_STREAM)) {
		...
		if(readl(io_p2v(BCM21553_MLARB_BASE + 0x100)) != 0x0300200f)
				printk(KERN_ERR"****** L-Matrix register changed to 0x%x *******\n", readl(io_p2v(BCM21553_MLARB_BASE + 0x100)));
#endif
...
//printk_cam_ioctl(cmd,arg);
	...
	case CAM_IOCTL_ENABLE_AUTOFOCUS:
		{
			//poppy_camera_preview_start_mode_flag=1;//remove for CTS
			if (cam_g->sens[sensor].sens_m->
...
CAMERADBG("camdrv:ZOOM=%d \n",parm.zoom);
	...
	case CAM_IOCTL_GET_CAPTURE_TYPE:
		...
		break;
	case CAM_IOCTL_SET_START_MODE:
		{
			if(arg==100)
			{
				poppy_camera_preview_start_mode_flag=1;
				break;
			}
			
			if(poppy_camera_preview_start_mode_flag==1)
				break;
			
			if(poppy_camera_preview_start_mode_old==CamSM_Poweoff)
			{
				poppy_camera_preview_start_mode_old=arg;
				if(arg==CamSM_OpenCamera)
					Preview_config(0);
				if(arg==CamSM_OpenVideo)
					Preview_config(1);
				break;
			}
			poppy_camera_preview_start_mode=arg;
//			CAMERADBG("======== poppy_camera_preview_start_mode=%d =========",poppy_camera_preview_start_mode);
			if(arg==CamSM_Poweoff)
			{
				poppy_camera_preview_start_mode_old=CamSM_Poweoff;
				flash_en_ioctl_ioctl(0,0);
			}
			else if( (arg==CamSM_OpenCamera) ||(arg==CamSM_Video2Camera) )
				Preview_config(0);
			else if( (arg==CamSM_OpenVideo) ||(arg==CamSM_Camera2Video) )
				Preview_config(1);
			break;
		}
	default:
		printk(KERN_ERR "Default cam IOCTL *************\n");
		break;
	}
	return rc;
}
 ...
 * The disable method wakes up with gProcessFrameRunning = 0 which means exit */
			...
			if(c->sCaptureJpegSize == 0){
				printk("Major problem with JPEG capture .. size 0 returned but tp is %d\n",tp);
			} else {
			...
			if(c->main.format != CamDataFmtJPEG){
				...
				//printk("youle: after  get jpeg picture %d \n", c->sCaptureJpegSize );
			...
			printk(KERN_ERR"interrupted by signal .. returning 0 length\n");
			c->sCaptureJpegSize = 0;
			c->sCaptureRawSize = 0;
			break;
		}
	}
	printk("Exit process frame\n");

	if(flash_switch==Flash_On)
	{
		//gpio_request(22, "flash_en");
		//if (gpio_is_valid(22))
		capture_frame_conuter=0;
		flash_flag_1capture_0preview=0;
		gpio_direction_output(22, 0);
		ktime_t kt = ktime_get();
		CAMERADBG("after set flash=0[new]: Cam_init end sec %d nsec %d\n",kt.tv.sec,kt.tv.nsec);
	}
...
#define TORCH_IOCTL _IO('T',   88)

static struct miscdevice  torch;

#if 1 //change duty_cycle arg is duty_cycle, frequency is 200Hz
void flash_en_ioctl_ioctl(unsigned long freq,unsigned long duty)
{

	/* 
		following is for LEDTorch
		pwm2 setting: pwm out frequency 200Hz, duty-cycle>0
		freq=2, duty>0
	*/
	if((freq==2/*200Hz*/)&&(duty!=0))
	{
		/* Add the wakelock for Bug202814 [Alvin 2012.01.05] */
		if(!wake_lock_active(&flashled_wake_lock))
			wake_lock(&flashled_wake_lock);

//disable pwm2 output
		writel(readl(0xf8940000) & (~(1<<2)), 0xf8940000);
//enable pwm reference clock
		writel(readl(0xf81401c4) | (1), 0xf81401c4);
//switch gpio22 to pwm2
		writel(readl(0xf8880028) | (1<<5), 0xf8880028);
//pwm_prescale = 1, prescaled clock = 26MHz/2 = 13MHz
		writel(readl(0xf8940004) & (~(7<<8)) | (1<<8), 0xf8940004);
//pwm2_cnt = 13M/(freq*100), PWM2 output frequency is freq*100Hz
		writel(1300*100/freq, 0xf8940018);
//pwm2_high = 13M/(freq*100)*0.5
		writel(1300*duty/freq, 0xf894001c);
//enable pwm2 output
		writel(readl(0xf8940000) | (1<<2), 0xf8940000);
	}

	/* 
		following is for camcorder flash
		pwm2 setting: pwm out frequency 1000Hz, duty-cycle=50%
		freq=10, duty=50
	*/
	else if(freq==10/*1000Hz*/&&(duty==50))
	{
		/* Add the wakelock for Bug202814 [Alvin 2012.01.05] */
		if(!wake_lock_active(&flashled_wake_lock))
			wake_lock(&flashled_wake_lock);
...
//pwm_prescale = 1, prescaled clock = 26MHz/2 = 13MHz
		writel(readl(0xf8940004) & (~(7<<8)) | (1<<8), 0xf8940004);
//pwm2_cnt = 13M/(arg*100), PWM2 output frequency is arg*100Hz
		writel(130*1000/freq, 0xf8940018);
//pwm2_high = 13M/(arg*100)*0.5, duty-cycle is 0.5
		writel(130*500/freq, 0xf894001c);
//enable pwm2 output
		writel(readl(0xf8940000) | (1<<2), 0xf8940000);
	}
#else//change duty_cycle arg is frequency, duty_cycleis 50%
	if(arg!=0)
	{
		/* Add the wakelock for Bug202814 [Alvin 2012.01.05] */
		if(!wake_lock_active(&flashled_wake_lock))
...
//pwm_prescale = 1, prescaled clock = 26MHz/2 = 13MHz
		writel(readl(0xf8940004) & (~(7<<8)) | (1<<8), 0xf8940004);
//pwm2_cnt = 13M/(arg*100), PWM2 output frequency is arg*100Hz
		writel(130*1000/arg, 0xf8940018);
//pwm2_high = 13M/(arg*100)*0.5, duty-cycle is 0.5
		writel(130*500/arg, 0xf894001c);
//enable pwm2 output
		writel(readl(0xf8940000) | (1<<2), 0xf8940000);
	}
#endif
	else if(duty==0)
	{
		/* Add the wakelock for Bug202814 [Alvin 2012.01.05] */
		if(wake_lock_active(&flashled_wake_lock))
			wake_unlock(&flashled_wake_lock);

//disable pwm2 output
		writel(readl(0xf8940000) & (~(1<<2)), 0xf8940000);
//disable pwm reference clock
		writel(readl(0xf81401c4) & (~(0x1)), 0xf81401c4);
//switch pwm2 to gpio22
		writel(readl(0xf8880028) & (~(1<<5)), 0xf8880028);
		...
		if (gpio_is_valid(22))
			...
			printk(KERN_ERR "\n GPIO22 (flash_en) is not vaild \n");
	}
}

static int torch_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg)
{
/*
arg=0 : power off torch
arg>0 : arg Hz pwm
*/
	int ret;

	CAMERADBG(" cmd=%d arg=%d \n", (int)cmd,(int)arg);

	if(cmd!=TORCH_IOCTL)
	{
		printk(" LEDTorch : CMD ERROR ! \n");
		return -1;
	}
	flash_en_ioctl_ioctl(2/*200Hz*/,arg);
	return arg;
}

static int torch_open(struct inode *inode, struct file *filp)
{
	//CAMERADBG("do nothing .\n");
	return 0;
}

static int torch_release(struct inode *inode, struct file *file)
{
	//CAMERADBG("do nothing .\n");
	return 0;
}

static struct file_operations torch_fops = {
	.owner =    THIS_MODULE,
	.ioctl =  torch_ioctl,
	.open =     torch_open,
			 ...
			 const struct i2c_device_id *dev_id)
{
	...
	printk(KERN_ERR"In I2C probe **** \n");
	if(dev_id)
	{
		printk("primary_cam_i2c_probe, dev_id.name (%s) \r\n",dev_id->name);
...
//Add flash led pwm , begin
	torch.minor = MISC_DYNAMIC_MINOR;
	torch.name = "LEDTorch";
	torch.fops = &torch_fops;

	int err = misc_register(&torch);
	if (err) {
		printk("\n error: register torch failed \n.");
		return 0;
	}
//Add flash led pwm , end
			 ...
			 const struct i2c_device_id *dev_id)
{
	...
	if(dev_id)
	{
		printk("secondary_cam_i2c_probe, dev_id.name (%s) \r\n",dev_id->name);
...
//    CAMERADBG("======== poppy_camera_preview_start_mode=%d=========",poppy_camera_preview_start_mode);
	if(poppy_camera_preview_start_mode!=CamSM_Poweoff)  return 0;
...
{
	if(flash_flag_1capture_0preview==1)
	{
		if((capture_frame_conuter++)==3)
			{
				CAMERADBG("\n counter= %d \n",capture_frame_conuter);
				gpio_direction_output(22, 1);
			}

		ktime_t kt = ktime_get();
		CAMERADBG("after set flash=1[new]: Cam_init end sec %d nsec %d\n",kt.tv.sec,kt.tv.nsec);
	}
	cslCamFrameLisr();
	return IRQ_HANDLED;
}
...
{
	if(poppy_camera_preview_start_mode_flag==0)
	{
		if(poppy_camera_preview_start_mode!=CamSM_Poweoff)  
			return 0;
	}
...
//		;//preview_mode_change_toCapture_config0();
//	else
//		;//Preview_config();
...
/*
...
#endif 
				cslCamBuffer1.start_addr = cam_g->cam_buf.phy + (18 * 512 * 1024) - (c->main.size_window.end_pixel * c->main.size_window.end_line * 2);
				cslCamBuffer1.start_addr &= 0xFFFF0000;
				size = sensor_size.resX * sensor_size.resY;
			}
			one_buffer=1;
		...
		cslCamBuffer1.line_stride = sensor_size.resX * 2;
		cslCamBuffer1.buffer_wrap_en = FALSE;	
		cslCamBuffer2.start_addr = (cslCamBuffer1.start_addr + size)+1024;  /*add a gap between */
		...
		c->mode = CAM_STREAM;
		c->state = CAM_INIT;
		CAMERADBG("Enabled camera VF/Video\n");
		poppy_camera_preview_start_mode_flag=0;
	}
...
//    CAMERADBG("======== poppy_camera_preview_start_mode=%d=========",poppy_camera_preview_start_mode);
	...
	flash_en_ioctl_ioctl(0,0);/* Turn off LEDTorch */
...
HAL_CAM_Result_en_t CAM_WriteI2c_Byte(UInt8 camRegID, UInt16 DataCnt, UInt8 *Data)
{
	...
	if (!DataCnt || (DataCnt > MAX_I2C_DATA_COUNT)) {
		...
		printk("%s() - DataCnt = %d out of range\n", __FUNCTION__,
			 DataCnt);
		goto done;
	}
	...
	if ((cam_g->sens[s].cam_i2c_datap != NULL)
		...
		printk("%s() - Camera I2C adapter null\n", __FUNCTION__);
		result = HAL_CAM_ERROR_INTERNAL_ERROR;
	}
done:
	return result;
...
static int __init cam_init(void)
{
	...
	if (!cam_g->cam_regulator || IS_ERR(cam_g->cam_regulator)) {
		printk(KERN_ERR "No Regulator available for DIGLD03_1V8 \n");
		rc = -EFAULT;
		goto err2;
	}
	...
	if (!cam_g->cam_regulator_focus || IS_ERR(cam_g->cam_regulator_focus)) {
		printk(KERN_ERR "No Regulator available for cam_regulator_focus\n");
		rc = -EFAULT;
		goto err2;
	}
	...
	pr_debug("Adding I2C driver fir primay cam **** \n");
	...
	c->sensor_intf = c->sens_m->DRV_GetIntfConfig(cam_g->curr);
	if (!c->sensor_intf) {
		printk(KERN_ERR "Unable to get sensor interface config id %d \n",cam_g->curr);
		rc = -EFAULT;
	}
	cam_g->sens[cam_g->curr].cam_i2c_datap = NULL;
	pr_debug("Adding I2C driver fir secondary cam **** \n");
	...
	/* Add the wakelock for Bug202814 [Alvin 2012.01.05] */
	wake_lock_init(&flashled_wake_lock, WAKE_LOCK_SUSPEND, "flashled_wake_lock");
...
static void __exit cam_exit(void)
{
	...
	*/
	/* Add the wakelock for Bug202814 [Alvin 2012.01.05] */
	wake_lock_destroy(&flashled_wake_lock);
...
static void printk_cam_ioctl(unsigned int cmd,  unsigned long arg)
{
	...
	case CAM_IOCTL_ENABLE:
		{
			CAMERADBG("### cmd=CAM_IOCTL_ENABLE ,arg=%d ###",arg);
			break;
		}
	case CAM_IOCTL_SET_PARAMS:
		{
			CAMERADBG("### cmd=CAM_IOCTL_SET_PARAMS ,arg=%d ###",arg);
			break;
		}
	case CAM_IOCTL_SET_FPS:
		{
			CAMERADBG("### cmd=CAM_IOCTL_SET_FPS ,arg=%d ###",arg);
			break;
		}
	case CAM_IOCTL_SET_THUMBNAIL_PARAMS:
		{
			CAMERADBG("### cmd=CAM_IOCTL_SET_THUMBNAIL_PARAMS ,arg=%d ###",arg);
			break;
		}
	case CAM_IOCTL_MEM_REGISTER:
		{
			CAMERADBG("### cmd=CAM_IOCTL_MEM_REGISTER ,arg=%d ###",arg);
			break;
		}
	case CAM_IOCTL_MEM_BUFFERS:
		{
			//CAMERADBG("### cmd=CAM_IOCTL_MEM_BUFFERS ,arg=%d ###",arg);
			break;
		}
	case CAM_IOCTL_GET_FRAME:
		{
			//CAMERADBG("### cmd=CAM_IOCTL_GET_FRAME ,arg=%d ###",arg);
			break;
		}
	case CAM_IOCTL_GET_FRAME1:
		{
			CAMERADBG("### cmd=CAM_IOCTL_GET_FRAME1 ,arg=%d ###",arg);
			break;
		}
	case CAM_IOCTL_GET_JPEG:
		{
			CAMERADBG("### cmd=CAM_IOCTL_GET_JPEG ,arg=%d ###",arg);
			break;
		}
	case CAM_IOCTL_GET_THUMBNAIL:
		{
			CAMERADBG("### cmd=CAM_IOCTL_GET_THUMBNAIL ,arg=%d ###",arg);
			break;
		}
	case CAM_IOCTL_SET_DIGITAL_EFFECT:
		{
			CAMERADBG("### cmd=CAM_IOCTL_SET_DIGITAL_EFFECT ,arg=%d ###",arg);
			break;
		}
	case CAM_IOCTL_SET_SCENE_MODE:
		{
			CAMERADBG("### cmd=CAM_IOCTL_SET_SCENE_MODE ,arg=%d ###",arg);
			break;
		}
	case CAM_IOCTL_SET_WB:
		{
			CAMERADBG("### cmd=CAM_IOCTL_SET_WB ,arg=%d ###",arg);
			break;
		}
	case CAM_IOCTL_SET_ANTI_BANDING:
		{
			CAMERADBG("### cmd=CAM_IOCTL_SET_ANTI_BANDING ,arg=%d ###",arg);
			break;
		}
	case CAM_IOCTL_SET_FLASH_MODE:
		{
			CAMERADBG("### cmd=CAM_IOCTL_SET_FLASH_MODE ,arg=%d ###",arg);
			break;
		}
	case CAM_IOCTL_SET_FOCUS_MODE:
		{
			CAMERADBG("### cmd=CAM_IOCTL_SET_FOCUS_MODE ,arg=%d ###",arg);
			break;
		}
	case CAM_IOCTL_ENABLE_AUTOFOCUS:
		{
			CAMERADBG("### cmd=CAM_IOCTL_ENABLE_AUTOFOCUS ,arg=%d ###",arg);
			break;
		}
	case CAM_IOCTL_DISABLE_AUTOFOCUS:
		{
			CAMERADBG("### cmd=CAM_IOCTL_DISABLE_AUTOFOCUS ,arg=%d ###",arg);
			break;
		}
	case CAM_IOCTL_SET_JPEG_QUALITY:
		{
			CAMERADBG("### cmd=CAM_IOCTL_SET_JPEG_QUALITY ,arg=%d ###",arg);
			break;
		}
	case CAM_IOCTL_SET_START_MODE:
		{
			CAMERADBG("### cmd=CAM_IOCTL_SET_START_MODE ,arg=%d ###",arg);
			break;
		}
	case CAM_IOCTL_GET_STILL_YCbCr:
		{
			CAMERADBG("### cmd=CAM_IOCTL_GET_STILL_YCbCr ,arg=%d ###",arg);
			break;
		}

	case CAM_IOCTL_SET_ZOOM:
		{
			CAMERADBG("### cmd=CAM_IOCTL_SET_ZOOM ,arg=%d ###",arg);
			break;
		}
	
	case CAM_IOCTL_PHY_ZOOM_ADDR:
		{
			CAMERADBG("### cmd=CAM_IOCTL_PHY_ZOOM_ADDR ,arg=%d ###",arg);
			break;
		}
		
	case CAM_IOCTL_GET_DATA:
		{
			CAMERADBG("### cmd=CAM_IOCTL_GET_DATA ,arg=%d ###",arg);
			break;
		}

	case CAM_IOCTL_GET_CAPTURE_TYPE:
		{
			CAMERADBG("### cmd=CAM_IOCTL_GET_CAPTURE_TYPE ,arg=%d ###",arg);
			break;
		}
		break;
	case CAM_IOCTL_SET_EXPOSURE:
		{
			CAMERADBG("### cmd=CAM_IOCTL_SET_EXPOSURE ,arg=%d ###",arg);
			break;
		}
		break;
	default:
		printk(KERN_INFO"\n ERROR Default cam IOCTL *************\n");
		break;;
	}
	arg=123456;
}
