...
// Enable clock    
    ...
    // Enable Clock Select 1
        ...
        // give it time to stop
            ...
            { 
                pr_debug(  "[cslCamClock][Error] : DIGITAL_CH1_STPRSTS: Clk not Stopped \n") );
                success |= -1;
            }
        ...
        // Check Clock started
            ...
            { 
                pr_debug(  "[cslCamClock][Error] : DIGITAL_CH1_STPRSTS: Clk not Started \n") );
                success |= -1;
            }
        }
        else
        ...
        // give it time to stop
            ...
            { 
                pr_debug(  "[cslCamClock][Error] : DIGITAL_CH0_STPRSTS: Clk not Stopped \n") );
                success |= -1;
            }
        ...
        // Set Dig Clk0 Divider
            ...
            {
                pr_debug(  "[cslCamClock][Error] : DIGITAL_CH0_STPRSTS: Clk Trigger not Set \n") );
                success |= -1;
            }
        #endif
        // Start Dig Clk0
        ...
        // Check Clock running
            ...
            { 
                pr_debug(  "[cslCamClock][Error] : DIGITAL_CH0_STPRSTS: Clk not Started \n") );
                success |= -1;
            }
        }
    }
...
// Only Run Task if Task is Ready 
    ...
    {
        pr_debug(  "[cslCamFrameHisr][Info]: sSemCslCamTask release: \n" );
...
static void cslCamFrameTask( void )
{
	...
	OSSEMAPHORE_Obtain(sSemCslCamTask,TICKS_FOREVER);
        ...
        {
         //   pr_debug(  "[cslCamFrameTask][Info]: sSemCslCamTask Obtain timeout=%d: \n", cslCamDrv.intf_cfg.frame_time_out );
		...
		if (cslCamFrameProcess( (CSL_CAM_HANDLE)camH ) == 0)
		{
		    ...
		    pr_debug(  "[cslCamTaskEntry][Error] : cslCamFrameProcess FAILED \n" );
		}
       //Csl Task Call-back
        ...
        {
            pr_debug(  "[cslCamFrameTask][Info]: sSemCslCamTask Obtain TICKS_FOREVER: active=%d frame_time_out=%d callback=0x%x\n", camH->state.active,cslCamDrv.intf_cfg.frame_time_out,camH->state.task_callback);
...
//    Int32                       success = -1;                   //fail by default
    ...
    pr_debug(  "[cslCamFrameProcess][Info] : size=0x%x ptr=0x%x\n",camH->state.frame_size,camH->state.frame_buffer );
    if ( (camH->state.frame_size == 0) || (camH->state.frame_buffer == NULL) )
    {
        pr_debug(  "[cslCamFrameProcess][Error] : size or pointer incorrect\n" );
        return 0;
    }
...
// FSP Check Not Working in ATHENA)
    ...
    {
        pr_debug(  "[cslCamFrameProcess][Info] : JPEG FSP Check: size=0x%x Rd=0x%x Wr=0x%x \n",image_size,src_ptr8,dest_ptr8 );
        soi_flag = FALSE;
        eoi_flag = FALSE;
        sos_flag =  FALSE;
        marker = 0;
        ...
        while ( (i < camH->state.frame_size) && (eoi_flag == FALSE) )
        {
            ...
            {
                soi_flag = TRUE;
                pr_debug(  "[cslCamFrameProcess][Info] : JPEG SOI OK \n" );
            }
            else if (marker == 0xFFDA)
            {
                sos_flag = TRUE;
                pr_debug(  "[cslCamFrameProcess][Info] : JPEG SOS OK \n" );
            }
            ...
            else if ( (sos_flag == TRUE) && (marker == 0xFF00) )
            {
                ...
                if (a5_data == 0xA5)
                {
                    pr_debug(  "[cslCamFrameProcess][Info] : JPEG FSP A5 removed bytes=0x%x Rd=0x%x Wr=0x%x\n", i,src_ptr8,dest_ptr8 );
                    a5_data = *src_ptr8++; // throw out A5
                }
                else
                {
                    pr_debug(  "[cslCamFrameProcess][Info] : JPEG FSP FAILED: bytes=0x%x Rd=0x%x Wr=0x%x\n", i,src_ptr8,dest_ptr8 );
                }
            }
        ...
        if (eoi_flag == FALSE)
        {
            pr_debug(  "[cslCamFrameProcess][Info] : JPEG EOI FAILED: bytes=0x%x Rd=0x%x Wr=0x%x\n", i,src_ptr8,dest_ptr8 );
        }
    }
#endif
    return image_size;
...
// Set interface pointer
    ...
    // Set IOCR in Init state
        ...
        {
            pr_debug(  "[csl_cam_init][Error] :  cslCamIocrSet() FAILED \n" );
            return -1;
        }
	#endif
    ...
    // Assign Port Channels for each handle        
        ...
        // Init CHAL
            ...
            {
                pr_debug(  "[csl_cam_init][Error] : chal_cam_init() FAILED \n" );
                success |= -1;
            }
            else
            {
            ...
            #endif
                pr_debug(  "[csl_cam_init]: Ok \n" );
            }
        }
        else
        {
            success |= -1;
            pr_debug(  "[csl_cam_init][Error] : cslCamTaskInit() FAILED \n" );
        }    
    }    
    return success;
}
...
Int32 csl_cam_exit( void )
{
    ...
    // Remove Task
        ...
        {
            pr_debug(  "[csl_cam_exit]: cslCamTaskDeInit]: Ok \n" );
        }
        else
        {
            pr_debug(  "[csl_cam_exit][ERROR]:  cslCamTaskDeInit \n" );
            success = -1;
        }
	#endif
        cslCamDrv.init = 0;
    } 
    pr_debug(  "[csl_cam_exit][Info] : exit: \n" );
    return success;
}
...
#endif
#endif
    pr_debug(  "[csl_cam_open][Info] : Start! \n" );
...
// need to guard against NULL params being passed
    *cslCamH = NULL;
    ...
    if( intfCfg )
    {
        if ( cslCamDrv.init != 1 )
        {
            pr_debug(  "[csl_cam_open][Error] : Not Initiailized \n" );
            return ( success );
        }
    // Check for maximum channels already opened
        if (cslCamDrv.use_count == CAM_MAX_HANDLES)
        {
            pr_debug(  "[csl_cam_open][Error] : MAX handles allocated! \n" );
            return ( success );
        }    
    ...
    // If Channel opened, check Interface & AFE port is same
        ...
        {
            pr_debug(  "[csl_cam_open][Error] : Channel opened, Interface must be same! \n" );
            return ( success );
        }    
    ...
    // Get available channel
        ...
        while ( (instance < CAM_MAX_HANDLES) && (camH == NULL) )
        {
            ...
            { 
                ...
                pr_debug(  "[csl_cam][Info] : csl_cam_open(): Instance=0x%x \n", instance );
            }
            instance++;
        }
    // Verify channel available
        if (camH == NULL)
        {            
            pr_debug(  "[csl_cam_open][Error] : No Channels available! \n" );
            return ( success );
        }
    ...
    // If Initial Channel opening on Interface
        ...
        // Update Interface Info
            ...
            pr_debug(  "[csl_cam][Info] : csl_cam_open(): intf=0x%x afe=0x%x chan=0x%x input=0x%x time_out=0x%x \n", 
        ...
        // Set IOCR in open state
            ...
            {
                pr_debug(  "[csl_cam_open][Error] :  cslCamIocrSet() FAILED \n" );
                return -1;
            }
#endif
#if ( defined (_ATHENA_) )
        ...
        // Set DSI Clock Registers
            ...
            {
                pr_debug(  "[csl_cam][Info] : csl_cam_open(): CLKDRV_Set_Clock(CLK_MIPIDSI_CMI): ERROR \n" );
            }
        // Set AFE Clock Registers
            ...
            {
                pr_debug(  "[csl_cam][Info] : csl_cam_open(): CLKDRV_Set_Clock(CLK_MIPIDSI_AFE): ERROR \n" );
            }
        // Start MIPI Clock
            if (CLKDRV_Start_Clock(sClkHandle, CLK_MIPIDSI_CMI) != CLKDRV_STATUS_OK)
            {
                pr_debug(  "[csl_cam][Info] : csl_cam_open(): CLKDRV_Start_Clock(CLK_MIPIDSI_CMI): ERROR \n" );
            }
#endif
 	...
 	* processing */
        ...
        {
            pr_debug(  "[csl_cam_open][Error] :  chal_cam_xxx: FAILED \n" );
        }
    }
    else
    {
        pr_debug(  "[csl_cam_open][Error] :  params: FAILED \n" );
    }
...
// If all handles closed on this interface, close everything
    if (cslCamDrv.use_count == 0)
    {
        ...
        {
            pr_debug(  "[csl_cam_close][Error] :  csl_cam_reset(): CSL_CAM_RESET_SWR: FAILED \n" );
            success |= -1;
        }
        if ( csl_cam_reset( cslCamH, CSL_CAM_RESET_ARST ) )
        {
            pr_debug(  "[csl_cam_close][Error] :  csl_cam_reset(): CSL_CAM_RESET_ARST: FAILED \n" );
            success |= -1;
        }
    // Disable all Lanes  
        chal_cam_param_st.intf = cslCamChalIntf(cslCamDrv.intf_cfg.intf);
        ...
        {
            pr_debug(  "[csl_cam_close][Error] : ERROR!\n" );
            success |= -1;
        }
    ...
    // Power Down Analog
        ...
        {
            pr_debug(  "[csl_cam_close][Error] :  chal_cam_set_analog_pwr(): CHAL_CAM_ANALOG: FAILED \n" );
            success |= -1;
        }
        chal_cam_param_st.select = CHAL_CAM_ANALOG;
        if (chal_cam_set_analog_pwr(cslCamDrv.chalCamH, &chal_cam_param_st) != CHAL_OP_OK)
    ...
    // Set IOCR in closed state
        ...
        {
            pr_debug(  "[csl_cam_close][Error] :  cslCamIocrSet() FAILED \n" );
        }
	#endif
#if ( defined (_ATHENA_) )
    // Disable Clocks
...
Int32 csl_cam_get_handle( CSL_CAM_HANDLE* cslCamH )
{
    ...
    *cslCamH = NULL;
    ...
    {
        pr_debug(  "[csl_cam_get_handle][Error] :  No Handle available \n" );
    }
    return success;
}
...
// Panic
    ...
    {
        pr_debug(  "[csl_cam_set_intf_control][Info] : ERROR!\n" );
    }    
...
Int32 csl_cam_set_input_mode( CSL_CAM_HANDLE cslCamH, pCSL_CAM_INPUT_st inputMode )
{
    ...
    pr_debug(  "[csl_cam_set_input_mode][Info] : mode = 0x%x\n", inputMode->input_mode );
...
// Cfg Lane Enables 
    ...
    if (chal_status != CHAL_OP_OK)
    {
        pr_debug(  "[csl_cam_set_input_mode][Info] : ERROR!\n" );
        success |= -1;
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
...
Int32 csl_cam_set_input_addr( CSL_CAM_HANDLE cslCamH, pCSL_CAM_BUFFER_st imageAddr_0, pCSL_CAM_BUFFER_st imageAddr_1, pCSL_CAM_BUFFER_st dataAddr )
{
    ...
    if (imageAddr_0 != NULL)
    {   
        pr_debug(  "[csl_cam_set_input_addr][Info] : Image0 -> addr=0x%x, size=%d, line_stride=%d, buffer_wrap_en=%d \n", 
        ...
        if (imageAddr_1 != NULL)
        {
            ...
            pr_debug(  "[csl_cam_set_input_addr][Info] : Image1 -> addr=0x%x, size=%d, line_stride=%d, buffer_wrap_en=%d \n", 
                           imageAddr_1->start_addr, imageAddr_1->size, imageAddr_1->line_stride, imageAddr_1->buffer_wrap_en );  
        }
        else
        {    
            pr_debug(  "[csl_cam_set_input_addr][Info] : Image1 -> NULL \n"); 
            memset( &camH->state.image_buffer_1, 0, sizeof( CSL_CAM_BUFFER_st_t ) );            
        }
    }
    else
    {        
       pr_debug(  "[csl_cam_set_input_addr][Error] : ERROR! imageAddr_0 is NULL. No image buffers \n" );    
       success |= -1;
    }
...
// Set Image buffer
    ...
    {
        pr_debug(  "[csl_cam_set_buffer_params][Info] : ERROR!\n" );
    }    
...
// Interrupts
    ...
    {
        success = -1;
        pr_debug(  "[csl_cam_set_frame_control][Error] :  LINE_START/LINE_END not allowed!\n" );
    }
...
// Line Count
    pr_debug(  "[csl_cam_set_frame_control][Info] :  intr=0x%x line_count=%d\n",chal_cam_frame_cfg_st.interrupts,frameCtrl->int_line_count );
...
// Capture mode
    ...
    {
        success = -1;
        pr_debug(  "[csl_cam_set_frame_control][Info] : ERROR!\n" );
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
    return success;
}
...
// Image Channel Id's
    ...
    {
        success = -1;
        pr_debug(  "[csl_cam_set_image_type_control][Info] : ERROR!\n" );
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
    return success;
}
...
// FSP Decoding
    ...
    {
        success = -1;
        pr_debug(  "[csl_cam_set_data_type_control][Info] : ERROR!\n" );
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
    return success;
}
...
// Config packet captures
    ...
    {
        success = -1;
        pr_debug(  "[csl_cam_set_packet_control][Info] : ERROR!\n" );
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
    return success;
}
...
// Parameter
    ...
    {
        success = -1;
        pr_debug(  "[csl_cam_set_lane_control][Info] : ERROR!\n" );
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
    return success;
}
...
// Decode Pixel unpacking
    ...
    {
        success = -1;
        pr_debug(  "[csl_cam_set_pipeline_control][Info] : ERROR!\n" );
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
    return success;
}
...
// Channel
    ...
    {
        success = -1;
        pr_debug(  "[csl_cam_set_image_window][Info] : ERROR!\n" );
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
    return success;
}
...
// Channel
    ...
    {
        success = -1;
        pr_debug(  "[csl_cam_get_buffer_status][Info] : ERROR!\n" );
    }
    return success;
}
...
// Lane Status    
    ...
    *laneStatus = cslCamChalLaneStatus( (CHAL_CAM_LANE_STATE_t)chal_cam_param_st.param );
    ...
    {
        success = -1;
        pr_debug(  "[csl_cam_get_lane_status][Info] : ERROR!\n" );
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
    return success;
}   
...
// Clock to Camera Sensor
	...
	if(camclk == NULL)
        ...
        pr_debug(  "[csl_cam_set_clk][Info] : csl_cam_set_clk(): enable_clock: \n" );
...
// Channel
    ...
    {
        success = -1;
        pr_debug(  "[csl_cam_trigger_capture][Info] : ERROR!\n" );
    }
    else
    {
    // Update Capture mode
...
{
	pr_debug("csl_cam_rx_start called \n");
    Int32                       success = 0;                    // pass by default
    CAM_HANDLE                  camH = (CAM_HANDLE)cslCamH;
    CHAL_CAM_STATUS_CODES       chal_status = CHAL_OP_OK;
    CHAL_CAM_PARAM_st_t         chal_cam_param_st;
...
// Verify Receiver was opened
    ...
    // Set IOCR in stream state
        ...
        {
            pr_debug(  "[csl_cam_rx_start][Error] :  cslCamIocrSet() FAILED \n" );
    ...
    // Release Task Semaphore to put in proper time-out state
    	//pr_debug("we are here 1 \n");
        if ( (sSemCslCamTask != NULL) && (sCslCamTask != NULL) )
        {
        	//pr_debug("we are here 2 \n");
        	OSSEMAPHORE_Release(sSemCslCamTask);
	/*
            sem_count = OSDAL_SEMA_COUNT(sSemCslCamTask);
            if (sem_count == 0)
            {
            //pr_debug("we are here 3 \n");
                pr_debug(  "[csl_cam_rx_start][Info]: sSemCslCamTask release: \n") );
    ...
    if (chal_status != CHAL_OP_OK)
    {
    	pr_debug("Error is CHAL status \n");
        success = -1;
        pr_debug(  "[csl_cam_rx_start][Info] : ERROR!\n" );
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
	...
	pr_debug("Mo error in CHAL status\n");
...
// Verify Receiver is active
    ...
    // Set Receiver to Inactive
        ...
        if ( csl_cam_reset( cslCamH, CSL_CAM_RESET_ARST ) )
        {
            pr_debug(  "[csl_cam_rx_stop][Error] :  csl_cam_reset(): CSL_CAM_RESET_ARST: FAILED \n" );
            success |= -1;
        }
    // Release Task Semaphore to put in proper time-out state
        ...
        if ( (sSemCslCamTask != NULL) && (sCslCamTask != NULL) )
        {
            ...
            if (sem_count == 0)
            {
                pr_debug(  "[csl_cam_rx_stop][Info]: sSemCslCamTask release: \n" );
                OSDAL_SENDEVENT( sSemCslCamTask ); //let task handle this state.
            }
        }
	#endif
    ...
    {
        success = -1;
        pr_debug(  "[csl_cam_rx_stop][Info] : ERROR!\n" );
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
    return success;
}
...
// Reset 
    ...
    {
        success = -1;
        pr_debug(  "[csl_cam_reset][Info] : ERROR!\n" );
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
    return success;
}
...
// CallBack Event
    switch (event)
    {
        ...
        default:
            success = -1;
            pr_debug(  "[csl_cam_register_event_callback][Info] : ERROR! No Selection\n" );
            break;
    }
    return success;
}
