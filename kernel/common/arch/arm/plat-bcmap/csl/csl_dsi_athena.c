...
static CSL_LCD_RES_T cslDsiDmaStart ( DSI_UPD_REQ_MSG_T* updMsg )
{
    ...
    updMsg->dmaChInfo.dstMaster = 0;
    updMsg->dmaChInfo.srcMaster = 0;
    ...
    if (updMsg->updReq.multiLLI) {
                ...
                if (DMADRV_Bind_Data_Ex
                    ...
                    != DMADRV_STATUS_OK) {
                        LCD_DBG(LCD_DBG_ID, "[CSL DSI] cslDsiDmaStart:"
    ...
    )
{
    ...
    dsiH->clkCfg.hsPll_N_int    = (UInt32)( hsClk.clkIn_MHz / 4);
    hsClkSet = 4 * dsiH->clkCfg.hsPll_N_int;
    ...
    )
{
    ...
    UInt8		excess_bytes;
    UInt32		buf_size;
    UInt32		*params_buf_vir_ptr = NULL;
    dma_addr_t		params_buf_phy_ptr;
    DMA_CHANNEL		dsi_dcs_dma_ch;
    Dma_Buffer_List	dsi_dcs_buf_list;
    Dma_Data		dsi_dcs_dma_data;
    Dma_Chan_Info	dsi_dcs_dma_chinfo;
    CHAL_DSI_CM_CFG_t	dsi_dcs_cmcfg;
...
#ifndef _BRCM_8BYTE_MSG_CONSTRAINT
    if(command->msgLen > 255) {
	return CSL_LCD_ERR;
    }
    else if(command->msgLen > 8) {
	printk("[CSL_DSI_SendPacket:]DCS params > 7 bytes\n");
	if(((command->msgLen-1)%4) != 0) {
	    buf_size = ((command->msgLen -1)/4)*4;
	    excess_bytes = (command->msgLen - 1)%4;
	}
	else {
	    buf_size = (command->msgLen -1);
	    excess_bytes = 0;
	}
	// use CMD_DATA_FIFO(DCS command) + PIXEL_FIFO (DCS params)
	// Program the dma for dumping the params to the pixel fifo here and for CMD_DATA_FIFO the rest of logic should take care of it.
	//params_buf_vir_ptr = kzalloc(buf_size,GFP_ATOMIC | __GFP_DMA);
	params_buf_vir_ptr = dsiH->params_buf;
	if(ZERO_OR_NULL_PTR(params_buf_vir_ptr)) {
	    //error
	    LCD_DBG ( LCD_DBG_ID,
		"[CSL DSI] CSL_DSI_SendPacket(): ERROR allocating memory for DMA transfer\r\n");
	    return CSL_LCD_ERR;
	}
	params_buf_phy_ptr = (dma_addr_t)virt_to_phys(params_buf_vir_ptr);
	memcpy(params_buf_vir_ptr,(command->msg+(1+excess_bytes)),buf_size);
	// Reserve Channel

	printk("[CSL_DSI_SendPacket:]params_buf_vir_ptr[%x] buf_size[%d]\n",params_buf_vir_ptr,buf_size);
	if ( DMADRV_Obtain_Channel(
		DMA_CLIENT_MEMORY,
		DMA_CLIENT_DSI_CM,
		&dsi_dcs_dma_ch ) != DMADRV_STATUS_OK )
	{
	    LCD_DBG ( LCD_DBG_ID,
		"[CSL DSI] CSL_DSI_SendPacket(): ERROR Obtaining DMA Channel\r\n");
	    //printk("[CSL_DSI_SendPacket:] error obtaining DMA Channel\n");
	    return CSL_LCD_DMA_ERR;
	}
	printk("[CSL_DSI_SendPacket:] dsi_dcs_dma_ch:%d\n",dsi_dcs_dma_ch);
	dsi_dcs_dma_chinfo.srcID          = DMA_CLIENT_MEMORY;
	dsi_dcs_dma_chinfo.dstID          = DMA_CLIENT_DSI_CM;
	dsi_dcs_dma_chinfo.type           = DMA_FCTRL_MEM_TO_PERI;
	dsi_dcs_dma_chinfo.srcBstSize     = DMA_BURST_SIZE_16;
	dsi_dcs_dma_chinfo.dstBstSize     = DMA_BURST_SIZE_16;
	dsi_dcs_dma_chinfo.srcDataWidth   = DMA_DATA_SIZE_32BIT;
	dsi_dcs_dma_chinfo.dstDataWidth   = DMA_DATA_SIZE_32BIT;
	dsi_dcs_dma_chinfo.dstMaster = 0;
	dsi_dcs_dma_chinfo.srcMaster = 0;
	dsi_dcs_dma_chinfo.incMode        = DMA_INC_MODE_SRC;
	dsi_dcs_dma_chinfo.xferCompleteCb = (DmaDrv_Callback)dsiH->dma_cb;
	dsi_dcs_dma_chinfo.freeChan       = TRUE;
	dsi_dcs_dma_chinfo.prot           = 0;
	dsi_dcs_dma_chinfo.bCircular      = FALSE;
	dsi_dcs_dma_chinfo.alignment      = DMA_ALIGNMENT_32;			/* Need to check it may have to be set to DMA_ALIGNMENT_8*/

	if ( DMADRV_Config_Channel (dsi_dcs_dma_ch, &dsi_dcs_dma_chinfo)
	    != DMADRV_STATUS_OK )
	{
	    LCD_DBG ( LCD_DBG_ID,
		"[CSL DSI] CSL_DSI_SendPacket(): ERROR DMADRV_Config_Channel, "
		"DmaCh[%d]\r\n", dsi_dcs_dma_ch );
	    printk("[CSL_DSI_SendPacket:] error configuring DMA Channel\n");
	    return CSL_LCD_DMA_ERR;		/* check whether this */
	}

	dsi_dcs_buf_list.buffers[0].srcAddr	= params_buf_phy_ptr;
	dsi_dcs_buf_list.buffers[0].destAddr	= 0x084a0140;
	dsi_dcs_buf_list.buffers[0].length	= buf_size;
	dsi_dcs_buf_list.buffers[0].bRepeat	= 0;
	dsi_dcs_buf_list.buffers[0].interrupt	= 1;
	dsi_dcs_dma_data.numBuffer		= 1;
	dsi_dcs_dma_data.pBufList		= (Dma_Buffer_List *)&dsi_dcs_buf_list;


	if ( DMADRV_Bind_Data(dsi_dcs_dma_ch, &dsi_dcs_dma_data )
	    != DMADRV_STATUS_OK )
	{
	    LCD_DBG ( LCD_DBG_ID,
		"[CSL DSI] CSL_DSI_SendPacket(): ERROR DMADRV_Bind_Data, "
		"DmaCh[%d]\r\n", dsi_dcs_dma_ch );
	    printk("[CSL_DSI_SendPacket:] error bind data to DMA Channel\n");
	    return CSL_LCD_DMA_ERR;
	}

	    if ( DMADRV_Start_Transfer ( dsi_dcs_dma_ch ) != DMADRV_STATUS_OK )
	{
	    LCD_DBG ( LCD_DBG_ID,
		"[CSL DSI] CSL_DSI_SendPacket(): ERROR DMADRV_Start_Transfer, "
		"DmaCh[%d]\r\n", dsi_dcs_dma_ch );
	    printk("[CSL_DSI_SendPacket:] error starting Channel\n");
	    return CSL_LCD_DMA_ERR;
	}

	// set DE1 ColorMode
	chal_dsi_de1_set_cm ( dsiH->chalH, DE1_CM_565 );
	// enable DE1
	chal_dsi_de1_enable ( dsiH->chalH, TRUE  );
	chal_dsi_de1_set_wc ( dsiH->chalH, (buf_size>>1) );
   }
#endif	//_BRCM_8BYTE_MSG_CONSTRAINT
    {
    ...
    {
        if( !clientH->hasLock ) OSSEMAPHORE_Release ( dsiH->semaDsi );
	    //printk("[CSL_DSI_SendPacket:failed to send the command\n]");
...
#ifndef _BRCM_8BYTE_MSG_CONSTRAINT
    if((command->msgLen > 8) && (!ZERO_OR_NULL_PTR(params_buf_vir_ptr))) {
	//printk("[CSL_DSI_SendPacket:]DCS params > 7 bytes transfer completed with stat[%x]\n",stat);
	//kfree(params_buf_vir_ptr);
	params_buf_vir_ptr = NULL;
	chal_dsi_de1_enable ( dsiH->chalH, FALSE  );
	DMADRV_Force_Shutdown_Channel(dsi_dcs_dma_ch);
	DMADRV_Release_Channel(dsi_dcs_dma_ch);
	chal_dsi_clr_fifo ( dsiH->chalH,
	    CHAL_DSI_CTRL_CLR_CMD_DATA_FIFO | CHAL_DSI_CTRL_CLR_PIX_DATA_FIFO );
    }
#endif	//_BRCM_8BYTE_MSG_CONSTRAINT
    if( !clientH->hasLock ) OSSEMAPHORE_Release ( dsiH->semaDsi );
...
#define MAX_SIZE_ONE_PACKET 64
    ...
    UInt32              pktWc;
    Boolean             isTE;
 //   unsigned int* src_buffer = (unsigned int*)phys_to_virt((req->buff));
    ...
    if ( dsiH->ulps )
		return CSL_LCD_BAD_STATE;
	transferWc = req->lineCount*req->lineLenP;
    if( !clientH->hasLock )
       OSSEMAPHORE_Obtain ( dsiH->semaDsi, TICKS_FOREVER );
    chal_dsi_clr_status ( dsiH->chalH, 0xFFFFFFFF );
    // set DE1 ColorMode
    ...
    {
		// DCS START + DCS CONTINUE
		if(transferWc > MAX_SIZE_ONE_PACKET)
			pktWc = MAX_SIZE_ONE_PACKET;
		else
		pktWc = transferWc;
		// SET NUMBER OF AHB TRANSACTIONs REQUIRED PER DSI PACKET, NA to HERA
		chal_dsi_de1_set_wc ( dsiH->chalH, pktWc );
		dsiChH->chalCmCfg.pktCount = 1;
		pktSizeBytes  =pktWc * dsiChH->bpp_wire;
		if( pktSizeBytes > DSI_PKT_SIZE_MAX )
			return ( CSL_LCD_ERR );
		// DSI PACKET SIZE IN BYTEs
		dsiChH->chalCmCfg.pktSizeBytes = pktSizeBytes;

		chal_dsi_update_pixel_fifo(dsiH->chalH,data_buff,MAX_SIZE_ONE_PACKET);
		// send first line with START CMND
		chal_dsi_clr_status ( dsiH->chalH, 0xFFFFFFFF );
		chal_dsi_de1_send ( dsiH->chalH, TX_PKT_ENG_0, &dsiChH->chalCmCfg );
        ...
        // send rest of the data with CONTINUE CMND
		dsiChH->chalCmCfg.dcsCmnd  = dsiChH->dcsCmndCont;
		transferWc -= pktWc;
	}
	...
	if( !clientH->hasLock ) OSSEMAPHORE_Release ( dsiH->semaDsi );
	if( req->cslLcdCb )
		req->cslLcdCb (res, clientH,req->cslLcdCbRef);
	//printk(KERN_ERR "after packet send\n");
	return ( res );
}
// CSL_DSI_CPU_UpdateCmVc
...
//*****************************************************************************
#ifndef _BRCM_8BYTE_MSG_CONSTRAINT
CSL_LCD_RES_T  CSL_DSI_Init ( const pCSL_DSI_INIT_CFG initCfg, const pCSL_DSI_CFG dsiCfg,void *ptr )
#else
CSL_LCD_RES_T  CSL_DSI_Init ( const pCSL_DSI_INIT_CFG initCfg, const pCSL_DSI_CFG dsiCfg)
#endif	//_BRCM_8BYTE_MSG_CONSTRAINT
{
    CSL_LCD_RES_T       res = CSL_LCD_OK;
    DSI_HANDLE          dsiH;
    ...
    if( dsiH->initOnce != DSI_INITIALIZED )
    {
        ...
        if( dsiH->bus == 0 )
        {
            dsiH->dsiCoreRegAddr = initCfg->dsi_base_address;
            dsiH->interruptId    = initCfg->interruptId;
    ...
    {   
        ...
        else
        {
            ...
            {
                chal_dsi_de1_set_dma_thresh ( dsiH->chalH, 48 );
