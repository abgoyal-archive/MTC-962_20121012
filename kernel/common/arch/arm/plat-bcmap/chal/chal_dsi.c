...
typedef struct
{
    ...
    UInt32  period;          // <out> dbg
} DSI_COUNTER;

// hsBitRate_Mbps Period Record
typedef struct
{
    UInt32 hsBitRate_Mbps;
    UInt32 ui_ns;
    UInt32 MulFactor;
} DSI_hsBitRate;

//escClk_MHz Period Record
typedef struct
{
    UInt32 escClk_MHz;
    UInt32 escClk_ns;
...
// DSI_hsBitRate Frequency & Time period Values
DSI_hsBitRate hsBitRate_Array[]=
{
    {100, 10000, 1000},
    {104, 9615, 1000},
    {108, 9259, 1000},
...
//DSI_escClk Frequency and Time Period Values
DSI_escClk escClk_array[]=
{
    {8, 125000, 1000},
    {9, 111111, 1000},
    {10, 100000, 1000},
...
{                         
    ...
    { "HS_INIT"      , DSI_C_TIME_HS,  0,
            100000,0,0,0,  0,0,  0,0xFFFFFFFF,  4,4,  0 },
    ...
    )
{
    ...
    counter  = (dividend*MulFactor) / divisor;
    ...
    if((counter > pDsiC[i].counter_offs))
    ...
    )
{
    ...
    cUInt32         escClk_ns;
    cUInt32         i;
    cUInt32         esc_MulFactor;
    cUInt32         hs_MulFactor;
    cUInt32         MulFactor;
    DSI_COUNTER *   pDsiC;
    CHAL_DSI_HANDLE pDev;
    DSI_hsBitRate * phsBitRate;
    DSI_escClk *    pescClk;

	chal_dprintf ( CDBG_INFO, "chal_dsi_set_timing() escClk_MHz:%d hsBitRate_Mbps:%d lpBitRate_Mbps:%d ", escClk_MHz, hsBitRate_Mbps, lpBitRate_Mbps);
    
    pDev        = (CHAL_DSI_HANDLE) handle;

    phsBitRate  = &hsBitRate_Array;
    pescClk     = &escClk_array;

    /* Setting the default values for the ui_ns and hs_MulFactor for DSI clock freqeuncy of 500(to the Max supported) MHz*/
    ui_ns          = phsBitRate[((sizeof(hsBitRate_Array)/sizeof(DSI_hsBitRate)) -1)].ui_ns;
    hs_MulFactor      = phsBitRate[((sizeof(hsBitRate_Array)/sizeof(DSI_hsBitRate)) -1)].MulFactor;
    /* Setting the default values for the ui_ns and esc_MulFactor for  esc clock freqeuncy of 78(to the Max supported) MHz*/
    escClk_ns      = pescClk[((sizeof(escClk_array)/sizeof(DSI_escClk)) - 1)].escClk_ns;
    esc_MulFactor      = pescClk[((sizeof(escClk_array)/sizeof(DSI_escClk)) - 1)].MulFactor;

    for (i=0; i < (sizeof(hsBitRate_Array)/sizeof(DSI_hsBitRate)); i++) {
        if(phsBitRate[i].hsBitRate_Mbps == hsBitRate_Mbps) {
            ui_ns          = phsBitRate[i].ui_ns;
            hs_MulFactor      = phsBitRate[i].MulFactor;
		    break;
        }
    }
    //ui_ns          = phsBitRate[i].ui_ns;
    //MulFactor      = phsBitRate[i].MulFactor;

    for (i=0; i < (sizeof(escClk_array)/sizeof(DSI_escClk)); i++) {
            if(pescClk[i].escClk_MHz == escClk_MHz) {
            escClk_ns      = pescClk[i].escClk_ns;
            esc_MulFactor      = pescClk[i].MulFactor;
		    break;
        }
    }
    //escClk_ns      = pescClk[i].escClk_ns;
    //MulFactor      = pescClk[i].MulFactor;
    ...
    if( ! chalDsiTimingDivAndRoundUp( pDsiC, DSI_C_ESC2LP_RATIO, 
        (cUInt32)escClk_MHz, (cUInt32)lpBitRate_Mbps * 2 * esc_MulFactor, esc_MulFactor) )
    {
        return ( FALSE );
    }            
    ...
    {
        // Period_min1 [ns]
        time_min1 =  (cUInt32)pDsiC[i].time_min1_ns
                   + ((cUInt32)pDsiC[i].time_min1_ui * ui_ns)/hs_MulFactor;

        // Period_min2 [ns]
        if( pDsiC[i].mode & DSI_C_MIN_MAX_OF_2 )
            time_min2 = (cUInt32)pDsiC[i].time_min2_ns
                      + ((cUInt32)pDsiC[i].time_min2_ui * ui_ns)/hs_MulFactor;
        else
            time_min2 = 0;
        ...
        if( pDsiC[i].mode & DSI_C_HAS_MAX )
            time_max = (cUInt32)pDsiC[i].time_max_ns
                     + ((cUInt32)pDsiC[i].time_max_ui * ui_ns)/hs_MulFactor;
        else
            time_max = 0;
        // Period_units [ns]
        if ( pDsiC[i].timeBase & DSI_C_TIME_HS ) {
            period = ui_ns;
            MulFactor = hs_MulFactor;
        } else if ( pDsiC[i].timeBase & DSI_C_TIME_ESC ) {
            period = escClk_ns;
            MulFactor = esc_MulFactor;
        } else
            period = 0;
        ...
        if( period != 0 )
        {
            res = chalDsiTimingDivAndRoundUp ( pDsiC, i, time_min, period, MulFactor);
            if( !res )
                return ( res );
                
            if( pDsiC[i].mode & DSI_C_HAS_MAX )
            {
                if( (((cUInt32)pDsiC[i].counter * period)/MulFactor ) > time_max )
                {
    ...
    for( i=0; i < DSI_C_MAX; i++ )
    {
        ...
        else
        {
            chal_dprintf ( CDBG_INFO, "[cHAL DSI] chal_dsi_set_timing: "
                ...
                pDsiC[i].name, 
                pDsiC[i].counter, 
                ((cUInt32)pDsiC[i].counter + pDsiC[i].counter_offs)
                * pDsiC[i].period );
        }
    }
...
/***************************************************
// Function Name: chal_dsi_update_pixel_fifo


*************************************************/

CHAL_DSI_RES_t chal_dsi_update_pixel_fifo (
    CHAL_HANDLE      handle,
    cUInt32*  bufPtr,
    cUInt32  bufSize )

{
    ...
    for( i=0; i < bufSize; i++ )
    {
         BRCM_WRITE_REG ( pDev->baseAddr, DSI_PIX_FIFO0, bufPtr[i] );
...
#ifndef _BRCM_8BYTE_MSG_CONSTRAINT
	if(cmnd->msgLen > 8) {
	    wc_fifo = 1+((cmnd->msgLen - 1)%4);
	    wc_params = cmnd->msgLen - wc_fifo;
	    printk("[chal_dsi_send_cmnd:]More than 8 bytes so using both cmdfifo and pixelfifo wc_fifo[%d] wc_params[%d]\n",wc_fifo,wc_params);
	}
	else {
	    wc_fifo = cmnd->msgLen;
	    wc_params = cmnd->msgLen;
	}
	pkth =   DSI_REG_FIELD_SET ( DSI_CMD_PKTH, WC_FIFO,  wc_fifo )
	       | DSI_REG_FIELD_SET ( DSI_CMD_PKTH, WC_PARAM, wc_params );
#else
