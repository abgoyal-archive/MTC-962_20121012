...
enum {
	BCM21553_CORECLK_MHZ_312 = (312U),
	BCM21553_CORECLK_MHZ_832 = (832U),
};

#define BCM_CORECLK_TURBO	BCM21553_CORECLK_MHZ_832
...
static struct bcm_freq_tbl bcm215xx_cpu0_freq_tbl[] = {
	FTBL_INIT(BCM_CORE_CLK_NORMAL, 1200000),
	FTBL_INIT(BCM_CORECLK_TURBO, 1360000),
};
/* BCM21553 CPU info */
...
void __init update_turbo_freq(void)
{
	...
	cpu_freq = (cpu_freq * 2) / 3;          /* in Hz  */
	cpu_freq = cpu_freq / 1000 / 1000;      /* in MHz */

	bcm215xx_cpu0_freq_tbl[BCM_TURBO_MODE].cpu_freq = cpu_freq;
...
#define SYSPARM_VOLT(name, dst)						\
	do {								\
		...
		if (ret == 0) {						\
			pr_info("sysparm: %s: %x\n", name, parm);	\
			parm = max89xx_csr_reg2volt(parm);		\
			if (parm >= 0)					\
				dst = (u32)parm;			\
		}							\
	} while (0)
