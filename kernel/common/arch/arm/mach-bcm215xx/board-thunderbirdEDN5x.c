...
#define CONFIG_HAPTIC_ISA1000 1
#endif
#endif
...
extern  void __iomem *cdebugger_mem_base;
#endif
...
/*    mutex_lock(&bcm4325_pwr_lock); */
	...
	gpio_request(31, "wlan_bt_reset");
	gpio_request(SDIO1_WLAN_MUX_TTL, "wlan_mux");
...
void bcm_wlan_power_on(int val)
{
	...
	pr_info("%s: Enter.\n ", __FUNCTION__);
		...
		gpio_request(BT_RST_CHECK, "bt_check_on");
		gpio_request(BT_VREG_CTRL, "bt_vreg_on");
		...
		gpio_free(BT_RST_CHECK);
		gpio_free(BT_VREG_CTRL);
...
void bcm_wlan_power_off(int val)
{
	...
	default:
		...
		gpio_request(BT_RST_CHECK, "bt_check_off");
		gpio_request(BT_VREG_CTRL, "bt_vreg_off");
...
//wqf
	...
	{BCM_KEY_ROW_7, BCM_KEY_COL_7, "unused", 0},
        ...
        {BCM_KEY_ROW_1, BCM_KEY_COL_6, "Volume Up", KEY_VOLUMEUP},
        {BCM_KEY_ROW_1, BCM_KEY_COL_7, "unused", 0},
        ...
        {BCM_KEY_ROW_2, BCM_KEY_COL_6, "Volume Down", KEY_VOLUMEDOWN},
        {BCM_KEY_ROW_2, BCM_KEY_COL_7, "unused", 0},
        {BCM_KEY_ROW_3, BCM_KEY_COL_0, "unused", 0},
        ...
        {BCM_KEY_ROW_3, BCM_KEY_COL_6, "Home_key", KEY_HOME},
        {BCM_KEY_ROW_3, BCM_KEY_COL_7, "unused", 0},
...
#endif
#if defined (CONFIG_HAPTIC_ISA1000)
void isa1000_gpio_setup(void)
{
	board_sysconfig(SYSCFG_HAPTIC_VIB, SYSCFG_INIT);
}

struct platform_device haptic_isa1000_pwm_device = {
	.name   = "samsung_pwm_haptic",
	.id     = -1,
};

static struct haptic_platform_data haptic_isa1000_data = {
	.name = "isa1000",
	.gpio = 56,
	.setup_pin = isa1000_gpio_setup,
	.pwm_timer = 0,
        .regl_id = "haptic_pwm",
};
#endif /* CONFIG_HAPTIC_ISA1000 */


#ifdef CONFIG_LEDS_GPIO
#define BCM21553_GREEN_LED 22
#define BCM21553_RED_LED 39
static struct gpio_led gpio_leds[] = {
	{
		.name	= "green",
		.default_trigger = "timer",
		.gpio	= BCM21553_GREEN_LED ,
		.active_low = 0,
	},
	{
		.name	= "red",
		.default_trigger = "timer",
		.gpio	= BCM21553_RED_LED ,
		.active_low = 0,
	},
};
...
{
	/* GPIO 52 muxed with GPEN7 */
	board_sysconfig(SYSCFG_COMPASS, SYSCFG_INIT);
	gpio_request(52, "akm8975");
	gpio_direction_input(52);
	set_irq_type(GPIO_TO_IRQ(52), IRQF_TRIGGER_RISING);
...
/*
EDN50x uses ALDO9 to power VDDO_SDIO1
*/
...
struct i2c_slave_platform_data ft5x06_i2c_pdata = {
        .i2c_spd = I2C_SPD_400K,
};
#endif
...
#ifdef CONFIG_TOUCHSCREEN_FT5X06
{
	...
	.platform_data = &ft5x06_i2c_pdata,
	.irq = GPIO_TO_IRQ(PEN_IRQ_GPIO),
},
#endif
};
...
static struct platform_device *devices[] __initdata = {
	...
	&leds_gpio,
#endif
#ifdef CONFIG_HAPTIC_ISA1000
	&haptic_isa1000_pwm_device,
#endif
#ifdef CONFIG_USB_ANDROID
        &android_usb_device,
#endif
...
#define GPEN11_AS_GPIO56 ((1 << 17) | (1 << 20))
		if (op == SYSCFG_INIT) {
			writel(readl(io_p2v(BCM21553_SYSCFG_BASE) + 0x00) |
					GPEN11_AS_GPIO56,
					io_p2v(BCM21553_SYSCFG_BASE) + 0x00);
...
static void __init bcm21553_athenaray_init(void)
{
...
#endif
#ifdef CONFIG_HAPTIC_ISA1000
	haptic_isa1000_pwm_device.dev.platform_data = &haptic_isa1000_data,
#endif
#ifdef CONFIG_KEYBOARD_BCM
...
#define GPIO_RED_LED 	(39)
	/* Configure RED LED GPIO as output GPIO to save power */
...
#define GPIO_GREEN_LED 	(22)
	/* Configure GREEN LED GPIO as output GPIO to save power */
	gpio_request(GPIO_GREEN_LED, "gpio_green_led");
	gpio_direction_output(GPIO_GREEN_LED, 0);
	gpio_free(GPIO_GREEN_LED);
#else
#define GPIO_HOME_LED 22
        gpio_request(GPIO_HOME_LED, "gpio_home_led");
        gpio_direction_output(GPIO_HOME_LED, 0);
        gpio_free(GPIO_HOME_LED);
#endif
...
#define GPIO51	51
	gpio_request(GPIO51, "gpio51");
	gpio_direction_input(GPIO51);
	bcm_gpio_pull_up(GPIO51, false);
	bcm_gpio_pull_up_down_enable(GPIO51, false);
	gpio_free(GPIO51);
...
//added by wqf begin  
//undefine CONFIG_LEDS_GPIO  CONFIG_KEYBOARD_BCM  CONFIG_SENSORS_AK8975
#if 1
#define GPIO_LCD_ID 0
        gpio_request(GPIO_LCD_ID, "gpio_lcd_id");
        gpio_direction_input(GPIO_LCD_ID);
        gpio_free(GPIO_LCD_ID);

#define GPIO_TW_WAKEUP 5
        gpio_request(GPIO_TW_WAKEUP, "gpio_tw_wakeup");
        gpio_direction_output(GPIO_TW_WAKEUP, 1);
        gpio_free(GPIO_TW_WAKEUP);

#define GPIO_CAM1_PWDN 6
        gpio_request(GPIO_CAM1_PWDN, "gpio_cam1_pwdn");
        gpio_direction_output(GPIO_CAM1_PWDN, 0);
        gpio_free(GPIO_CAM1_PWDN);

#define GPIO_CAM_3M_RST 8
        gpio_request(GPIO_CAM_3M_RST, "gpio_cam_3m_rst");
        gpio_direction_output(GPIO_CAM_3M_RST, 1);
        gpio_free(GPIO_CAM_3M_RST);

#define GPIO_CAM_3M_ID 9
        gpio_request(GPIO_CAM_3M_ID, "gpio_cam_3m_id");
        gpio_direction_input(GPIO_CAM_3M_ID);
        gpio_free(GPIO_CAM_3M_ID);

#define GPIO_TW_ID 10
        gpio_request(GPIO_TW_ID, "gpio_tw_id");
        gpio_direction_input(GPIO_TW_ID);
        gpio_free(GPIO_TW_ID);

#define GPIO_PROX_INT 11
        gpio_request(GPIO_PROX_INT, "gpio_prox_int");
        gpio_direction_input(GPIO_PROX_INT);
        gpio_free(GPIO_PROX_INT);

#define GPIO_GPS_RESET_N 13
        gpio_request(GPIO_GPS_RESET_N, "gpio_gps_reset_n");
        gpio_direction_output(GPIO_GPS_RESET_N, 1);
        gpio_free(GPIO_GPS_RESET_N);

#define GPIO_TW_RST 23
        gpio_request(GPIO_TW_RST, "gpio_tw_rst");
        gpio_direction_output(GPIO_TW_RST, 1);
        gpio_free(GPIO_TW_RST);

#define GPIO_LCD_RST_EMI 24
        gpio_request(GPIO_LCD_RST_EMI, "gpio_lcd_rst_emi");
        gpio_direction_output(GPIO_LCD_RST_EMI, 1);
        gpio_free(GPIO_LCD_RST_EMI);

#define GPIO_SPEAKER_EN 26
        gpio_request(GPIO_SPEAKER_EN, "gpio_speaker_en");
        gpio_direction_output(GPIO_SPEAKER_EN, 0);
        gpio_free(GPIO_SPEAKER_EN);

#define GPIO_CAM_3M_PWDN 35
        gpio_request(GPIO_CAM_3M_PWDN, "gpio_cam_3m_pwdn");
        gpio_direction_output(GPIO_CAM_3M_PWDN, 1);
        gpio_free(GPIO_CAM_3M_PWDN);

#define GPIO_ACCELE_INT2 38
        gpio_request(GPIO_ACCELE_INT2, "gpio_accele_int2");
        gpio_direction_input(GPIO_ACCELE_INT2);
        gpio_free(GPIO_ACCELE_INT2);
#endif
//added by wqf end
	return 0;
}
...
static int ramdump_panic(struct notifier_block *this, unsigned long event, void *ptr)
{
    ...
    iowrite32(0x34,cdebugger_mem_base);
