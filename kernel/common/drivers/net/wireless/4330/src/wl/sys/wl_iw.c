...
extern unsigned int DVFS_Disable;
extern struct mutex g_dvfs_mutex; /* qiuxu */
extern int g_driver_exiting;/* qiuxu */
#endif
...
#endif /* COEX_DHCP */
				pm=PM_FAST;
				dev_wlc_ioctl(dev, WLC_SET_PM, &pm, sizeof(pm_local)); //Shri
		}
...
)
{
	...
	/* if(wlan_enc_client ) */
	if(wlan_enc_client && DVFS_Disable > 0/* avoid double enabling dvfs to dump kernel warning, qiuxu */) 
	{
...
#ifdef CONFIG_CPU_FREQ_GOV_BCM21553
	mutex_lock(&g_dvfs_mutex); 
	if (g_driver_exiting) { /* occured CQ - MobC00171375, qiuxu */
		mutex_unlock(&g_dvfs_mutex);
		WL_ERROR(("Warning: Calling %s() during driver removing!\n", __FUNCTION__));
		return -1; 
	}

	if(wlan_enc_client && (DVFS_Disable==0))
	{
