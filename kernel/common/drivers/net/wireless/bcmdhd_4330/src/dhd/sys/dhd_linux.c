 ...
 *
 * $Id: dhd_linux.c 291923 2011-10-25 21:35:26Z $
 */
...
unsigned int DVFS_Disable=0;
EXPORT_SYMBOL(DVFS_Disable);
/* CQ - MobC00171375:
 * wl_iw_control_wl_on() preempted cpufreq_bcm_client_put() during driver removing
 * to increase dvfs ref count, when return to cpufreq_bcm_client_put(), kernel 
 * panic by BUG.
 * Solution: 1) Protect dvfs operations by mutex
 *           2) Get wl_iw_control_wl_on() exited directly if driver is the process
 *              of removing.
 * by qiuxu 110919
*/
DEFINE_MUTEX(g_dvfs_mutex);
int g_driver_exiting = 0;
#endif
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	DHD_ERROR(("%s: TRY  Start DVFS WLAN as Client Registration DVFS Ptr=%x \n", __FUNCTION__,wlan_enc_client));
		mutex_lock(&g_dvfs_mutex);
		DHD_ERROR(("Patch mutex for dvfs\n"));
		if(!wlan_enc_client)
		{
			...
			/* Turn off dvfs at WLAN STARTup up so that the  process is not
			 * slowed down.
			 */
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	if (!ret) {
		...
		DHD_ERROR(("iovar = %02x %02x %02x %02x %02x %02x\n", ea_addr.octet[0], ea_addr.octet[1]
			, ea_addr.octet[2], ea_addr.octet[3]
			, ea_addr.octet[4], ea_addr.octet[5]));
#if 0    //if driver writes mac address bus fails, it will continue to execute.
		if (ret < 0) {
...
#ifdef CONFIG_CPU_FREQ_GOV_BCM21553
	mutex_lock(&g_dvfs_mutex);
	DHD_ERROR(("Patch mutex for dvfs and driver removing flag\n"));
	g_driver_exiting = 1;
	DHD_ERROR(("%s: WLAN as DVFS Client Start Release DVFS Ptr=%x \n", __FUNCTION__,wlan_enc_client));
	if(wlan_enc_client )
	{
	if (DVFS_Disable > 0) /* dvfs would be enabled by wl_iw_control_wl_off(), MobC00171375, added by qiuxu */
