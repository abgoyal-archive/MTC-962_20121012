/* drivers/input/keyboard/ft5x06_i2c.c
 *
 *
 */
...
#include <mach/gpio.h>
//#include <mach/vreg.h>
#include "ft5x06_i2c.h"


#undef DEBUG_FT5X06

#define FT5X06_I2C_NAME "ft5x06-ts"
#define REQUIRED_VERSION 0x12
...
struct i2c_client *ft5x06_client = NULL;
...
static struct proc_dir_entry *ft5x06proc;
static int proc_ft5x06_read(char *page, char **start,
                             off_t off, int count,
                             int *eof, void *data)
{
        int len, ret, i;
...
/*
	...
	*/
	...
	printk("ft5x06_ts_probe: 0x3b: %2x %2x %2x %2x\n",
...
static void add_ft5x06_proc(void)
{
	ft5x06proc = create_proc_read_entry("ft5x06proc",
	                                 0444, NULL,
	                                 proc_ft5x06_read,
	                                 NULL);
	if(ft5x06proc== NULL) {
...
int32_t ft5x06_i2c_txdata(unsigned short saddr,
	unsigned char *txdata, int length)
{
	...
	if (i2c_transfer(ft5x06_client->adapter, msg, 1) < 0) {
		CDBG("ft5x06_i2c_txdata faild\n");
		return -EIO;
	}
...
int ft5x06_i2c_rxdata(int txlen, unsigned char *rxdata, int length)
{
	struct i2c_msg msgs[] = {
	{
	...
	if (i2c_transfer(ft5x06_client->adapter, msgs, 2) < 0) {
		CDBG("ft5x06_i2c_rxdata failed!\n");
		return -EIO;
	}
	return 0;
}
...
static char write_reg(unsigned char addr, char v)
{
	...
	rc = ft5x06_i2c_txdata(ft5x06_client->addr, tmp, 4);
	if (rc < 0){
		CDBG("ft5x06 write reg failed!\n");
		return rc;
	}
	return 0;
}
...
char read_reg(unsigned char addr)
{
	...
	tmp[0] = I2C_WORK_STARTREG;
	tmp[1] = addr +0x40;
	rc = ft5x06_i2c_rxdata(2, tmp, 2);
	if (rc < 0){
		CDBG("ft5x06_i2c_read failed!\n");
		return rc;
	}
	return tmp[0];
}
...
static int32_t read_info(void * buf)
{
	...
	len = 26;
	rc = ft5x06_i2c_rxdata(1, tmp, len);
	if (rc < 0){
		CDBG("ft5x06_i2c_read failed!\n");
		return rc;
	}
...
static int ft5x06_init_panel(struct ft5x06_ts_data *ts)
{
	int ret;
...
/*********************************************************/
/****** Function porting from Ft5x06 Sample code *********/
...
/****** Function porting from Ft5x06 Sample code end *****/
/*********************************************************/
...
static enum hrtimer_restart ft5x06_ts_timer_func(struct hrtimer *timer)
{
	struct ft5x06_ts_data *ts = container_of(timer, struct ft5x06_ts_data, timer);
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		printk(KERN_ERR "ft5x06_ts_probe: need I2C_FUNC_I2C\n");
		ret = -ENODEV;
	...
	//client->irq=MSM_GPIO_TO_INT(FT5X06_INT);
	client->irq=GPIO_TO_IRQ(FT5X06_INT);	
	ts->client = client;
	ft5x06_client = client;
	i2c_set_clientdata(client, ts);
#if 0	
	ts->power = ft5x06_power;
	if (ts->power) {
		ret = ts->power(1);
		if (ret < 0) {
			printk(KERN_ERR "ft5x06_ts_probe power on failed\n");
	...
	ret = read_reg(0x3d);
	printk("ft5x06 id is %02x\n", ret);
	if (ret != 0x79) {
		printk(KERN_ERR "ft5x06 probe failed\n");
		goto err_detect_failed;
	}
	printk("found ft5x06 touch screen!!!\n");
	

     /****************upgrade tp******************/ 
#if 0
        ret = read_reg(0x3b);
	printk("ft5x06 focaltec's version is %x\n",ret);	
	if(ret <= 0)
	   {printk(KERN_ERR "ft5x06 read 0x3b failed\n");
                goto err_detect_failed;     
	}

	if((ret != REQUIRED_VERSION)&&(ret != 0x07)&&(ret >= 0)){
		printk("ft5x06 firmware upgrade.........\n");
	...
	/*
	printk(KERN_INFO "ft5x06_ts_probe: 0xe0: %x %x %x %x %x %x %x %x\n",
	       buf1[0], buf1[1], buf1[2], buf1[3],
	       buf1[4], buf1[5], buf1[6], buf1[7]);
	       */

	ret = ft5x06_init_panel(ts); /* will also switch back to page 0x04 */
	if (ret < 0) {
		printk(KERN_ERR "ft5x06_init_panel failed\n");
		goto err_detect_failed;
	}
	...
	if (ts->input_dev == NULL) {
		ret = -ENOMEM;
		printk(KERN_ERR "ft5x06_ts_probe: Failed to allocate input device\n");
		goto err_input_dev_alloc_failed;
	}
	ts->input_dev->name = "ft5x06-touchscreen";
	...
	if (ret) {
		printk(KERN_ERR "ft5x06_ts_probe: Unable to register %s input device\n", ts->input_dev->name);
		goto err_input_register_device_failed;
	}
	
       //set_irq_type(GPIO_TO_IRQ(FT5X06_INT), IRQF_TRIGGER_FALLING);
	//ret = request_irq(GPIO_TO_IRQ(FT5X06_INT), ft5x06_ts_irq_handler, IRQF_TRIGGER_RISING, client->name, ts);
	ret = request_irq(GPIO_TO_IRQ(FT5X06_INT), ft5x06_ts_irq_handler, IRQF_TRIGGER_FALLING, client->name, ts);
	if (ret == 0)
		ts->use_irq = 1;
	else
	...
	if (!ts->use_irq) {
		...
		ts->timer.function = ft5x06_ts_timer_func;
		hrtimer_start(&ts->timer, ktime_set(1, 0), HRTIMER_MODE_REL);
	}
	
#ifdef CONFIG_HAS_EARLYSUSPEND
	ts->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 1;
	ts->early_suspend.suspend = ft5x06_ts_early_suspend;
	ts->early_suspend.resume = ft5x06_ts_late_resume;
	register_early_suspend(&ts->early_suspend);
#endif

	printk(KERN_INFO "ft5x06_ts_probe: Start touchscreen %s in %s mode\n", ts->input_dev->name, ts->use_irq ? "interrupt" : "polling");
#ifdef DEBUG_FT5X06
	reg_dump();
	add_ft5x06_proc();
#endif
...
static int ft5x06_ts_suspend(struct i2c_client *client, pm_message_t mesg)
{
	int ret;
	struct ft5x06_ts_data *ts = i2c_get_clientdata(client);
	printk("=========Enter %s\n",__func__);
	...
	if (ts->power) {
		ret = ts->power(0);
		if (ret < 0)
			printk(KERN_ERR "ft5x06_ts_resume power off failed\n");
	}	
#endif	
	return 0;
}

static int ft5x06_ts_resume(struct i2c_client *client)
{
	int ret;
	struct ft5x06_ts_data *ts = i2c_get_clientdata(client);
	printk("=========Enter %s\n",__func__);	
	...
	if (ts->power) {
		ret = ts->power(1);
		if (ret < 0)
			printk(KERN_ERR "ft5x06_ts_resume power on failed\n");
	}
#endif

	gpio_set_value(FT5X06_WK,1);
	gpio_set_value(FT5X06_WK,0);
	udelay(500);
	gpio_set_value(FT5X06_WK,1);
...
{
	struct ft5x06_ts_data *ts;
	ts = container_of(h, struct ft5x06_ts_data, early_suspend);
	ft5x06_ts_suspend(ts->client, PMSG_SUSPEND);
}
...
static struct i2c_driver ft5x06_ts_driver = {
	...
	.driver = {
		.name	= FT5X06_I2C_NAME,
	},
};

static int __devinit ft5x06_ts_init(void)
{
	int ret;

        gpio_request(FT5X06_WK, "gpio_tw_wakeup");
        gpio_direction_output(FT5X06_WK, 1);

        gpio_request(FT5X06_INT, "gpio_tw_int");
        gpio_direction_input(FT5X06_INT);		
