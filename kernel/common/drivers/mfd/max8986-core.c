...
static struct max_pmu_irq *max89xx_find_irq_handler(struct max8986 *max8986,
	int irq)
{
...
int max89xx_request_irq(struct max8986 *max8986, int irq, bool enable_irq,
	void (*handler) (int, void *), void *data)
{
	...
	if (WARN_ON(max89xx_find_irq_handler(max8986, irq)))
		return -EBUSY;
	...
	enable_irq ? max89xx_enable_irq(max8986, irq) : \
		max89xx_disable_irq(max8986, irq);

	return 0;
}
EXPORT_SYMBOL_GPL(max89xx_request_irq);

int max89xx_free_irq(struct max8986 *max8986, int irq)
{
	struct max_pmu_irq  *irq_info;
	...
	irq_info = max89xx_find_irq_handler(max8986, irq);
	if (irq_info) {
		mutex_lock(&max8986->list_lock);
...
EXPORT_SYMBOL_GPL(max89xx_free_irq);

int max89xx_disable_irq(struct max8986 *max8986, int irq)
{
	int reg_inx;
	int st;
	int reg;
	...
	handler = max89xx_find_irq_handler(max8986, irq);
	if (handler)
		handler->irq_enabled = false;

	return 0;
}
EXPORT_SYMBOL_GPL(max89xx_disable_irq);

int max89xx_enable_irq(struct max8986 *max8986, int irq)
{
	int reg_inx;
	int st;
	int reg;
...
EXPORT_SYMBOL_GPL(max89xx_enable_irq);

int max89xx_register_ioctl_handler(struct max8986 *max8986, u8 sub_dev_id,
...
EXPORT_SYMBOL_GPL(max89xx_register_ioctl_handler);

void max89xx_shutdown(struct max8986 *max8986)
{
	int ret = 0;
	u8 hostact;
...
EXPORT_SYMBOL(max89xx_shutdown);

static void max89xx_power_off(void)
{
	pr_debug("%s\n", __func__);

	if (max8986_info)
		max89xx_shutdown(max8986_info);
}

static void max89xx_irq_workq(struct work_struct *work)
{
...
static irqreturn_t max89xx_irq(int irq, void *dev_id)
{
...
static int max89xx_client_dev_register(struct max8986 *max8986,
	const char *name)
{
...
static int max89xx_find_slave(struct max8986 *max8986, int slave)
{
	int i;
...
static int max89xx_read_pmu_register(struct max8986 *max8986, u32 command,
	u8 *reg_val)
{
	s32 data = 0;
	int i;
	...
	i = max89xx_find_slave(max8986, slave);
...
static int max89xx_write_pmu_register(struct max8986 *max8986, u32 command,
	u8 val)
{
	int ret = -EINVAL;
	int i;
...
static int max89xx_read_pmu_mul_register(struct max8986 *max8986, u32 command,
	u32 length, u8 *val)
{
	int ret = -EINVAL;
	int index;
	...
	mutex_lock(&max8986->i2c_rw_lock);
	index = max89xx_find_slave(max8986, slave);
...
static int max89xx_write_pmu_mul_register(struct max8986 *max8986, u32 command,
	u32 length, u8 *val)
{
	int ret = -EINVAL;
	int index;
...
static void max89xx_init_chip(struct max8986 *max8986)
{
	int i;
	u8 reg_val;
...
static int max89xx_open(struct inode *inode, struct file *file)
{
	pr_debug("%s\n", __func__);
...
int max89xx_release(struct inode *inode, struct file *file)
{
	file->private_data = NULL;
	return 0;
}

static int max89xx_ioctl(struct inode *inode, struct file *file,
...
static void max89xx_dbg_usage(void)
{
	pr_info("Usage:\n");
...
static int max89xx_dbg_parse_args(char *cmd, struct pmu_debug *dbg)
{
...
static ssize_t max89xx_write(struct file *file, const char __user *buffer,
	size_t len, loff_t *offset)
{
	...
	if (max89xx_dbg_parse_args(cmd, &dbg) < 0) {
		max89xx_dbg_usage();
		return -EINVAL;
	}
...
static int __devinit max89xx_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	...
	INIT_WORK(&max8986->work, max89xx_irq_workq);
	...
	max8986->read_dev =  max89xx_read_pmu_register;
	max8986->write_dev =  max89xx_write_pmu_register;
	max8986->read_mul_dev = max89xx_read_pmu_mul_register;
	max8986->write_mul_dev = max89xx_write_pmu_mul_register;

	max8986_info = max8986;

	max8986->read_dev(max8986, MAX8986_PM_REG_ID, &val);
	max8986->rev = val;

	max89xx_init_chip(max8986);
...
static int __devexit max89xx_remove(struct i2c_client *client)
{
	struct max8986 *max8986;
	int i;
...
static struct i2c_driver max8986_driver = {
	...
	.probe = max89xx_probe,
	.remove = __devexit_p(max89xx_remove),
};

static int __init max89xx_init(void)
{
	return i2c_add_driver(&max8986_driver);
}

static void __exit max89xx_exit(void)
{
	i2c_del_driver(&max8986_driver);
}

subsys_initcall(max89xx_init);
module_exit(max89xx_exit);
