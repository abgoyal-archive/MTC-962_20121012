...
//add by Xu Danbin for PR253087/CSP#517773 reset the time if the year is early than 2012
static int max89xx_rtc_set_time(struct device *dev, struct rtc_time *tm);
 ...
 */
static int max89xx_rtc_read_time(struct device *dev, struct rtc_time *tm)
{
...
static int max89xx_rtc_alarm_irq_enable(struct device *dev, unsigned enabled)
{
	int ret = 0;
	struct max8986_rtc *max8986_rtc = dev_get_drvdata(dev);
	struct max8986 *max8986 = max8986_rtc->max8986;
	...
	if (enabled)
		ret = max89xx_enable_irq(max8986, MAX8986_IRQID_INT1_RTCA);
	else
		ret = max89xx_disable_irq(max8986, MAX8986_IRQID_INT1_RTCA);
...
static int max89xx_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alm)
{
	struct max8986_rtc *max8986_rtc = dev_get_drvdata(dev);
	struct max8986 *max8986 = max8986_rtc->max8986;
	int ret = 0;
...
static int max89xx_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alm)
{
	struct max8986_rtc *max8986_rtc = dev_get_drvdata(dev);
	struct max8986 *max8986 = max8986_rtc->max8986;
	int ret = 0;
	...
	if (alm->enabled == 0) {
		max89xx_rtc_alarm_irq_enable(dev, 0);
		return 0;
	}
	...
	max89xx_rtc_alarm_irq_enable(dev, 1);
...
// Fix the bug#254909 of auto-bootup(CSP#520632) [Alvin 2012.04.13.]
//enable=0 ,disable alarm
//enable=1, enable alarm

static int max89xx_rtc_alarm_enable(struct device *dev, int enable)
{
    struct max8986_rtc *max8986_rtc = dev_get_drvdata(dev);
    struct max8986 *max8986 = max8986_rtc->max8986;
    int ret = 0;
   ...
   ret = max8986->read_mul_dev(max8986, MAX8986_RTC_REG_SEC_ALARM1,RTC_NO_OF_ALARM_REGS, &alarm_regs[0]);
   if (ret < 0) {
        pr_err("%s: read alarm regs failed\n", __func__);
        return ret;
   } 
   ...
   ret = max8986->write_mul_dev(max8986, MAX8986_RTC_REG_SEC_ALARM1,RTC_NO_OF_ALARM_REGS, &alarm_regs[0]);
   ret |= max8986->read_dev(max8986, MAX8986_RTC_REG_UPDATE1, &update_reg);
   ret |= max8986->write_dev(max8986, MAX8986_RTC_REG_UPDATE1,(update_reg | MAX8986_RTC_UPDATE_REGISTERS));
   if (ret != 0) {
             pr_err("%s: disable alarm failed: ret %d\n", __func__, ret);
             return ret;
   } 
...
static const struct rtc_class_ops max8986_rtc_ops = {
	.read_time = max89xx_rtc_read_time,
	.set_time = max89xx_rtc_set_time,
	.read_alarm = max89xx_rtc_read_alarm,
	.set_alarm = max89xx_rtc_set_alarm,
	.alarm_irq_enable = max89xx_rtc_alarm_irq_enable,
	.update_irq_enable = max89xx_rtc_alarm_irq_enable,
};

#ifdef CONFIG_PM
static int max89xx_rtc_suspend(struct device *dev)
{
	pr_debug("%s\n", __func__);
	return 0;
}

static int max89xx_rtc_resume(struct device *dev)
{
	pr_debug("%s\n", __func__);
	 ...
	 */
	max89xx_rtc_alarm_irq_enable(dev, 0);
    max89xx_rtc_alarm_enable(dev,0); // Fix the bug#254909 of auto-bootup(CSP#520632) [Alvin 2012.04.13.]
#endif

	return 0;
}
#else
#define max89xx_rtc_suspend NULL
#define max89xx_rtc_resume NULL
#endif
 ...
 */
static void max89xx_rtc_time_fixup(struct device *dev)
{
	...
	max89xx_rtc_read_time(dev, &current_rtc_time);
	...
	if (current_rtc_time.tm_wday != reconverted_rtc_time.tm_wday) {
		...
		current_rtc_time.tm_wday = reconverted_rtc_time.tm_wday;
		max89xx_rtc_set_time(dev, &current_rtc_time);
	}
}

static int max89xx_rtc_probe(struct platform_device *pdev)
{
	...
	ret = max89xx_request_irq(max8986, MAX8986_IRQID_INT1_RTCA,
	...
	max89xx_rtc_time_fixup(&pdev->dev);
	pr_info("%s: passed\n", __func__);
...
err_rtc_register:
	max89xx_free_irq(max8986_rtc->max8986, MAX8986_IRQID_INT1_RTCA);
...
static int __devexit max89xx_rtc_remove(struct platform_device *pdev)
{
...
static struct dev_pm_ops max8986_rtc_pm_ops = {
	.suspend = max89xx_rtc_suspend,
	.resume = max89xx_rtc_resume,
	.thaw = max89xx_rtc_resume,
	.restore = max89xx_rtc_resume,
	.poweroff = max89xx_rtc_suspend,
};

static struct platform_driver max8986_rtc_driver = {
	.probe = max89xx_rtc_probe,
	.remove = __devexit_p(max89xx_rtc_remove),
	.driver = {
		   .name = "max8986-rtc",
...
static int __init max89xx_rtc_init(void)
{
	return platform_driver_register(&max8986_rtc_driver);
}

module_init(max89xx_rtc_init);

static void __exit max89xx_rtc_exit(void)
{
	platform_driver_unregister(&max8986_rtc_driver);
}

module_exit(max89xx_rtc_exit);

MODULE_DESCRIPTION("RTC driver for the Maxim MAX89xx PMU");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:max89xx-rtc");
