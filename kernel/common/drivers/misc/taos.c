...
#include <linux/wakelock.h> //added by xqj,avoid enter late suspend
...
//#define SMBUS_SUPPORT
#ifndef  SMBUS_SUPPORT
#define i2c_smbus_write_byte_data i2c_taos_write_byte_data
#define i2c_smbus_read_byte   i2c_taos_read_byte
#define i2c_smbus_write_byte i2c_taos_write_byte
#endif
...
static struct wake_lock prox_wake_lock;//added by xqj,avoid enter late suspend
...
static int TAOSI2C_RxData(unsigned char *rxData, unsigned char reg,int length)
{
	uint8_t loop_i;
...
#if 0
	printk(KERN_INFO "RxData: len=%02x, ,slave=%d\n  ", length, this_client->addr);	
#endif
	if (loop_i >= 1) {
		printk(KERN_ERR "%s retry over 1\n", __func__);
		return -EIO;
	}
#if 0
	printk(KERN_INFO "RxData: len=%02x, addr=%02x\n  data=", length, addr);
	for (loop_i = 0; loop_i < length;loop_i++) {
		printk(KERN_INFO " %02x", rxData[loop_i]);
	}
    printk(KERN_INFO "\n");
#endif
	return 0;
...
static int TAOSI2C_TxData(unsigned char  *txData, int length)
{
	uint8_t loop_i;
	struct i2c_msg msg[] = {
		{
	...
	//mdelay(10);////2011,9.13,temprory ,later will remove
	if (loop_i >= 1) {
...
#if 0
	printk(KERN_INFO "TxData: len=%02x, addr=%02x data= ", length, txData[0]);
	for (loop_i= 0; loop_i< (length-1); loop_i++) {
		printk(KERN_INFO "  %02x", txData[loop_i + 1]);
	}
	printk(KERN_INFO "\n");
#endif
	return 0;
}
s32 i2c_taos_write_byte(struct i2c_client *client, u8 value)
{
       s32 ret;
	   ret=TAOSI2C_TxData(&value,1);
	   return ret;
}
s32 i2c_taos_read_byte(struct i2c_client *client)
{
...
s32 i2c_taos_write_byte_data( struct i2c_client *client,u8 command, u8 value)
{
       s32 ret;
       unsigned char txdata[2];
       txdata[0]=command;
       txdata[1]=value;
       ret=TAOSI2C_TxData((unsigned char *)&txdata,2);
      return ret;
}
//=========================================
static int lightsensor_setup()
{
	int ret;
	taos_datap->ls_input_dev = input_allocate_device();
	if (!taos_datap->ls_input_dev) {
		pr_err(
			"[taos error]%s: could not allocate ls input device\n",
			__func__);
		return -ENOMEM;
	}
	taos_datap->ls_input_dev->name = "lightsensor";
	set_bit(EV_ABS, taos_datap->ls_input_dev->evbit);
	input_set_abs_params(taos_datap->ls_input_dev, ABS_MISC, 0, 9, 0, 0);

	ret = input_register_device(taos_datap->ls_input_dev);
	if (ret < 0) {
		pr_err("[taos error]%s: can not register ls input device\n",
				__func__);
...
err_free_ls_input_device:
	input_free_device(taos_datap->ls_input_dev);
	return ret;
}

static int psensor_setup()
{
	int ret;
	taos_datap->ps_input_dev = input_allocate_device();
	if (!taos_datap->ps_input_dev) {
		pr_err(
			"[taos error]%s: could not allocate ps input device\n",
			__func__);
		return -ENOMEM;
	}
	...
	if (ret < 0) {
		pr_err(
			"[taos error]%s: could not register ps input device\n",
			__func__);
    ...
    if(ret < 0) {
        pr_err("[taos ]%s: req_irq(%d) fail (%d)\n", __func__,
                        lpi->irq, ret);
...
err_free_ps_input_device:
	input_free_device(taos_datap->ps_input_dev);
	return ret;
}
...
#endif
        wake_lock_init(&prox_wake_lock, WAKE_LOCK_SUSPEND, "proximity");////added by xqj,avoid enter late suspend
	printk("XXXXXXXXXXXXXXXX taos_probe OK XXXXXXXXXXXXXX ret=%d\n",ret);
         mdelay(10);
#if 0
	//ret = taos_als_on();
	//als_on = 1;
#if ALS_TIMER_FUNC
	//als_timer_count++;
	//taos_als_poll_timer_start(1400);
#endif
...
#if PROX_TIMER_FUNC
		//prox_timer_count++;
	//	taos_prox_poll_timer_start(1400);
#endif
	//	printk("XXXXXXXXX TAOS_LOCTL_PROX_ON ret=%dXXXXXXXXXX\n",ret);
#endif
...
static int taos_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case TAOS_IOCTL_PROX_CALIBRATE:
		...
		if ((ret =
			...
			printk(KERN_ERR "TAOS: i2c_smbus_write_byte_data failed in ioctl prox_on\n");
			//return (ret);
			goto cal_ret;//if calibrate failed ,so it used the previous calibrate value or default value
		}
		...
		for (i = 0; i < 5; i++) {
			if ((ret = taos_prox_poll(&prox_cal_info[i])) < 0 ) {
				printk(KERN_ERR "TAOS: call to prox_poll failed in ioctl prox_calibrate %d\n");
				goto cal_ret;//if calibrate failed ,so it used previous calibrate value or default value
				//return -61;
			}
   ...
   /* mabye some layout design problem, is is easy that proximity value is higher than threhold value in common condition
		,so it should be increase threhold value ,it have been tested ok ,xu qijun*/
		if (taos_cfgp->prox_threshold_hi > ((sat_prox * 70) / 100)) {
			taos_cfgp->prox_threshold_lo = sat_prox * 60 / 100;
			taos_cfgp->prox_threshold_hi = sat_prox * 70 / 100;
		}
		...
		if ((ret =
		      (i2c_smbus_write_byte_data
		       (taos_datap->client, (TAOS_TRITON_CMD_REG | TAOS_TRITON_WAIT_TIME), taos_cfgp->wait_time))) < 0) {
			printk(KERN_ERR "TAOS: i2c_smbus_write_byte_data failed in ioctl prox_on\n");
			//return (ret);//if calibrate failed ,so it used previous calibrate value or default value
	}
...
static int taos_prox_poll(struct taos_prox_info *prxp)
{
	...
	/*read status == 0x30,note modified by xqj,now not need check status reg status,according to TAOS supplier FAE suggest*/
      /*
	if ((ret = (i2c_smbus_write_byte(taos_datap->client, (TAOS_TRITON_CMD_REG | TAOS_TRITON_STATUS)))) < 0) {
	...
	while ((status & 0x30) != 0x30) {
		...
		wait_count++;
		if (wait_count > 30) {
			printk(KERN_ERR "TAOS: Prox status invalid for 300 ms in taos_prox_poll() status=%d\n",status);
			return -ENODATA;
		}
		mdelay(10);
	}
*/
	...
	{
            prxp->prox_event = 1;//modified by xqj,when in strong sun light,proximity will return  always 1,far,old is keep unchange
            return 0;
	}
	...
	prox_history_hi &= 0x01;//xqj,slove prox reponse slow problem ,from three times to one,0x07;
	prox_history_lo <<= 1;
	prox_history_lo |= ((prxp->prox_data > taos_cfgp->prox_threshold_lo) ? 1 : 0);
	prox_history_lo &= 0x01;//xqj,slove prox reponse slow problem ,from three times to one,0x07;
	if (prox_history_hi == 0x01)//xqj,slove prox reponse slow problem ,from three times to one,0x07;
		event = 0;
...
static void taos_prox_work_f(struct work_struct *work)
{
	...
	if (prox_on) {
		...
		/* 0 is close, 1 is far */
	        input_report_abs(taos_datap->ps_input_dev, ABS_DISTANCE, prox_cur_infop->prox_event);
	        input_sync(taos_datap->ps_input_dev);
		wake_lock_timeout(&prox_wake_lock, 2 * HZ);//added by xqj,avoid enter late suspend
...
static void taos_prox_poll_timer_func(unsigned long param)
{
	...
	taos_prox_poll_timer_start(200);//2011.12.06  modified  by xqj,poll time change from 400 to 200ms
}
...
static void taos_als_work_f(struct work_struct *work)
{
	...
	if (als_on) {
		...
		input_report_abs(taos_datap->ls_input_dev, ABS_MISC, lux_value);
	        input_sync(taos_datap->ls_input_dev);
