...
#define KLOG_E(fmt,args...) \
					do { printk(KERN_ERR "Error: [%s:%s:%d][%d:%d] "fmt"\n", KLOG_TAG, __func__, __LINE__, current->pid, current->tgid, \
		    ##args); } \
					while (0)
#else
...
static int bmem_oom_find_adj(void)
{
    ...
    KLOG_V("array_size(%d) bmem_adj_size(%d) bmem_minfree_size(%#x)", array_size, bmem_adj_size, bmem_minfree_size);
...
static int bmem_oom_kill_task(int min_adj, pid_t *tgid_killed)
{
    ...
    for_each_process(p) {
        ...
        if (!mm || !sig) {
			...
			if (min_adj == bmem_adj_on_fail)
				KLOG_D("Skip-kp %s: pid(%d) tgid(%d) bmem(%#x)", p->comm, p->pid, p->tgid, bmem_used_by_tgid);
#endif
            task_unlock(p);
            continue;
        }
        ...
        if (oom_adj < min_adj) {
			...
			if (min_adj == bmem_adj_on_fail)
            KLOG_D("Skip-adj %s: pid(%d) tgid(%d) bmem(%#x) oom_adj(%d) ", p->comm, p->pid, p->tgid, bmem_used_by_tgid, oom_adj);
#endif
            continue;
        }
        ...
        if (selected_bmem_size >= bmem_used_by_tgid) {
            KLOG_D("Skip-size %s: pid(%d) tgid(%d) bmem(%#x) oom_adj(%d) ", p->comm, p->pid, p->tgid, bmem_used_by_tgid, oom_adj);
            continue;
        }
        selected = p;
        selected_bmem_size = bmem_used_by_tgid;
        selected_oom_adj = oom_adj;
        KLOG_D("Select %s: pid(%d) tgid(%d) bmem(%#x) oom_adj(%d) ", selected->comm, selected->pid, selected->tgid, selected_bmem_size, selected_oom_adj);
    }
...
static int bmem_oom_check(void)
{
    int min_adj;
    pid_t tgid_killed;
    int bmem_free = 0, bmem_big_blk = 0;

    if (logic.GetFreeMemorySize != NULL) {
        bmem_free = logic.GetFreeMemorySize();
    }
    if (logic.GetBiggestFreeBlock != NULL) {
        bmem_big_blk = logic.GetBiggestFreeBlock();
    }
    KLOG_D("Status %s: free(%#x) big(%#x) ", current->comm, bmem_free, bmem_big_blk);
...
static int bmem_oom_on_failure(int bmem_to_be_freed)
{
    ...
    KLOG_D("bmem fail current %s: [pid(%d)] bmem_to_be_freed(%#x) ",
	...
	{
        int bmem_free = BMEM_SIZE, bmem_big_blk = 0, bmem_used_by_tgid = 0, oom_adj = OOM_ADJUST_MAX + 1;
		...
		KLOG_D("bmem freed current %s: [pid(%d) bmem(%#x) oom_adj(%d)] bmem_free(%#x) big(%#x) bmem_to_be_freed(%#x) bmem_freed_sum(%#x) ",
			current->comm, current->tgid, bmem_used_by_tgid, oom_adj, bmem_free, bmem_big_blk, bmem_to_be_freed, bmem_freed_sum);
	}
...
static int bmem_parse_string(const char *inputStr, u32 *opCode, u32 *arg)
{
	...
	*opCode = 0;
	...
	else if (strcmp(tempStr, "test_oom") == 0) {
		*opCode = 6;
	}
	...
	case GEMEMALLOC_WRAP_ACQUIRE_BUFFER:
		{
			...
			if (page_aligned_size == 0) {
				...
				if (logic.AllocMemory != NULL) {
					down(&bmem_sem);
					KLOG_D ("GEMEMALLOC_WRAP_ACQUIRE_BUFFER: size[%#x]", memparams.size);
					bmem_oom_check();
					result =
					...
					if (result) {
						if (bmem_oom_on_failure(page_aligned_size) >= page_aligned_size) {
                            KLOG_D("Retry size(%#x)", page_aligned_size);
                            bmem_oom_check();
							result =
								logic.AllocMemory(p_data->bmem_handle,
									&memparams.busAddress,
					...
					bmem_oom_check();
					KLOG_D ("GEMEMALLOC_WRAP_ACQUIRE_BUFFER: result[%d]", result);
					up(&bmem_sem);
				} else {
	...
	case GEMEMALLOC_WRAP_RELEASE_BUFFER:
		{
			...
			if (logic.FreeMemory != NULL) {
				down(&bmem_sem);
                KLOG_D ("GEMEMALLOC_WRAP_RELEASE_BUFFER ");
                bmem_oom_check();
				result = logic.FreeMemory(p_data->bmem_handle, &busaddr);
                bmem_oom_check();
                KLOG_D ("GEMEMALLOC_WRAP_RELEASE_BUFFER result(%d)", result);
				up(&bmem_sem);
	...
	case HANTRO_WRAP_ACQUIRE_BUFFER:
		{
			...
			if (page_aligned_size == 0) {
				...
				if (logic.AllocMemory != NULL) {
					down(&bmem_sem);
					KLOG_D ("HANTRO_WRAP_ACQUIRE_BUFFER: size[%#x]", memparams.size);
					bmem_oom_check();
					result =
						logic.AllocMemory(p_data->bmem_handle,
								  &memparams.busAddress,
					...
					bmem_oom_check();
					KLOG_D ("HANTRO_WRAP_ACQUIRE_BUFFER: result[%d]", result);
					up(&bmem_sem);
				} else {
					KLOG_E("AllocMemory() is NULL");
					memparams.busAddress = 0;
	...
	case HANTRO_WRAP_RELEASE_BUFFER:
		{
			...
			if (logic.FreeMemory != NULL) {
				down(&bmem_sem);
                KLOG_D ("HANTRO_WRAP_RELEASE_BUFFER ");
                bmem_oom_check();
				result = logic.FreeMemory(p_data->bmem_handle, &temp_hw_OutBuf.busAddress);
                bmem_oom_check();
                KLOG_D ("HANTRO_WRAP_RELEASE_BUFFER result(%d)", result);
				up(&bmem_sem);
...
static int bmem_wrapper_release(struct inode *inode, struct file *filp)
{
	...
	if ((p_data) && (logic.release)) {
        ...
        bmem_oom_check();
        KLOG_D ("file[%p] handle[%p] freed", filp, p_data->bmem_handle);
		up(&bmem_sem);
	}

	KLOG_V ("file[%p] handle[%p] freed", filp, p_data->bmem_handle);
	if (p_data)
		kfree(p_data);
				   ...
				   struct vm_area_struct *vma)
{
	...
	if (0 == r) {
		...
		if (is_pmem_alloc_needed(file, vma)) {
			...
			if (logic.AllocMemory != NULL) {
				down(&bmem_sem);
				KLOG_D ("Allocate memory for pmem: size[%#x]", (int)page_aligned_size);
				bmem_oom_check();
				r = logic.AllocMemory(p_data->bmem_handle, &busAddress, page_aligned_size);
				if (r) {
					if (bmem_oom_on_failure(page_aligned_size) >= page_aligned_size) {
                        bmem_oom_check();
                        KLOG_D ("Retry size(%#x)", (int)page_aligned_size);
						r = logic.AllocMemory(p_data->bmem_handle, &busAddress, page_aligned_size);
					}
				}
				bmem_oom_check();
				KLOG_D ("Allocate memory for pmem: result[%d]", (int)r);
				up(&bmem_sem);
			} else {
				KLOG_E("AllocMemory() is NULL");
				return -EAGAIN;
