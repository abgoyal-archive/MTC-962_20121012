...
struct cpufreq_client_desc *lcdc_client;
#endif
...
struct wake_lock glcdfb_wake_lock;
#endif
//static struct semaphore gDmaSema;
#ifdef CONFIG_REGULATOR
struct regulator *lcdc_regulator = NULL;
#endif
...
*
***************************************************************************/
inline int lcd_pm_update(PM_CompPowerLevel compPowerLevel,
...
static int __init lcdc_probe(struct platform_device *pdev)
{
	...
	pr_info("lcd:probe lcdc_base = 0x%x gpio = %d\n", (int)lcdc_base,
		lcd_reset_gpio);
	printk("lcd:probe lcdc_base = 0x%x gpio = %d\n", (int)lcdc_base,
		lcd_reset_gpio);
	...
	if (!pdata->te_supported) {
		...
		printk("lcdc: Disabling te_support since the board doesn't support\n");
...
//#endif
	...
	lcd_interface->lcd_get_interface_info(&lcd_dest_phy_addr);
